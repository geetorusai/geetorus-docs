...
Geetorus has two separate identity flows for humans on the command line and browser:
...
- **CLI auth** — the everyday flow that pairs a local `geetorusai` CLI process with a signed-in board user, so the CLI can call `/api` endpoints on your behalf without you typing credentials into your shell.
...
- You just installed Geetorus in **authenticated** deployment mode on a server, and the loopback trust is still active → run **Board claim** once.
- You want to run `geetorusai` commands against that instance from your laptop → run **CLI auth** (`geetorusai auth login`).
...
## Device-code flow: `geetorusai auth login`
...
Your browser lands on a page titled **Approve Geetorus CLI access**:

The panel shows what the CLI is asking for:

- **Command** — the CLI command that initiated the login (e.g. `geetorusai auth login`).
- **Client** — the `clientName` the CLI sent, default `geetorusai cli`.
...
{
  "ok": true,
  "apiBase": "https://geetorus.example.com",
  "userId": "usr_...",
  "approvalUrl": "https://geetorus.example.com/cli-auth/..."
}
```

If you cancel or let the challenge expire, the CLI errors out with `CLI auth challenge was cancelled.` or `CLI auth challenge expired before approval.` — re-run `geetorusai auth login` to try again.
...
All other CLI commands — the ones documented in [Setup Commands](../reference/cli/setup-commands.md) and Control-Plane Commands — reuse this credential unless you pass an explicit `--token` or override `GEETORUS_API_KEY`.
...
If the server-side revoke fails (network error, token already invalid) the local credential is still removed — you won't be stuck with a stale entry. A new `geetorusai auth login` mints a fresh challenge and a fresh token with no relationship to the old one.
...
- **"Invalid CLI auth URL."** — You opened the approval page with a missing `id` or `token` query param. Re-run `geetorusai auth login` to get a fresh link.
...
- **"CLI auth challenge expired before approval."** on the CLI — You let the browser tab sit too long. Just rerun `geetorusai auth login`.
...
- [Setup Commands](../reference/cli/setup-commands.md) — installing, onboarding, and configuring a Geetorus instance from the CLI.
