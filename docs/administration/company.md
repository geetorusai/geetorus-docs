...
When no logo is set, Geetorus draws an auto-generated pattern icon whose hue is derived from the organization name — there is no separate brand-color setting to pick. A live preview of the tile is rendered to the left of the field so you can see the result before saving.
...
The **Feedback Sharing** section controls whether AI outputs you explicitly vote on in Geetorus may also be shared with Geetorus Labs. The toggle is labelled **Allow sharing voted AI outputs with Geetorus Labs**.
...
Geetorus uses four human membership roles. The role determines a set of implicit permission grants:
...
> **Note:** Geetorus's agent chain of command (CEO, managers, reports) is separate from these human roles. Human roles gate access to the Geetorus UI and company-level actions; agent roles describe the reporting tree inside the company. See [Org Structure](../guides/org/org-structure.md) for the agent side.
...
If you do not have permission to manage members, Geetorus replaces the page body with a message explaining why. An instance admin who is viewing the page without an active company membership sees a banner noting that the current account has admin-level access but is not a member of this company.
...
1. Generates a new invite link against the current Geetorus domain.
...
If the clipboard is unavailable, Geetorus shows a warning toast and you can copy the URL manually from the field. The invite URL is clickable as an **Open invite** button for quick testing in another tab.
...
When a human opens the invite URL, they land on a Geetorus join page that creates a join request tied to the invite. If the invite carries a default role (for example **Operator**), that role is attached to the request so approvers can see it in context. The member does not become active until someone with `joins:approve` approves the request.
...
- `README.md` and `.geetorus.yaml` are regenerated on the fly so they always reflect the current selection.
...
Click **Export N files** to build the final package. Geetorus assembles the zip with the filtered files (including the regenerated README and `.geetorus.yaml`) and triggers a browser download. A success toast confirms the file count and the zip's root path.
...
The **Company Import** page is the reverse direction: it takes an exported package and loads it into Geetorus, either as a new company or on top of an existing one.
...
- **Local zip** — upload a `.zip` package exported from Geetorus. Geetorus warns against re-zipping archives in Finder or Explorer; use the download from the Export page.
...
- **Rename on conflict** — the default. Geetorus proposes a new name (prefixed with the package name, for example `gstack-CEO`) for each conflicting entity.
...
3. A file tree identical in layout to the export tree, but with an **action badge** on each row (`create`, `update`, `skip`, or `replace`) so you can see what Geetorus plans to do to every file. Directories that will be renamed show an arrow and the target name.

The sticky bar at the top counts selected files and surfaces the number of conflicts and errors.

### Applying the import

When you are happy with the preview, click **Import N files**. The button is disabled if there are errors in the preview or no files selected. On success, Geetorus refreshes the companies list, navigates you to the dashboard of the imported (or updated) company, and restores the imported sidebar order if the package included one.
...
Company administration covers the pieces of Geetorus that sit around the agents rather than inside them: identity, membership, and portability. Once you have the company set up and people invited in, day-to-day work happens in the approvals queue, the dashboard, and the task surfaces that the rest of this guide set covers.
