...
Your choice is remembered in the browser (under `geetorus.secrets.viewMode` in local storage), so the screen comes back the way you left it. Until you pick a side, Geetorus guesses: if any secret name contains a slash you land in Folders, and if none do you get Flat — a company that never used prefixes never sees a folder tree it doesn't need.
