...
Geetorus has two settings surfaces, and it matters which one you're looking at.
...
**Instance Settings** is the admin surface. Anything you change there applies to the whole Geetorus instance — every company, every user, every agent that runs against this install. If you're running Geetorus on a VPS that hosts five companies, a toggle on the Instance page flips for all five.
...
- **Local trusted** — the instance is treating browser requests as a local board operator. No sign-in is required. This is the default when you run Geetorus on your own machine.
...
Controls how long automatic database backups are kept at each tier. Geetorus takes daily backups and then thins them out over time:
...
Controls whether thumbs-up / thumbs-down votes on AI output can send the voted output to Geetorus Labs. Three states exist:

- **Prompt (default)** — no choice has been made yet. The next time you vote, Geetorus will ask once and save your answer.
...
- **Built-in Adapters** — the adapters that ship inside Geetorus itself. They cannot be removed, only hidden. If an external adapter has overridden a built-in (for example, a forked version of the Claude adapter installed from npm), you also see a synthesised "built-in" row tagged **Overridden by …** so it is obvious which built-in has been shadowed.
...
- **Reload** (circular arrow) hot-swaps the adapter module in the running process. Useful after you have published a new version of a local-path adapter and want to pick up changes without bouncing Geetorus.
...
A **Remove** (trash) icon is available on external adapters only. Removing an adapter unregisters it, tears down its npm install on disk, and cannot be undone — Geetorus prompts for confirmation first.
...
- **npm package** — enter the package name (for example `my-geetorus-adapter`) and optionally a version. Geetorus installs the package from npm and registers it.
- **Local path** — paste a Linux, WSL, or Windows path to a checked-out adapter package. Geetorus auto-converts Windows paths and installs the adapter from disk. Local-path adapters are the fastest way to iterate on a new adapter before publishing.

In both cases the adapter package must export `createServerAdapter()` — the dialog reminds you of this contract.

For the full catalog of adapters Geetorus supports out of the box, along with the per-agent fields each one expects, see the reference docs: [Adapters overview](../reference/adapters/overview.md).
...
An experimental flag in Geetorus is a feature that:
...
- **[Auto-Restart Dev Server When Idle](../experimental/auto-restart-dev-server.md)** — only relevant under `pnpm dev:once`; restarts a stale dev boot once all local agent runs finish. For development on Geetorus itself, not production.
...
If you run Geetorus for other people — a managed cloud, an internal shared server — you can shape which settings surfaces your users see and what the defaults are, without touching any user's data. Two environment variables do this. Both are read at boot, applied at read time, and **never persisted**: clear the variable and stock behavior comes back everywhere a user hasn't chosen otherwise. With both unset, the UI and API behave exactly as this page describes.

### Hide surfaces with `GEETORUS_HIDDEN_SETTINGS`

Set `GEETORUS_HIDDEN_SETTINGS` to a comma-separated list of surface keys to remove from the UI. Some keys also floor their mutation API with a `403` (carrying the `settings_operator_managed` code); the rest hide UI only, so agents and integrations keep working against the underlying routes.
...
### Override defaults with `GEETORUS_SETTING_DEFAULTS`

Set `GEETORUS_SETTING_DEFAULTS` to a JSON object to replace the schema default of selected Instance General settings — currently `feedbackDataSharingPreference`. For example:

```sh
GEETORUS_SETTING_DEFAULTS='{"feedbackDataSharingPreference":"allowed"}'
```
