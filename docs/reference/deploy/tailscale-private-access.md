...
seo_description: Reach a Geetorus instance from a private network rather than localhost only — the practical path to an authenticated, private deployment.
---

# Tailscale Private Access

Use this page when you want a Geetorus instance that is reachable from a private network, not just from `localhost`.
...
Start Geetorus with the private authenticated dev mode:
...
- `GEETORUS_AUTH_BASE_URL_MODE=auto`
- `HOST=0.0.0.0`
...
From the machine that is running Geetorus:
...
Use the private-network host or IP with the Geetorus port:
...
If you use a private hostname that Geetorus has not seen before, add it to the allowlist:

```sh
pnpm geetorusai allowed-hostname my-macbook.tailnet.ts.net
```
...
- If login or redirect errors mention the hostname, add it with `geetorusai allowed-hostname`.
