...
seo_description: Geetorus runs PostgreSQL through Drizzle ORM. One schema across every supported mode — what differs is where Postgres runs and how it is reached.
---

# Database

Geetorus uses PostgreSQL through Drizzle ORM. The same schema works across the supported database modes; the difference is where PostgreSQL is running and how Geetorus connects to it.
...
On first start, Geetorus:

1. creates `~/.geetorus/instances/default/db/`
