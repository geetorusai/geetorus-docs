...
seo_description: Every variable Geetorus reads for server configuration, plus the ones it injects into agent processes at runtime — the list to wire deployments from.
---

# Environment Variables

This page lists the environment variables Geetorus reads for server configuration and the variables it injects into agent processes at runtime.
...
| `GEETORUS_BIND` | inferred from `HOST` | Bind mode for the server socket. One of the values in `BIND_MODES` (see `packages/shared`); overrides `server.bind` in `config.json`. |
| `GEETORUS_BIND_HOST` | inferred | Custom host when `GEETORUS_BIND` is set to a custom mode; overrides `server.customBindHost`. |
| `GEETORUS_TAILNET_BIND_HOST` | auto-detected via `tailscale ip -4` | Tailnet IPv4 address the server binds to when bind mode is `tailnet`. Set explicitly to skip the `tailscale` CLI probe. |
| `GEETORUS_WORKSPACE_GIT_SCAN_CONCURRENCY` | `2` | How many expensive full-tree workspace Git scans may run at once. Clamped to 1–16. |
| `GEETORUS_WORKSPACE_GIT_SCAN_QUEUE_CAPACITY` | `32` | How many scans may wait in the queue before new ones are rejected. Clamped to 0–1024. |
| `GEETORUS_WORKSPACE_GIT_SCAN_TIMEOUT_MS` | `8000` | Per-scan timeout, in milliseconds, before a workspace Git scan is abandoned. Clamped to 100–120000. |
| `GEETORUS_WORKSPACE_GIT_SCAN_CACHE_TTL_MS` | `10000` | How long a completed scan's result is reused before a fresh scan runs, in milliseconds. Clamped to 0–60000. |
| `GEETORUS_WORKSPACE_REAPER_COOLDOWN_DAYS` | `7` | How many days the terminal-workspace reaper waits after an issue tree becomes terminal before archiving its workspace. Someone can reopen the work inside this window. `0` disables the cooldown and restores immediate reaping; a negative or non-numeric value falls back to the default. |
...
| Variable | Meaning |
|---|---|
| `GEETORUS_PUBLIC_URL` | Canonical public URL for invites, redirects, and auth origin wiring. |
| `GEETORUS_AUTH_PUBLIC_BASE_URL` | Explicit auth base URL when you want Better Auth to use a fixed public origin. |
| `BETTER_AUTH_URL` | Alternate Better Auth base URL input. |
| `BETTER_AUTH_SECRET` | Signing secret for Better Auth sessions and tokens. Falls back to `GEETORUS_AGENT_JWT_SECRET` when unset; the server refuses to start if neither is configured. For local development the `.env.example` ships `geetorus-dev-secret`. |
...
| `GEETORUS_AGENT_JWT_SECRET` | Secret used to mint agent API JWTs. Required for local adapter auth. |
| `GEETORUS_AGENT_JWT_TTL_SECONDS` | Agent JWT lifetime in seconds. |
| `GEETORUS_AGENT_JWT_ISSUER` | Agent JWT issuer. |
| `GEETORUS_AGENT_JWT_AUDIENCE` | Agent JWT audience. |
| `GEETORUS_TOOL_ACTION_SIGNING_SECRET` | Signing secret for tool action approvals. Unlike `BETTER_AUTH_SECRET` it has no fallback — if it is unset, the server cannot issue signed tool action approvals and raises `ToolActionSigningSecretMissingError`. Set it in the instance's own environment; worktrees inherit it from `.geetorus/.env`. For local development the `.env.example` ships `geetorus-dev-tool-action-signing-secret-change-me`. |

Related deployment variables:

| Variable | Meaning |
|---|---|
| `GEETORUS_DEPLOYMENT_EXPOSURE` | Exposure policy override, typically `private` or `public` in authenticated mode. |
| `GEETORUS_AUTH_BASE_URL_MODE` | Base URL handling mode, such as `auto` or `explicit`. |
| `GEETORUS_ALLOWED_HOSTNAMES` | Comma-separated allowlist for authenticated/private host validation. |
...
> **Tip:** If `geetorusai doctor` is failing on hostnames, redirects, or auth origins, inspect this group first.

### Trusting a reverse proxy (`TRUST_PROXY`)

When you run Geetorus behind a load balancer or reverse proxy (nginx, Caddy, a cloud LB), the real client IP arrives in the `X-Forwarded-For` header rather than on the socket. `TRUST_PROXY` tells the server how far to trust that header so `req.ip` and rate-limiting see the actual client instead of your proxy.
...
| Variable | Default | Meaning |
|---|---|---|
| `GEETORUS_MANAGED_CONFIG` | unset (self-hosted) | Set only by the Geetorus Cloud harness. Holds one JSON document describing which experimental feature flags the harness manages for this instance, the plugin keys it declares for auto-install, and optionally the sandbox environment it provisions for you at boot. **Absent** means self-hosted: no overlay, no behavior change. **Present but blank, or malformed, makes the server refuse to start.** |
...
Geetorus publishes that tier map per release as a `feature-catalog.json` artifact — generated by `scripts/generate-feature-catalog.ts --version <catalogVersion>` — which the cloud harness imports and validates its feature writes against.
...
In the UI, every key the overlay controls renders with a lock badge reading **Managed by Geetorus Cloud** and a disabled toggle. Self-hosted responses carry no managed keys at all, so every toggle stays editable. See [Experimental features](../../experimental/overview.md#if-a-toggle-is-locked) for the user-facing view.
...
**At most one entry.** The database enforces a single Geetorus-managed sandbox row per instance (a partial unique index named `environments_managed_sandbox_idx`), and every entry in the list provisions that one row — so a longer list could never be satisfied and is rejected at parse time. For the same reason, a **non-empty** `environments` section and a forced `GEETORUS_EXECUTION_MODE` are mutually exclusive: both claim the same row, and configuring both stops the server rather than letting boot order pick a winner. An empty `"environments": []` declares nothing, so it sits alongside a forced execution mode without complaint.
...
If the provider isn't up, Geetorus skips the entry rather than writing an active row — and if an earlier boot already provisioned one, it archives that row so run scheduling stops selecting an environment whose leases would fail. The server logs the reason and keeps booting; the cost is that this one environment is unavailable, not a fleet stuck in a crash loop.

Recovery is automatic in the common case. When the plugin record is `ready` and only its worker is down — a crash in restart-backoff — Geetorus listens for the worker manager to respawn it and re-runs the setup the moment it comes back, so the environment returns without anyone restarting the server. A missing or non-`ready` plugin needs the next healthy boot instead.
...
| Variable | Meaning |
|---|---|
| `GEETORUS_SECRETS_MASTER_KEY` | 32-byte encryption key as base64, hex, or raw |
| `GEETORUS_SECRETS_MASTER_KEY_FILE` | Path to the local key file |
| `GEETORUS_SECRETS_STRICT_MODE` | Require secret refs for server-side env bindings. Does not apply to `geetorusai configure --section llm` or `config.llm.apiKey`. |
...
| Variable | Meaning |
|---|---|
| `GEETORUS_STORAGE_PROVIDER` | Storage backend, usually `local_disk` or `s3`. |
| `GEETORUS_STORAGE_LOCAL_DIR` | Base directory for local-disk storage. |
| `GEETORUS_STORAGE_S3_BUCKET` | S3 bucket name. |
| `GEETORUS_STORAGE_S3_REGION` | S3 region. |
| `GEETORUS_STORAGE_S3_ENDPOINT` | Custom S3-compatible endpoint for MinIO, R2, and similar providers. |
| `GEETORUS_STORAGE_S3_PREFIX` | Optional object key prefix. |
| `GEETORUS_STORAGE_S3_FORCE_PATH_STYLE` | Enable path-style S3 requests when the provider needs them. |
| `GEETORUS_IMPORT_ZIP_MAX_BYTES` | Maximum size, in bytes, of a company-import `.zip` upload. Defaults to `1073741824` (1 GB). Set it higher to accept larger packages, or lower to tighten the cap. Values are clamped to the range 1 byte – 64 GiB; anything non-numeric, fractional-to-zero, or out of range falls back to the 1 GB default. |
...
| Variable | Default | Meaning |
|---|---|---|
| `GEETORUS_FEEDBACK_EXPORT_BACKEND_URL` | unset | URL of the external feedback collector. When set, the server forwards `geetorusai feedback` submissions to this endpoint. |
| `GEETORUS_FEEDBACK_EXPORT_BACKEND_TOKEN` | unset | Bearer token used to authenticate the forwarding request. |
| `GEETORUS_TELEMETRY_BACKEND_URL` | unset | Legacy alias for `GEETORUS_FEEDBACK_EXPORT_BACKEND_URL`. Honoured for backwards compatibility — set the feedback variant in new deployments. |
| `GEETORUS_TELEMETRY_BACKEND_TOKEN` | unset | Legacy alias for `GEETORUS_FEEDBACK_EXPORT_BACKEND_TOKEN`. |

If neither variable is set, feedback submissions are stored locally and never leave the instance.

### Turning off telemetry

Geetorus ships anonymized usage telemetry (which adapters you run, and similar coarse signals) that helps the project understand how Geetorus is used. It carries no personal data, and you can turn it off completely by setting either of these variables. Both are checked for the exact value `1`, and either one on its own disables telemetry.

| Variable | Meaning |
|---|---|
| `GEETORUS_TELEMETRY_DISABLED` | Set to `1` to disable Geetorus's anonymized telemetry. |
...
Geetorus can emit distributed traces over OpenTelemetry (OTLP) so you can watch requests flow through the server in a tracing backend like Jaeger, Tempo, or Honeycomb. It is **opt-in and off by default** — nothing is loaded until you point it at a collector.
...
| `OTEL_SERVICE_NAME` | `geetorus` | Service name reported on spans. |
...
Geetorus can report server-side errors to [Sentry](https://sentry.io). It is **opt-in and off by default**, and everything about it fails open — no DSN, no package, no Sentry, no problem.
...
If you host Geetorus for other people, these two variables let you shape which settings surfaces your users see and what the defaults are. Both are read at boot, applied at read time, and **never persisted** — clearing the variable restores stock behavior. With both unset, the UI and API behave exactly as they did before these controls existed. See [Instance Settings](../../administration/settings.md) for the full walkthrough.

| Variable | Default | Meaning |
|---|---|---|
| `GEETORUS_HIDDEN_SETTINGS` | unset | Comma-separated list of settings-surface keys to hide from the UI — instance pages, company pages (Members, Invites, Secrets, Export, Import), individual sections and tabs, and individual experimental flags. Some keys also floor their mutation API with a `403`; the rest hide UI only. Unknown keys are warned about and ignored, so one list can roll across a fleet of mixed app versions. |
| `GEETORUS_SETTING_DEFAULTS` | unset | JSON object that replaces the schema default of selected instance settings, e.g. `{"feedbackDataSharingPreference":"allowed"}`. An explicit user choice always wins; only values still sitting at the schema default resolve to yours. Malformed JSON or an invalid value for a known field stops the server from booting; unknown field names are warned about and ignored. |
...
|---|---|---|
| `GEETORUS_AGENT_ID` | yes | Agent ID. |
| `GEETORUS_COMPANY_ID` | yes | Company ID. |
| `GEETORUS_API_URL` | yes | Geetorus API base URL. |
| `GEETORUS_API_KEY` | local adapters | Short-lived JWT for API auth. Use as `Authorization: Bearer $GEETORUS_API_KEY`. For non-local adapters, the operator sets this in adapter config. |
| `GEETORUS_RUN_ID` | yes | Current heartbeat run ID. Pass back as the `X-Geetorus-Run-Id` header on any request that mutates an issue, so server-side audit log entries link to this run. |
| `GEETORUS_TASK_ID` | wake-driven | Issue that triggered the wake. Empty for scheduled or unsolicited wakes. |
| `GEETORUS_WAKE_REASON` | wake-driven | Why this run was triggered. See enum below. |
| `GEETORUS_WAKE_COMMENT_ID` | comment wakes | Specific comment that triggered the wake (set with `issue_commented` and `issue_comment_mentioned`). |
| `GEETORUS_WAKE_PAYLOAD_JSON` | some adapters | Inline JSON wake payload: a compact issue summary plus the ordered batch of new comment payloads. Adapters that inject this let an agent skip the initial `GET /api/issues/:id` and `GET /api/issues/:id/comments` round-trips on comment wakes. |
| `GEETORUS_APPROVAL_ID` | approval wakes | Resolved approval ID. |
| `GEETORUS_APPROVAL_STATUS` | approval wakes | Approval decision. |
| `GEETORUS_LINKED_ISSUE_IDS` | optional | Comma-separated linked issue IDs. |

Use these values when your agent runtime needs to authenticate back to Geetorus or understand what context triggered the run.

### `GEETORUS_WAKE_REASON` values

| Value | When it fires |
|---|---|
| `issue_assigned` | A task was newly assigned to this agent. |
| `issue_commented` | A new comment was posted on an issue this agent owns. The triggering comment id is in `GEETORUS_WAKE_COMMENT_ID`. |
...
| `approval_resolved` | An approval the agent requested was approved or rejected. `GEETORUS_APPROVAL_ID` and `GEETORUS_APPROVAL_STATUS` are populated. |
...
When Geetorus realizes an execution workspace, it can also inject workspace-specific variables such as:

- `GEETORUS_WORKSPACE_CWD`
- `GEETORUS_WORKSPACE_PATH`
- `GEETORUS_WORKSPACE_REPO_ROOT`
- `GEETORUS_WORKSPACE_BRANCH`
- `GEETORUS_PROJECT_ID`
- `GEETORUS_ISSUE_ID`
...
> **Audit trail:** Every mutating API request from an agent run should include the `X-Geetorus-Run-Id: $GEETORUS_RUN_ID` header. The server uses it to attribute issue updates, comments, checkouts, and subtasks to the heartbeat run that produced them. Read-only requests do not require it.

### How `GEETORUS_*` env bindings reach the run

You can define your own environment variables on an agent, project, routine, or adapter config, and they flow into the run environment — including ones you deliberately name with a `GEETORUS_` prefix (for example a secret called `GEETORUS_CLOUD_PROD_PROVIDER_RAILWAY_TOKEN`). The server applies one simple policy when it builds each run's environment:

- **`GEETORUS_API_KEY` is never accepted** from your agent, project, routine, or adapter config. The harness mints a short-lived run token for every run, and that minted token is the only source of `GEETORUS_API_KEY`.
- **Harness-assigned runtime variables always win.** The variables Geetorus sets for the run itself (like `GEETORUS_RUN_ID`, `GEETORUS_AGENT_ID`, and the wake/workspace variables above) take precedence over any same-named binding you define, so you cannot accidentally shadow them.
- **Every other `GEETORUS_*` binding flows through** to the run environment just like a variable with no prefix. Naming a value with the `GEETORUS_` prefix no longer causes it to be dropped.

> **Upgrade note (v2026.722.0):** Earlier releases stripped *every* `GEETORUS_`-prefixed binding before resolving the run environment, which silently dropped custom secrets you had named with that prefix — those now pass through as expected. In the same change, a static `GEETORUS_API_KEY` you set in adapter or config env to override the run token no longer has any effect. If you relied on that override, drop it: the harness-minted run token is now the only source of the run API key.
...
The local CLI adapters can be pointed at a custom or remote OpenAI-compatible gateway through these server-read variables. Each takes a JSON value that Geetorus writes into the adapter's own runtime config before a run, so you can route an adapter at your own provider without editing the agent's machine by hand. The full JSON shape and behaviour for each live on the adapter's reference page.

| Variable | Adapter | Meaning |
|---|---|---|
| `GEETORUS_CODEX_PROVIDERS` | `codex_local` | JSON of custom providers (and an optional `model_provider`) written into Codex's managed `config.toml`. See [Codex](../adapters/codex.md). |
| `GEETORUS_PI_PROVIDERS` | `pi_local` | JSON of custom providers written into Pi's managed `models.json`. See [Pi](../adapters/pi.md). |
| `GEETORUS_OPENCODE_PROVIDERS` | `opencode_local` | JSON merged into OpenCode's `provider` config. See [OpenCode](../adapters/opencode.md). |
| `GEETORUS_OPENCODE_SMALL_MODEL` | `opencode_local` | Sets OpenCode's `small_model` (the auxiliary/helper model). See [OpenCode](../adapters/opencode.md). |

Values support `{env:VAR}` placeholders, which are expanded server-side so secrets stay out of the stored JSON.

---

## Geetorus ID Connector (Gmail OAuth broker)

These variables point your instance at the Geetorus ID OAuth broker that brokers Gmail and Google Workspace connections for your agents. They are optional — leave them unset unless you run against a self-managed broker. Enroll the instance with Geetorus Cloud first, and keep both private keys in your deployment's secret manager rather than in plain config.
...
| Variable | Meaning |
|---|---|
| `GEETORUS_ID_CONNECTOR_BASE_URL` | Broker base URL. Must use HTTPS; plain `http://` is accepted only for loopback hosts (for example `http://localhost:3000`). URLs carrying userinfo, a query string, or a fragment are rejected. |
| `GEETORUS_ID_CONNECTOR_ENVIRONMENT` | Broker environment. One of `development`, `staging`, or `production`; any other value is rejected. |
| `GEETORUS_ID_CONNECTOR_INSTANCE_ID` | Instance identifier issued when you enroll with the broker. |
| `GEETORUS_ID_CONNECTOR_SIGN_PRIVATE_KEY` | Private signing key for connector requests. Keep it in your secret manager. |
| `GEETORUS_ID_CONNECTOR_SEAL_PRIVATE_KEY` | Private sealing key for connector payloads. Keep it in your secret manager. |
...
| Variable | Default | Meaning |
|---|---|---|
| `GEETORUS_HTTP_ADAPTER_PRIVATE_ENDPOINT_ALLOWLIST` | unset (public origins only) | Comma-separated list of exact origins that may be reached over private networking. Each entry is a full origin — scheme, host, and port, like `http://hooks.internal.example:8080` — matched case-insensitively. Entries must be `http` or `https`, carry no path beyond `/`, and no userinfo, query string, or fragment; anything else in the list is ignored. |
