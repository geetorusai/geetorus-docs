...
A *provider vault* is a per-company routing record that tells Geetorus how to talk to an external secret store (AWS Secrets Manager today; GCP Secret Manager and HashiCorp Vault save as draft metadata only). Each vault stores the address, namespace, and prefixes the provider needs — never the credentials. Server-side credentials still come from the host's normal chain (env vars, instance role, `~/.aws/credentials`).

Vaults live under **Company Settings → Secrets → Provider vaults**. The page exposes one section per provider with an **Add vault** button, and shows health, default status, and removal controls on each card. Removing a vault from the UI drops only the Geetorus-side routing record — for AWS Secrets Manager the confirmation dialog spells this out explicitly: "This does not delete the remote AWS Secrets Manager vault, secrets, or any AWS data."
...
| `GEETORUS_SECRETS_MASTER_KEY_FILE` | Custom path to the local key file |
...
Strict mode does **not** apply to `geetorusai configure --section llm`. That command writes plaintext to `config.llm.apiKey` regardless of `GEETORUS_SECRETS_STRICT_MODE`.
...
> **Note:** `pnpm secrets:migrate-inline-env` is a repository-source script. It is available only from a Geetorus git checkout, not from an installed `geetorusai` CLI.
