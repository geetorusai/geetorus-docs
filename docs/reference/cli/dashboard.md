...
The `dashboard` command is the at-a-glance company read from the terminal — the same top-level numbers a human sees on the Geetorus dashboard, fetched in one call. Reach for it when you want a fast health check: how many agents are running, how much work is open, what the month's spend looks like, and whether anything is waiting on a human. It is read-only, so it is safe to run anywhere, on a loop, or inside a monitoring script.
...
```sh
geetorusai dashboard get --company-id <company-id>
```
...
| Flag | Use |
|---|---|
| `-C, --company-id <id>` | The company to summarize. Required unless resolved from `GEETORUS_COMPANY_ID` or your context profile. |
...
| `-d, --data-dir <path>` | Geetorus data directory root (isolates state from `~/.geetorus`). |
| `-c, --config <path>` | Path to the Geetorus config file (used to infer the local API base). |
...
If none of those produce a company, the command fails with a clear error telling you to pass `--company-id`, set `GEETORUS_COMPANY_ID`, or set a profile default with `geetorusai context set`.

For an agent persona whose profile is already pinned to one company, the bare command works without any flag:

```sh
geetorusai dashboard get
```
...
```sh
geetorusai dashboard get --company-id <company-id> --json
```
...
geetorusai dashboard get -C <company-id> --json | jq '.costs.monthUtilizationPercent'

# How many approvals are waiting on a human
geetorusai dashboard get -C <company-id> --json | jq '.pendingApprovals'

# Agents currently in an error state
geetorusai dashboard get -C <company-id> --json | jq '.agents.error'

# Failed runs over the reported activity window
geetorusai dashboard get -C <company-id> --json | jq '[.runActivity[].failed] | add'
```
...
1. `geetorusai dashboard get -C <company-id>` for the headline state.
