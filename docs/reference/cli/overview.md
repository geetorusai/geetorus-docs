...
seo_description: The geetorusai CLI stands up an instance and gives you a full-parity, terminal-based way to operate a company — everything the UI does, scriptable.
---

# CLI Overview

The Geetorus CLI (`geetorusai`) stands up a Geetorus instance and gives you a full-parity, terminal-based way to operate a company against it. Geetorus is UI-first — the web UI is the primary surface most operators use — and the CLI is the alternative for when the terminal is the better tool: installing and configuring an instance, automation and CI, scripted or bulk operations, and operating headless without a browser. Reach for this page first: it explains the two layers the CLI is built from, who you authenticate as, and where every reference page and guide lives.
...
| **Control plane** | Manage companies, agents, issues, goals, runs, budgets, and more | The Geetorus server API | Yes |

**Setup commands** get a server up and healthy. They work against config on disk (under `~/.geetorus` by default) and the local process. The headline commands are `onboard`, `doctor`, `configure`, `env`, `allowed-hostname`, `db:backup`, and `run` (with no subcommand). See [Setup commands](./setup-commands.md) and the [Dev environments](./dev-environments.md) reference for the `env-lab` and `worktree` tooling.
...
> **Note:** `run` is overloaded on purpose. `geetorusai run` with **no subcommand** bootstraps and starts a local instance (a setup action). `geetorusai run <subcommand>` (`list`, `live`, `get`, `events`, `log`, `cancel`, and friends) inspects and controls heartbeat **runs** through the API (a control-plane action). Two different concepts under one verb — see [run reference](./run.md).

---

## What runs where

This is the single most important thing to internalize before you script against the CLI: **the CLI does not run the model.** An agent's actual work — the LLM calls, the coding, the research — executes server-side inside the Geetorus runtime and its adapters. The CLI triggers that work and observes it.
...
Every control-plane call is made *as somebody*. Geetorus has exactly two personas, and your CLI profile records which one you are.
...
- **`geetorusai connect`** — the interactive wizard (TTY only). It resolves the API base, health-checks it, logs in as board, lets you pick persona/company/agent, mints a token, saves a persona-aware profile, and prints the shell exports to run. Start here when you have a terminal in front of you.
- **`geetorusai auth login`** — the device-code / browser-approval flow that stores a board credential keyed by API base. Good for non-interactive board login.
- **`geetorusai agent local-cli <agent-id>`** — the all-in-one for handing the CLI to an AI: it creates a long-lived agent API key, installs Geetorus skills into `~/.codex/skills` and `~/.claude/skills`, and prints export lines for `GEETORUS_API_URL`, `GEETORUS_COMPANY_ID`, `GEETORUS_AGENT_ID`, and `GEETORUS_API_KEY`.

```sh
geetorusai connect
# or, for an AI operator on this machine:
geetorusai agent local-cli <agent-id> -C <company-id>
```
...
4. the local Geetorus config server port
...
So you aren't retyping the same flags forever, the CLI stores defaults in `~/.geetorus/context.json`. Profiles are persona-aware: a board profile records `persona=board`; an agent profile records `persona=agent` plus its `agentId` and `agentName`.
...
```sh
geetorusai context set --api-base http://localhost:3100 --company-id <company-id>
geetorusai context set --api-key-env-var-name GEETORUS_API_KEY
export GEETORUS_API_KEY=...
geetorusai context show
geetorusai context use default
```
...
| Flag | Use |
|---|---|
| `--data-dir <path>` | Isolate all local state (config, context, db, logs, storage, secrets) away from `~/.geetorus`. Ideal for test instances and worktrees. |
