...
```sh
geetorusai agent me
geetorusai agent inbox
geetorusai agent inbox-mine --user-id <board-user-id> --status todo,in_progress
```
...
```sh
geetorusai agent list --company-id <company-id>
geetorusai agent get <agent-id>
```
...
```sh
geetorusai agent create --company-id <company-id> \
  --payload-json '{"name":"Builder","adapterType":"codex_local"}'

geetorusai agent hire --company-id <company-id> \
...
> **Tip:** Building the payload by hand is error-prone. The `geetorus-create-agent` skill inspects adapter options and drafts a valid config for you. The board can review and resolve a hire request with the [approval](./approval.md) commands.
...
```sh
geetorusai agent update <agent-id> --payload-json '{"title":"Senior Builder"}'
geetorusai agent delete <agent-id> --yes
```
...
```sh
geetorusai agent pause <agent-id>
geetorusai agent resume <agent-id>
geetorusai agent approve <agent-id>
geetorusai agent terminate <agent-id>
```
...
```sh
geetorusai agent wake <agent-id-or-shortname> \
...
```sh
geetorusai agent heartbeat:invoke <agent-id>
geetorusai agent claude-login <agent-id>
```
...
```sh
geetorusai agent permissions:update <agent-id> \
...
```sh
geetorusai agent configuration <agent-id>
geetorusai agent config-revisions <agent-id>
geetorusai agent config-revision:get <agent-id> <revision-id>
geetorusai agent config-revision:rollback <agent-id> <revision-id>
```
...
```sh
geetorusai agent runtime-state <agent-id>
geetorusai agent runtime-state:reset-session <agent-id> --task-key <key>
geetorusai agent task-sessions <agent-id>
```
...
```sh
geetorusai agent skills <agent-id>
geetorusai agent skills:sync <agent-id> --desired-skills geetorus,github
```

| Command | Use |
|---|---|
| `agent skills` | List the agent's current skills. |
| `agent skills:sync` | Replace the agent's desired skills with the comma-separated `--desired-skills` list (`POST /api/agents/{id}/skills/sync`). Required Geetorus skills stay server-enforced regardless. |
...
```sh
geetorusai agent instructions-path:update <agent-id> \
  --payload-json '{"path":"/tmp/AGENTS.md","adapterConfigKey":"instructionsFilePath"}'

geetorusai agent instructions-bundle <agent-id>
geetorusai agent instructions-bundle:update <agent-id> --payload-json '{"mode":"managed"}'

geetorusai agent instructions-file:get <agent-id> --path AGENTS.md
geetorusai agent instructions-file:put <agent-id> --path AGENTS.md --content-file ./AGENTS.md
geetorusai agent instructions-file:delete <agent-id> --path AGENTS.md
```
...
This is the practical bridge for running Claude or Codex **locally** as a Geetorus agent. It accepts an agent ID or shortname/url-key, requires `-C, --company-id <id>`, and in one shot:

1. Mints a long-lived agent API key (label it with `--key-name <name>`; defaults to a timestamped `local-cli` key).
2. Installs the bundled Geetorus skills into `~/.codex/skills`, `~/.claude/skills`, and `~/.kimi-code/skills` as symlinks (respecting `CODEX_HOME` / `CLAUDE_HOME` / `KIMI_CODE_HOME` if set).
3. Prints the shell `export` lines for `GEETORUS_API_URL`, `GEETORUS_COMPANY_ID`, `GEETORUS_AGENT_ID`, and `GEETORUS_API_KEY`.

```sh
geetorusai agent local-cli <agent-id-or-shortname> --company-id <company-id>
geetorusai agent local-cli codexcoder --company-id <company-id> --key-name workstation
geetorusai agent local-cli claudecoder --company-id <company-id> --no-install-skills
```
...
Source the printed exports into your shell, then launch Codex or Claude — they now act as that agent against your Geetorus API:

```sh
eval "$(geetorusai agent local-cli codexcoder --company-id <company-id> --json | jq -r .exports)"
```
