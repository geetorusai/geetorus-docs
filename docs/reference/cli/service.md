...
seo_description: Move Geetorus off a terminal window that dies when you close it, and run it as a managed background service that survives logout and reboot.
---

# Service Commands

Most people start out running Geetorus with `geetorusai run` in a terminal window, which works right up until you close the window. The `service` commands hand that job to your operating system instead: Geetorus starts when you log in, restarts itself if it crashes, keeps a log you can tail, and survives you quitting the terminal entirely. Reach for these commands when you want Geetorus to be *always there* rather than something you remember to start.
...
> **Note:** Every `service` subcommand accepts `-i, --instance <id>` to target a specific local instance (default `default`), and `--json` to print machine-readable output instead of text. If service management is not available — Windows, or a Linux container or WSL1 without a usable systemd user manager — each command says so and exits cleanly, and you should use [`geetorusai run`](./setup-commands.md#geetorusai-run-local-bootstrap) instead.

---

## What gets installed

The service definition is a small file in your own home directory that tells the OS to run `geetorusai run --instance <id>`. It is generated from your current install, so it always points at the CLI you actually have.

| | macOS | Linux |
|---|---|---|
| Supervisor | launchd | systemd (user) |
| Service name | `ing.geetorus.geetorusai` | `geetorusai.service` |
| Definition file | `~/Library/LaunchAgents/ing.geetorus.geetorusai.plist` | `~/.config/systemd/user/geetorusai.service` |
| Logs | `~/.geetorus/instances/<id>/logs/service.log` and `service.err.log` | the journal, via `journalctl --user` |

Non-default instances get their own service so they never collide: instance `dev` becomes `ing.geetorus.geetorusai.dev` on macOS and `geetorusai-dev.service` on Linux.

The service is started with `GEETORUS_SERVICE_MANAGED=1`, `GEETORUS_INSTANCE_ID`, and `GEETORUS_HOME` set, so the server knows it is running under a supervisor and which instance and home directory to use.

---

## Install the service

`service install` writes the definition, registers it with the supervisor, and starts Geetorus.

```sh
geetorusai service install
geetorusai service install --no-start-now
geetorusai service install --no-start-on-login
geetorusai service install --enable-linger
```
...
You can also get all of this during first-run setup: [`geetorusai onboard`](./setup-commands.md#geetorusai-onboard) offers to install the service for you, and `geetorusai onboard --install-service` skips the question.
...
Those can disagree, and the disagreement is the useful part: a service that is `active` but not healthy is a server that started and then failed, and a healthy instance while the service is inactive usually means a foreground `geetorusai run` is holding the port.
...
geetorusai service restart --expected-version 2026.609.0
```
...
`geetorusai update` performs this same hot restart for you when it detects an active service, so you rarely need to run it by hand after an update. See [Update Geetorus](../../how-to/update-geetorus.md).
...
```sh
geetorusai service logs
geetorusai service logs --follow
geetorusai service logs --lines 500
```
...
> **Tip:** `geetorusai service logs --follow` in one window while you trigger work in another is the quickest way to watch a startup problem happen in real time.
...
This removes only the service. Your Geetorus install, config, and data are untouched, and `geetorusai run` still works exactly as before. To remove the CLI itself, see [`geetorusai uninstall`](./installation.md#removing-the-managed-install) — which stops and removes the service for you as part of its own cleanup.

---

## How doctor sees the service

[`geetorusai doctor`](./setup-commands.md#geetorusai-doctor) checks the service as part of its normal run, so you usually find out something is wrong before you go looking. It verifies that the definition on disk matches what the current install would generate, that the service is active, that the health endpoint reports OK, and that the running version matches your managed install. On Linux it also warns when start-on-login is enabled but lingering is off — the combination that quietly stops Geetorus when you log out.

If no service is installed, doctor passes and says so: running Geetorus in the foreground is a perfectly valid choice.
...
- [Update Geetorus](../../how-to/update-geetorus.md) — updating an instance that runs as a service.
