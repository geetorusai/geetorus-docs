...
| `secrets create` | Store a new Geetorus-managed secret value. |
| `secrets link` | Register an external provider-owned secret without copying its value into Geetorus. |
...
```sh
geetorusai secrets list --company-id <company-id>
geetorusai secrets list --company-id <company-id> --json
```
...
```sh
geetorusai secrets declarations --company-id <company-id>
geetorusai secrets declarations --company-id <company-id> --include agents,projects --kind secret
```
...
### Create a Geetorus-managed secret

`secrets create` stores a value that Geetorus encrypts and manages itself. You must supply the value through exactly one of `--value` or `--value-env`. Always prefer `--value-env`: it reads the value from a named environment variable so the plaintext never appears in your shell history, your scrollback, or the process argument list.

```sh
# Preferred: the value is read from the env var, not typed on the command line
export ANTHROPIC_API_KEY=sk-ant-...
geetorusai secrets create --company-id <company-id> --name anthropic-api-key --value-env ANTHROPIC_API_KEY
```

```sh
# Discouraged: --value puts the secret into shell history
geetorusai secrets create --company-id <company-id> --name anthropic-api-key --value 'sk-ant-...'
```
...
`secrets link` registers a secret that lives in an external vault (for example AWS Secrets Manager) by reference. Geetorus records the pointer — the provider, the external ref, and an optional version ref — but never copies the value into its own store. This is the right choice when your organization's source of truth is a managed vault and you want Geetorus to resolve the value at runtime rather than hold a second copy.

```sh
geetorusai secrets link \
...
```sh
geetorusai secrets update <secret-id> --payload-json '{"description":"Rotated quarterly by ops"}'
```
...
`secrets rotate` replaces the value of a Geetorus-managed secret and bumps its version. As with `create`, supply the new value through one of `--value` or `--value-env`, and prefer `--value-env`.

```sh
export ANTHROPIC_API_KEY=sk-ant-new...
geetorusai secrets rotate <secret-id> --value-env ANTHROPIC_API_KEY
```
...
```sh
geetorusai secrets delete <secret-id> --yes --confirm <secret-id>
```
...
`secrets doctor` runs provider health checks through the Geetorus API and prints one row per provider with a `status` of `ok`, `warn`, or `error`, plus any warnings, missing-config hints, detected credential sources, and backup guidance.

```sh
geetorusai secrets doctor --company-id <company-id>
```

For AWS-backed secrets, `doctor` reports missing non-secret provider env and the expected AWS SDK runtime credential source. Do not store AWS bootstrap credentials *inside* Geetorus secrets — let the runtime resolve them from the standard AWS credential chain, and use `doctor` to confirm that chain is wired up.
...
```sh
geetorusai secrets providers --company-id <company-id>
```
...
geetorusai secrets provider-configs --company-id <company-id>

# Create a config
geetorusai secrets provider-config:create --company-id <company-id> --payload-json '{ ... }'

# Preview which secrets a vault would discover, without importing them
geetorusai secrets provider-config:discovery-preview --company-id <company-id> --payload-json '{ ... }'

# Inspect, update, default, health-check, or delete a single config
geetorusai secrets provider-config:get <config-id>
geetorusai secrets provider-config:update <config-id> --payload-json '{ ... }'
geetorusai secrets provider-config:default <config-id>
geetorusai secrets provider-config:health <config-id>
geetorusai secrets provider-config:delete <config-id>
```
...
When a provider vault is configured, you can pull selected secrets into Geetorus's registry. Always preview first: `remote-import:preview` shows what *would* be imported, and `remote-import` performs the selected import. Both take a `--payload-json` describing the source and selection.

```sh
geetorusai secrets remote-import:preview --company-id <company-id> --payload-json '{ ... }'
geetorusai secrets remote-import --company-id <company-id> --payload-json '{ ... }'
```
...
geetorusai secrets migrate-inline-env --company-id <company-id>

# Commit the migration
geetorusai secrets migrate-inline-env --company-id <company-id> --apply
```
