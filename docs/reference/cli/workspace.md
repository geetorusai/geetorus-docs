...
All of these commands are read-and-control wrappers over the Geetorus API. They TRIGGER and OBSERVE server-side state; the actual execution (runtime services, runtime commands, environment probes) runs on the Geetorus runtime, not in the CLI.
...
```sh
geetorusai org get --company-id <company-id>
geetorusai org svg --company-id <company-id> --out org.svg
geetorusai org png --company-id <company-id> --out org.png
```
...
- Without `--out`, the raw image bytes stream to stdout — pipe them to a file yourself (`geetorusai org png -C <company-id> > org.png`) or into another tool.
...
```sh
geetorusai agent-config list --company-id <company-id>
```
...
```sh
geetorusai workspace list --company-id <company-id>
geetorusai workspace get <execution-workspace-id>
geetorusai workspace close-readiness <execution-workspace-id>
geetorusai workspace operations <execution-workspace-id>
geetorusai workspace update <execution-workspace-id> --payload-json '{...}'
geetorusai workspace runtime-service <execution-workspace-id> start --payload-json '{...}'
geetorusai workspace runtime-command <execution-workspace-id> run --payload-json '{...}'
```
...
```sh
geetorusai workspace runtime-service <execution-workspace-id> restart --payload-json '{"service":"dev-server"}'
geetorusai workspace runtime-command <execution-workspace-id> run --payload-json '{"command":"npm test"}'
```
...
```sh
geetorusai environment list --company-id <company-id>
geetorusai environment capabilities --company-id <company-id>
geetorusai environment create --company-id <company-id> --payload-json '{...}'
geetorusai environment get <environment-id>
geetorusai environment leases <environment-id>
geetorusai environment lease <lease-id>
geetorusai environment update <environment-id> --payload-json '{...}'
geetorusai environment delete <environment-id>
geetorusai environment probe <environment-id>
geetorusai environment probe-config --company-id <company-id> --payload-json '{...}'
```
...
geetorusai environment probe-config --company-id <company-id> --payload-json '{"kind":"ssh","host":"build-01.internal"}'
geetorusai environment create --company-id <company-id> --payload-json '{"name":"build-01","kind":"ssh","host":"build-01.internal"}'
geetorusai environment probe <environment-id>
```
...
```sh
geetorusai project-workspace list <project-id>
geetorusai project-workspace create <project-id> --payload-json '{...}'
geetorusai project-workspace update <project-id> <workspace-id> --payload-json '{...}'
geetorusai project-workspace delete <project-id> <workspace-id>
geetorusai project-workspace runtime-service <project-id> <workspace-id> restart --payload-json '{...}'
geetorusai project-workspace runtime-command <project-id> <workspace-id> run --payload-json '{...}'
```
...
geetorusai project-workspace list <project-id>
geetorusai project-workspace delete <project-id> <workspace-id>
geetorusai project-workspace runtime-command <project-id> <workspace-id> run --payload-json '{"command":"pnpm build"}'
```
