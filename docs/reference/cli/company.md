...
Companies are the top-level container in Geetorus: every agent, project, goal, issue, and budget lives inside one. Use `geetorusai company` when you want to inspect companies, edit their settings and branding, move a whole company between instances as a portable package, pull feedback traces, or delete a company when the server policy allows it.
...
geetorusai company get <company-id>
geetorusai company current
geetorusai company stats
```
...
| `company current` | The full record for the company in scope, resolved from `-C, --company-id`, the context file, `GEETORUS_COMPANY_ID`, or the agent credential — no positional ID needed. |
...
```sh
geetorusai company create \
  --payload-json '{"name":"Acme","issuePrefix":"ACME"}'

geetorusai company update <company-id> \
  --payload-json '{"budgetMonthlyCents":500000}'

geetorusai company branding:update <company-id> \
...
```sh
geetorusai company archive <company-id>
```
...
Geetorus companies are portable. `company export` writes a company to a folder of markdown-backed files (plus a `.geetorus.yaml` manifest and any binary assets), and `company import` reads that package back — from a local path, a `.zip`, or a GitHub repository — into a new or existing company.
...
```sh
geetorusai company export <company-id> \
  --out ./exports/acme \
...
On success the command prints the resolved output path, the package root, the number of files written, the `.geetorus.yaml` extension path, and a warning count.

### Import a package

```sh
geetorusai company import ./exports/acme \
  --target new \
  --new-company-name "Acme (imported)"

geetorusai company import https://github.com/acme/company-pack \
...
| `--geetorus-url <url>` | Alias for `--api-base` on this command. |
...
```sh
geetorusai company export:preview <company-id> --payload-json '{...}'
geetorusai company export:api <company-id> --payload-json '{...}'
geetorusai company import:preview <company-id> --payload-json '{...}'
geetorusai company import:apply <company-id> --payload-json '{...}'
```
...
```sh
geetorusai company feedback:list --company-id <company-id> --status open --vote down
geetorusai company feedback:export --company-id <company-id> --from 2026-01-01 --format ndjson --out ./feedback.ndjson
```
...
```sh
geetorusai company delete PAP --yes --confirm PAP
geetorusai company delete 5cbe79ee-acb3-4597-896e-7662742593cd \
...
1. **Server policy.** The instance must have `GEETORUS_ENABLE_COMPANY_DELETION` enabled. If it is off, the API rejects the delete regardless of your flags.
