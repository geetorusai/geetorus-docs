...
Routines are Geetorus's recurring and event-driven work. A routine carries a payload describing the work to spawn, a status, a revision history, and one or more triggers — schedules, webhooks, or public API endpoints — that fire it. Reach for these commands when you want your AI to set up automation that runs on its own (a nightly summary, a webhook that turns inbound events into tasks), inspect what a routine has done, or pause everything in an emergency. The singular `routine` group talks to the control-plane REST API; the plural `routines` group is a local database-maintenance escape hatch.

> **Note:** Like every other work object in Geetorus, the actual work a routine produces runs server-side. These commands create, configure, fire, and observe routines through the API — they do not execute the work themselves. See [What Runs Where](../../guides/welcome/key-concepts.md).
...
The `routine` commands take the standard client flags (`--api-base`, `--api-key`, `--context`, `--profile`, `--json`, and `--data-dir`). The single `routines disable-all` command does not go through the API at all — it opens the local Geetorus database and flips routine statuses in place.
...
```sh
geetorusai routine list --company-id <company-id>
geetorusai routine list --company-id <company-id> --project-id <project-id>
```
...
```sh
geetorusai routine get <routine-id>
```
...
```sh
geetorusai routine create --company-id <company-id> --payload-json '{
...
```sh
geetorusai routine update <routine-id> --payload-json '{"status": "paused"}'
```
...
```sh
geetorusai routine revisions <routine-id>
```
...
```sh
geetorusai routine revision:restore <routine-id> <revision-id>
```
...
```sh
geetorusai routine runs <routine-id>
geetorusai routine runs <routine-id> --limit 50
```
...
```sh
geetorusai routine run <routine-id>
geetorusai routine run <routine-id> --payload-json '{"reason": "manual smoke test"}'
```
...
| `webhook` | An external system POSTs to the trigger's webhook URL. | Carries a secret you can rotate. Use it to turn inbound events into Geetorus work. |
...
```sh
geetorusai routine trigger:create <routine-id> --payload-json '{
  "kind": "schedule",
  "schedule": "0 7 * * *"
}'
```
...
```sh
geetorusai routine trigger:update <trigger-id> --payload-json '{"schedule": "0 */2 * * *"}'
```
...
```sh
geetorusai routine trigger:delete <trigger-id>
```
...
```sh
geetorusai routine trigger:rotate-secret <trigger-id>
```
...
```sh
geetorusai routine trigger:fire <public-id>
geetorusai routine trigger:fire <public-id> --payload-json '{"event": "deploy.finished"}'
```
...
This is the local maintenance command — the "stop everything" switch. It pauses every non-archived routine in one company by writing directly to the configured local Geetorus database, without going through the API. Reach for it when a local instance is misbehaving and you want all scheduled and webhook-driven work to stop firing immediately.
...
| Flag | Use |
|---|---|
| `-C, --company-id <id>` | Company whose routines to pause. Required (falls back to `GEETORUS_COMPANY_ID`). |
| `-c, --config <path>` | Path to the Geetorus config file, if not the default. |
| `-d, --data-dir <path>` | Geetorus data directory root, to isolate state from `~/.geetorus`. |
