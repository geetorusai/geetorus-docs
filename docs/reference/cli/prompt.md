...
The wake is a `POST /api/agents/<agent-id>/wakeup` with `source: "on_demand"`. It does not run the model inline. It tells the Geetorus runtime "there is work for this agent," and the server schedules the run. The prompt command returns once the issue/comment is created and the wakeup is acknowledged.
...
```sh
geetorusai agent-prompt <agent-id> <agent-api-key> "Draft the Q3 launch checklist and link the source issues"
```
...
geetorusai agent prompt "Investigate the failing nightly build and report findings"

# Read the key from a named environment variable
geetorusai agent prompt --api-key-env GEETORUS_API_KEY "Investigate the failing nightly build"

# Target a specific saved profile
geetorusai agent prompt --profile my-agent "Investigate the failing nightly build"
```
...
```sh
geetorusai board prompt --company-id <company-id> --agent <agent-name-or-id> \
...
geetorusai board prompt --company-id <company-id> --agent <agent-id> --issue PAP-142 \
...
geetorusai board prompt --company-id <company-id> --agent <agent-id> --no-wake \
