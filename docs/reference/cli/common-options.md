...
| Flag | Use |
|---|---|
| `-d, --data-dir <path>` | Geetorus data directory root. Isolates all local state (config, context, db, logs, storage, secrets) away from `~/.geetorus`. Indispensable for clean test instances and per-worktree setups. |
| `--api-base <url>` | Base URL for the Geetorus API. Highest-priority override of where the CLI connects. |
...
| `--context <path>` | Path to the CLI context file to read and write. Overrides the default `~/.geetorus/context.json` lookup. |
| `--profile <name>` | Which context profile to use. Defaults to the context's current profile. |
| `-c, --config <path>` | Path to the Geetorus config file. Used only to infer a local server port when no other API base is given. |
...
2. The `GEETORUS_API_URL` environment variable
3. The selected context profile's `apiBase`
4. A locally inferred base from your Geetorus config — `http://<host>:<port>`, where the host comes from `GEETORUS_SERVER_HOST` (default `localhost`), and the port comes from `GEETORUS_SERVER_PORT`, then the config file's `server.port`
...
| 2 | `GEETORUS_API_KEY` environment variable | `env` |
...
> **Warning:** Interactive auth recovery only triggers on a TTY. In headless contexts there is no prompt — supply a working credential up front via `--api-key`, `GEETORUS_API_KEY`, or a profile `apiKeyEnvVarName`, or the command fails with the underlying auth error.
...
2. The `GEETORUS_COMPANY_ID` environment variable
...
Profiles live in the CLI context file — by default `~/.geetorus/context.json`, a version-2 store with a `currentProfile` and a map of named `profiles`. The context path itself is resolved from `--context`, then the `GEETORUS_CONTEXT` env var, then the nearest `.geetorus/context.json` found by walking up from the current directory, and finally the default home location. The file is written with `0600` permissions.
...
geetorusai context set --profile my-agent \
  --persona agent \
  --agent-id <agent-id> \
  --agent-name "Ops Agent" \
  --api-key-env-var-name GEETORUS_API_KEY \
  --use
```
...
> **Tip:** Keep the secret out of the file. Set `--api-key-env-var-name GEETORUS_API_KEY` on the profile and export the actual token in your shell:
>
> ```sh
...
`--data-dir` redirects every piece of local Geetorus state — config, context, database, logs, storage, and secrets — to a directory you choose. Pass it on whichever commands should share that isolated root:

```sh
geetorusai run --data-dir ./tmp/geetorus-dev
geetorusai issue list --data-dir ./tmp/geetorus-dev
```

This keeps experiments, test instances, and parallel worktrees from colliding with your real `~/.geetorus` state.
...
- [Installation](installation.md) — installing the `geetorusai` binary
