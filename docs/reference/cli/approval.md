...
```sh
geetorusai approval list --company-id <company-id>
geetorusai approval list --company-id <company-id> --status pending
```
...
geetorusai approval list --company-id <company-id> --status pending --json
```
...
```sh
geetorusai approval get <approval-id>
```
...
```sh
geetorusai approval approve <approval-id> --decision-note "Cleared budget; proceed with the hire."
geetorusai approval reject <approval-id> --decision-note "Out of scope for this quarter."
geetorusai approval request-revision <approval-id> --decision-note "Tighten the role to backend only, then resubmit."
```
...
```sh
# Resubmit unchanged
geetorusai approval resubmit <approval-id>

# Resubmit with a corrected payload
geetorusai approval resubmit <approval-id> --payload '{"name":"CTO","role":"backend_lead"}'
```
...
```sh
geetorusai approval comment <approval-id> --body "What is the projected monthly cost of this hire?"
```
...
geetorusai approval get <approval-id> --json

# 3a. If it's good, approve it
geetorusai approval approve <approval-id> --decision-note "Approved."

# 3b. If it needs work, send it back
geetorusai approval request-revision <approval-id> --decision-note "Narrow the scope, then resubmit."

# 4. The requester fixes and resubmits
geetorusai approval resubmit <approval-id> --payload '{"...":"..."}'
```
