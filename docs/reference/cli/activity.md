...
```sh
geetorusai activity list --company-id <company-id>
geetorusai activity list --company-id <company-id> --agent-id <agent-id>
geetorusai activity list --company-id <company-id> --entity-type issue --entity-id <issue-id>
```
...
```sh
geetorusai activity create --company-id <company-id> \
...
> **Note:** You almost never need this command for normal operations. Geetorus writes activity entries on its own whenever issues, documents, and recovery actions change. Reserve `activity create` for stitching an external event — a deploy, an incident, a manual milestone — into the same audit trail.
...
```sh
geetorusai activity issue <issue-id>
geetorusai activity issue PAP-39 --json
```
