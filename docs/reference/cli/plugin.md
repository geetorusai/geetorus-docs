...
Plugins extend the Geetorus runtime server-side: they add connectors, workspace integrations, automation jobs, dashboards, and UI surfaces that agents and the board can use. These commands are how you scaffold a plugin, install it into your instance, watch its health, toggle it on and off, and drive its runtime surfaces from the terminal. Reach for them when you are authoring a plugin against the alpha SDK, or operating one that is already installed.

Like every plugin endpoint, the work runs in the Geetorus runtime. The CLI registers the plugin, flips its lifecycle state, and proxies calls to its tools, jobs, config, and bridge channels — it does not execute plugin code on your machine, with one exception: a **local-path install** runs trusted local code from your own disk, which is why those installs print an explicit warning.
...
```sh
geetorusai plugin init @acme/plugin-linear
geetorusai plugin init @acme/plugin-linear --template connector --category connector \
...
| `--sdk-path <path>` | Path to a local `@geetorusai/plugin-sdk` package, for SDK development against an unpublished build. |
...
pnpm install
pnpm dev
geetorusai plugin install plugin-linear
```

Keep `pnpm dev` running while you develop — Geetorus watches the rebuilt `dist` output and reloads the plugin worker.
...
```sh
geetorusai plugin install ./my-plugin                 # local path (auto-detected)
geetorusai plugin install @acme/plugin-linear         # npm package
geetorusai plugin install @acme/plugin-linear --version 1.2.0
geetorusai plugin install /abs/path/to/plugin --local # force local-path treatment
```
...
> **Warning:** Local-path installs run trusted local code from your machine. Only install paths you control. After a local install the CLI reminds you to keep `pnpm dev` running so Geetorus can reload the worker on rebuild.
...
geetorusai plugin list --status error
geetorusai plugin list --json
```
...
```sh
geetorusai plugin inspect @acme/plugin-linear
geetorusai plugin inspect <plugin-id> --json
```
...
```sh
geetorusai plugin disable @acme/plugin-linear
geetorusai plugin enable @acme/plugin-linear
```
...
```sh
geetorusai plugin uninstall @acme/plugin-linear
geetorusai plugin uninstall @acme/plugin-linear --force
```
...
geetorusai plugin examples --json
```

Copy the printed `geetorusai plugin install <local-path>` line to install one straight from the bundle.
...
```sh
geetorusai plugin ui-contributions
geetorusai plugin tools
geetorusai plugin tool:execute --payload-json '{"tool":"...","input":{}}'
```
...
```sh
geetorusai plugin health <plugin-id>
geetorusai plugin logs <plugin-id>
geetorusai plugin upgrade <plugin-id> --payload-json '{}'
```
...
```sh
geetorusai plugin config <plugin-id>
geetorusai plugin config:set <plugin-id> --payload-json '{"configJson":{"apiKey":"..."}}'
geetorusai plugin config:test <plugin-id> --payload-json '{"configJson":{"apiKey":"..."}}'
```
...
```sh
geetorusai plugin jobs <plugin-id>
geetorusai plugin job:runs <plugin-id> <job-id>
geetorusai plugin job:trigger <plugin-id> <job-id> --payload-json '{}'
```
...
```sh
geetorusai plugin webhook <plugin-id> <endpoint-key> --payload-json '{}'
geetorusai plugin dashboard <plugin-id>
geetorusai plugin data <plugin-id> <key> --payload-json '{}'
geetorusai plugin action <plugin-id> <key> --payload-json '{}'
```
...
```sh
geetorusai plugin bridge:data <plugin-id> --payload-json '{}'
geetorusai plugin bridge:action <plugin-id> --payload-json '{}'
geetorusai plugin bridge:stream <plugin-id> <channel>
geetorusai plugin bridge:stream <plugin-id> <channel> --duration-ms 10000
```
...
```sh
geetorusai plugin local-folders <plugin-id> -C <company-id>
geetorusai plugin local-folder:status <plugin-id> <folder-key> -C <company-id>
geetorusai plugin local-folder:validate <plugin-id> <folder-key> -C <company-id> --payload-json '{}'
geetorusai plugin local-folder:set <plugin-id> <folder-key> -C <company-id> --payload-json '{"path":"/abs/path"}'
```
