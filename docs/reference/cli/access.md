...
Most of these commands are thin wrappers over Geetorus API endpoints. They accept the standard client flags documented in [common options](./common-options.md) — `--api-base`, `--api-key`, `--context`, `--profile`, `--data-dir`, and `--json`. Company-scoped commands additionally take `-C, --company-id <id>`; if you omit it, the CLI falls back to the company recorded in your selected profile.
...
```sh
geetorusai profile session
geetorusai profile get
geetorusai profile update --payload-json '{"displayName":"Ada Lovelace"}'
geetorusai profile company-user <user-slug> --company-id <company-id>
```
...
```sh
geetorusai invite list --company-id <company-id>
geetorusai invite create --company-id <company-id> --payload-json '{"role":"member"}'
geetorusai invite revoke <invite-id>
geetorusai invite show <token>
geetorusai invite onboarding:text <token>
geetorusai invite accept <token> --payload-json '{}'
```
...
```sh
geetorusai join list --company-id <company-id> --status pending
geetorusai join approve <request-id> --company-id <company-id>
geetorusai join reject <request-id> --company-id <company-id>
geetorusai join claim-key <request-id> --claim-secret <secret>
```
...
```sh
geetorusai member list --company-id <company-id>
geetorusai member user-directory --company-id <company-id>
geetorusai member update <member-id> --company-id <company-id> --payload-json '{...}'
geetorusai member role-and-grants <member-id> --company-id <company-id> --payload-json '{...}'
geetorusai member permissions <member-id> --company-id <company-id> --payload-json '{...}'
geetorusai member archive <member-id> --company-id <company-id>
```
...
```sh
geetorusai admin user list --query ada
geetorusai admin user promote <user-id>
geetorusai admin user demote <user-id>
geetorusai admin user company-access <user-id>
geetorusai admin user company-access:update <user-id> --payload-json '{...}'
```
...
```sh
geetorusai instance scheduler-heartbeats
geetorusai instance settings:general
geetorusai instance settings:general:update --payload-json '{...}'
geetorusai instance settings:experimental
geetorusai instance settings:experimental:update --payload-json '{...}'
geetorusai instance database-backup
```
...
```sh
geetorusai sidebar preferences
geetorusai sidebar preferences:update --payload-json '{...}'
geetorusai sidebar project-preferences --company-id <company-id>
geetorusai sidebar project-preferences:update --company-id <company-id> --payload-json '{...}'
geetorusai sidebar badges --company-id <company-id>

geetorusai inbox dismissals --company-id <company-id>
geetorusai inbox dismiss --company-id <company-id> --payload-json '{"itemKey":"run:<run-id>"}'
geetorusai inbox dismiss --company-id <company-id> --payload-json '{"itemKey":"attention:<attention-key>","kind":"snooze","snoozedUntil":"2026-07-13T09:00:00.000Z"}'
```
...
```sh
geetorusai board-claim show <token>
geetorusai board-claim claim <token> --payload-json '{}'
geetorusai openclaw invite-prompt --company-id <company-id> --payload-json '{...}'
```
...
```sh
geetorusai available-skill list
geetorusai available-skill index
geetorusai available-skill get <skill-name>

geetorusai llm agent-configuration
geetorusai llm agent-configuration:adapter <adapter-type>
geetorusai llm agent-icons
```
