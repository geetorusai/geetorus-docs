...
seo_description: Install a ready-made starter group — agents, projects, tasks, and the skills they need — from the catalog Geetorus ships with the app.
---

# Teams Commands

A *team* in the catalog is a ready-made starter group — a small set of agents, projects, tasks, and the skills they need — that Geetorus ships with the app. Use `geetorusai teams` when you want to browse those team templates, inspect what one would create, preview the import against a real company, and then install it. Think of it as the team-shaped equivalent of the skills catalog: instead of one reusable procedure, you get a whole working unit you can drop into a company in one step.
...
```sh
geetorusai teams browse --kind bundled --category company-defaults
geetorusai teams search "engineering" --kind bundled
geetorusai teams inspect core-exec-team
```
...
```sh
geetorusai teams list --company-id <company-id>
geetorusai teams list --kind optional --company-id <company-id>
```
...
```sh
geetorusai teams preview core-exec-team --company-id <company-id>
geetorusai teams preview product-engineering \
...
```sh
geetorusai teams install core-exec-team --company-id <company-id>

geetorusai teams install product-engineering \
  --target-manager-slug cto \
...
| `--approval-issue-id <id>` | Issue ID to link to the fallback approval request. Defaults to `GEETORUS_TASK_ID` when that is set. |
...
Installing a team creates agents, which requires the `agents:create` permission. If the server denies the install for that reason, you would normally see a `403`. Pass `--request-approval-on-forbidden` (or run inside a Geetorus task, where `GEETORUS_TASK_ID` is set) and the CLI instead files a board approval request describing the install you attempted, so a board operator can approve it. Any secret values you passed are stripped from the stored request and redacted in the returned result.
