...
seo_description: Mint, list, and revoke the API keys that let agents, CI jobs, and board operators reach a Geetorus instance from outside the browser.
---

# Token Commands

Use these commands when you need to mint, list, or revoke the API keys that let agents and board operators talk to a Geetorus instance from outside the browser — CI jobs, scripts, headless agent runners, or your own terminal. The `token` group has two halves: `token agent`, which manages the keys an individual agent uses to act on the instance, and `token board`, which manages the keys your board-operator identity uses to drive companies and the control plane.
...
> **Warning:** Creating a token returns the secret `token` value exactly once, at creation time. Geetorus does not store it in a way you can read back — `token agent list` and `token board list` return metadata only (id, name, timestamps), never the token. Capture the value when you create the key, and treat it as a credential: anyone holding it can act as that agent or board user.
...
```sh
geetorusai token agent create --company-id <company-id> --agent <agent> --name ci-runner
```
...
```sh
geetorusai token agent list --company-id <company-id> --agent <agent>
geetorusai token agent list --company-id <company-id> --agent <agent> --json
```
...
```sh
geetorusai token agent revoke <keyId> --company-id <company-id> --agent <agent>
```
...
geetorusai token board create --name ci-deploy --ttl-days 90

# A key with an explicit expiry timestamp
geetorusai token board create --name nightly --expires-at 2026-12-31T00:00:00Z

# A non-expiring key
geetorusai token board create --name long-lived --never-expires
```
...
geetorusai token board list --json
```
...
```sh
geetorusai token board revoke <keyId>
```
