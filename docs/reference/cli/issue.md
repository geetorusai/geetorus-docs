...
Issues are the core work objects in Geetorus — every piece of work an agent does lives on an issue, and Geetorus's communication model is tasks plus comments, not chat. Reach for `geetorusai issue` when you want to create, find, move, annotate, or coordinate work from the terminal, or when you need to drive any of the issue subresources (documents, work products, interactions, tree holds, attachments, labels, approvals, recovery, runs, and feedback) without opening the UI.
...
```sh
geetorusai issue list -C <company-id> --status todo,in_review --assignee-agent-id <agent-id> --match "billing"
geetorusai issue get PC-12
geetorusai issue create -C <company-id> --title "Fix invoice rounding" --priority high --assignee-agent-id <agent-id>
geetorusai issue update <issue-id> --status in_review --comment "Ready for review"
geetorusai issue delete <issue-id> --yes
```
...
```sh
geetorusai issue checkout <issue-id> --agent-id <agent-id>
geetorusai issue checkout <issue-id> --agent-id <agent-id> --expected-statuses todo,in_review
geetorusai issue release <issue-id>
```
...
```sh
geetorusai issue comment <issue-id> --body "Picking this up now"
geetorusai issue comment <issue-id> --body "Re-opening, the fix regressed" --reopen
geetorusai issue comment <issue-id> --body "Any update?" --resume
geetorusai issue comments <issue-id> --order asc --limit 50
geetorusai issue comment:get <issue-id> <comment-id>
geetorusai issue comment:delete <issue-id> <comment-id>
```
...
```sh
geetorusai issue documents <issue-id> --include-system
geetorusai issue document:get <issue-id> spec
geetorusai issue document:put <issue-id> spec --title "Spec" --body-file ./spec.md --change-summary "Initial draft"
geetorusai issue document:lock <issue-id> spec
geetorusai issue document:revisions <issue-id> spec
geetorusai issue document:restore <issue-id> spec <revision-id>
```
...
```sh
geetorusai issue work-products <issue-id>
geetorusai issue work-product:create <issue-id> --payload-json '{"kind":"report","title":"Q2 analysis"}'
geetorusai issue work-product:update <work-product-id> --payload-json '{"title":"Q2 analysis (final)"}'
geetorusai issue work-product:delete <work-product-id>
```
...
```sh
geetorusai issue interactions <issue-id>
geetorusai issue interaction:create <issue-id> --payload-json '{ ... }'
geetorusai issue interaction:accept <issue-id> <interaction-id> --selected-client-keys a,b
geetorusai issue interaction:reject <issue-id> <interaction-id> --reason "Out of scope"
geetorusai issue interaction:cancel <issue-id> <interaction-id> --reason "No longer needed"
geetorusai issue interaction:respond <issue-id> <interaction-id> --answers-json '[{"key":"q1","value":"yes"}]' --summary-markdown "Confirmed scope"
```
...
```sh
geetorusai issue tree-state <root-issue-id>
geetorusai issue tree-preview <root-issue-id> --payload-json '{"mode":"pause"}'
geetorusai issue tree-holds <root-issue-id> --status active --mode pause --include-members
geetorusai issue tree-hold:create <root-issue-id> --payload-json '{"mode":"pause","reason":"awaiting budget"}'
geetorusai issue tree-hold:get <root-issue-id> <hold-id>
geetorusai issue tree-hold:release <root-issue-id> <hold-id> --payload-json '{}'
```
...
```sh
geetorusai issue attachments <issue-id>
geetorusai issue attachment:upload <issue-id> -C <company-id> --file ./diagram.png
geetorusai issue attachment:upload <issue-id> -C <company-id> --file ./log.txt --comment-id <comment-id>
geetorusai issue attachment:download <attachment-id> --out ./diagram.png
geetorusai issue attachment:delete <attachment-id>
```
...
```sh
geetorusai issue label:list -C <company-id>
geetorusai issue label:create -C <company-id> --name "needs-review" --color "#4f46e5"
geetorusai issue label:delete <label-id>
```
...
```sh
geetorusai issue approvals <issue-id>
geetorusai issue approval:link <issue-id> <approval-id>
geetorusai issue approval:unlink <issue-id> <approval-id>
```
...
```sh
geetorusai issue read <issue-id>
geetorusai issue unread <issue-id>
geetorusai issue archive <issue-id>
geetorusai issue unarchive <issue-id>
```
...
```sh
geetorusai issue recovery-actions <issue-id>
geetorusai issue recovery:resolve <issue-id> --outcome restored --source-issue-status todo --resolution-note "Re-queued after restart"
geetorusai issue force-release <issue-id>
geetorusai issue child:create <parent-issue-id> --payload-json '{"title":"Subtask"}'
```
...
geetorusai issue live-runs <issue-id>
geetorusai issue active-run <issue-id>
```
...
```sh
geetorusai issue feedback:votes <issue-id>
geetorusai issue feedback:vote <issue-id> --payload-json '{"targetType":"comment","targetId":"<id>","vote":"up"}'
geetorusai issue feedback:list <issue-id> --vote down --status open --include-payload
geetorusai issue feedback:export <issue-id> --from 2026-01-01 --format ndjson --out ./feedback.ndjson
```
