...
Adapters are the server-side runtimes that actually execute an agent's work — they wrap Claude, Codex, or another provider and turn a wakeup into a real run. The CLI does not run an adapter; it inspects and administers the adapters the Geetorus runtime already has registered. Use these commands when you need to see what adapter types exist, read an adapter's config schema before hiring an agent against it, list the models a company can use, or install, update, and remove external adapter packages.

> **Note:** Most agent work in Geetorus is executed server-side by the runtime and its adapters. The CLI triggers and observes that work — see [What runs where](../../guides/welcome/key-concepts.md). The `adapter` commands let a board operator look under the hood of that execution layer.
...
```sh
geetorusai adapter list
geetorusai adapter get <adapter-type>
geetorusai adapter config-schema <adapter-type>
geetorusai adapter ui-parser <adapter-type>
```
...
geetorusai adapter list --json
geetorusai adapter config-schema codex_local --json
```
...
```sh
geetorusai adapter models <adapter-type> --company-id <company-id>
geetorusai adapter models codex_local --company-id <company-id> --refresh
geetorusai adapter models codex_local --company-id <company-id> --environment-id <environment-id>
```
...
```sh
geetorusai adapter detect-model <adapter-type> --company-id <company-id>
```
...
```sh
geetorusai adapter install --payload-json '{"packageName":"@scope/adapter","version":"1.2.3"}'
geetorusai adapter delete <adapter-type>
```
...
```sh
geetorusai adapter update <adapter-type> --payload-json '{"disabled":true}'
geetorusai adapter override <adapter-type> --payload-json '{"paused":true}'
geetorusai adapter reload <adapter-type>
geetorusai adapter reinstall <adapter-type>
```
...
```sh
geetorusai adapter test-environment <adapter-type> --company-id <company-id> --payload-json '{}'
```
