...
seo_description: Spin up an isolated, self-initialized Geetorus instance with a ready-made company and CEO agent so you can try the product locally.
---

# Test Drive Command

`test-drive` is the fastest way to see Geetorus working on your own machine. One command starts a fresh local instance, creates a company and a CEO agent for you, seeds the provider credential that agent needs, and opens the dashboard in your browser. It is built for kicking the tyres — a manual, throwaway environment you can create, poke at, and walk away from.

```sh
geetorusai test-drive
```

The command runs a **local Geetorus instance** the same way `run` does, then bootstraps starter content on top of it. Nothing here talks to a remote control plane you already operate; it stands up its own.
...
1. **Isolates the environment first.** Before the CLI's normal config and `.env` loading happens, `test-drive` clears every `GEETORUS_*` variable (and a handful of others like `DATABASE_URL`, `HOST`, and `PORT`) from the process so a stray shell export cannot point it at some other instance. The one thing it preserves is the environment variable you named with `--api-key-env`, if you used it.
...
> **Note:** The data directory is retained when Geetorus exits. If you did not pass `--data-dir`, `test-drive` creates a fresh temporary directory for you and prints its path — clean it up yourself when you are done.
...
geetorusai test-drive --harness codex

# OpenCode requires an openrouter/<model> id
geetorusai test-drive --harness opencode --model openrouter/anthropic/claude-sonnet-4
```
...
geetorusai test-drive --api-key-env MY_TEAM_ANTHROPIC_KEY
```
...
| Flag | Default | Use |
|---|---|---|
| `-d, --data-dir <path>` | a fresh temp directory | Geetorus data directory to create or reuse. Omit it to get an isolated throwaway directory. |
