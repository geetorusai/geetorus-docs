...
seo_description: The friendly front door for a fresh terminal — walks you through pointing the CLI at a running Geetorus instance in one guided flow.
---

# Connect Command

When you sit down at a fresh terminal and want the Geetorus CLI to start talking to a running instance, `connect` is the friendly front door. It walks you through one short conversation — which server, which persona, which company and agent — and at the end it has minted a token, saved a reusable profile, and printed the environment exports you need. You do not have to know any IDs up front or hand-craft a context file; the wizard discovers what is available and lets you pick.

Reach for `geetorusai connect` the very first time you set up the CLI, whenever you want to add another saved profile (a second instance, a different company, a new agent), or any time you would rather click through choices than type out `context set` and `auth login` by hand.
...
```sh
geetorusai connect --token-name "laptop-board" --api-key-env-var-name GEETORUS_API_KEY
```

| Flag | Default | Use |
|---|---|---|
| `--persona <persona>` | _(prompted)_ | The persona to configure: `board` or `agent`. Pass it to skip the persona question. |
| `--api-key-env-var-name <name>` | `GEETORUS_API_KEY` | The name of the environment variable the profile will read its key from at call time. The profile stores this **name**, never the token itself. |
...
The `GEETORUS_COMPANY_ID` and `GEETORUS_AGENT_ID` lines appear only when they apply, and the last line uses whatever name you set with `--api-key-env-var-name`. Paste the block into your shell and the rest of the CLI will pick up the new profile and credential immediately.
