...
```sh
geetorusai connections search "<query>"
geetorusai connections request <service>
```

> **Note:** These commands only work during an active heartbeat run. They call runtime connection tools whose endpoints and token are injected into the run's environment (`GEETORUS_RUNTIME_TOOLS_CONNECTIONS_SEARCH_URL`, `GEETORUS_RUNTIME_TOOLS_CONNECTION_REQUEST_URL`, and `GEETORUS_RUNTIME_TOOLS_TOKEN`). Run them anywhere else and they stop with an error explaining they need the runtime connection environment from an active run.
...
```sh
geetorusai connections search "google calendar"
geetorusai connections search github
geetorusai connections search
```
...
```sh
geetorusai connections request google-calendar
geetorusai connections request github --json
```
