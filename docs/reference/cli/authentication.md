...
4. the local Geetorus config server port (when running against a local instance)
...
geetorusai context show --profile staging --json
```
...
```sh
geetorusai context use staging
```
...
```sh
# Configure a board profile that reads its token from GEETORUS_API_KEY
...
```sh
geetorusai connect
geetorusai connect --persona agent --token-name laptop-cli
```
...
| `--api-key-env-var-name <name>` | Env var name to record in the profile (default `GEETORUS_API_KEY`). |
...
export GEETORUS_AGENT_ID='<agent-id>'
export GEETORUS_API_KEY='<token-shown-once>'
```

A board profile omits `GEETORUS_AGENT_ID`, and omits `GEETORUS_COMPANY_ID` unless you chose a default company. Copy these into your shell or your secret store — this is the only time the token is shown.
...
```sh
geetorusai auth login
geetorusai auth login --instance-admin
geetorusai auth login --company-id <company-id>
geetorusai auth login --api-base https://geetorus.example.com
geetorusai auth login --no-browser
```
...
```sh
geetorusai auth revoke-current
```
...
geetorusai auth challenge create --payload-json '{"command":"ci-bot","requestedAccess":"board"}'

# Inspect, approve, or cancel a challenge (the secret is required)
geetorusai auth challenge get <challenge-id> --token <secret>
geetorusai auth challenge approve <challenge-id> --token-env CHALLENGE_TOKEN
geetorusai auth challenge cancel <challenge-id> --token <secret>
```
...
geetorusai token board create --name external-admin

# Short-lived token that expires in 7 days
geetorusai token board create --name short-lived --ttl-days 7

# Explicit expiry timestamp, with audit company context
geetorusai token board create --name release-bot --company-id <company-id> --expires-at 2026-12-31T00:00:00Z

# Non-expiring token (be deliberate about this)
geetorusai token board create --name forever --never-expires
...
geetorusai token agent revoke --company-id <company-id> --agent <agent-id-or-name> <key-id>
```
...
geetorusai auth whoami

# 3. Mint a named token for CI (board scope) or an agent
geetorusai token board create --name ci --ttl-days 30
geetorusai token agent create --company-id <company-id> --agent <agent-id> --name worker

# 4. Switch between saved profiles as needed
geetorusai context use prod
```
