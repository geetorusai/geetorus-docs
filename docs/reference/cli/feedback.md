...
> **Note:** `feedback report` and `feedback export` resolve the company from `-C/--company-id`, then `GEETORUS_COMPANY_ID`, then your CLI context default. If none of those is set, the command falls back to the **first** company returned by the API. On an instance with more than one company, always pass `--company-id` so you export the company you mean.
...
```sh
geetorusai feedback report --company-id <company-id>
```
...
```sh
geetorusai feedback report --company-id <company-id> --payloads
```
...
```sh
geetorusai feedback report --company-id <company-id> --json
```
...
```sh
geetorusai feedback export --company-id <company-id> --out ./feedback/acme
```
...
```sh
geetorusai feedback export --company-id <company-id> --json
```
...
```sh
geetorusai feedback trace <trace-id>
geetorusai feedback trace <trace-id> --json
```
...
```sh
geetorusai feedback bundle <trace-id>
geetorusai feedback bundle <trace-id> --json
```
