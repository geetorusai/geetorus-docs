...
There are two command groups. The plural `geetorusai skills` group is the one you reach for in practice — it understands catalog references, resolves skills by `id`/`key`/`slug`, and prints readable tables. The singular `geetorusai skill` group is a thin, payload-driven wrapper over the same company-skill API endpoints; use it only when you need raw control over the request body in a script.

---

## The three operations

`geetorusai skills` spans three distinct things. Keeping them straight is the whole mental model:
...
The Geetorus app ships a curated catalog of skills. Catalog commands let you read it without writing anything to a company — there is no company context required to browse, search, or inspect.

```sh
geetorusai skills browse --kind bundled --category github
geetorusai skills search "pull request" --kind bundled
geetorusai skills inspect github-pr-workflow
```
...
```sh
geetorusai skills install github-pr-workflow --company-id <company-id>
geetorusai skills install github-pr-workflow --as review-prs --company-id <company-id>
```
...
```sh
geetorusai skills list --company-id <company-id>
geetorusai skills show <skill-ref> --company-id <company-id>
geetorusai skills file <skill-ref> --path SKILL.md --company-id <company-id>
geetorusai skills import <source> --company-id <company-id>
geetorusai skills create --name "Review PRs" --slug review-prs --body-file SKILL.md --company-id <company-id>
geetorusai skills scan-projects --project-id <project-id> --company-id <company-id>
```
...
```sh
geetorusai skills check --company-id <company-id>
geetorusai skills update <skill-ref> --company-id <company-id>
geetorusai skills update --all --company-id <company-id>
geetorusai skills audit <skill-ref> --company-id <company-id>
geetorusai skills reset <skill-ref> --yes --company-id <company-id>
```
...
```sh
geetorusai skills remove <skill-ref> --yes --company-id <company-id>
```
...
```sh
geetorusai skills agent list <agent-ref> --company-id <company-id>
geetorusai skills agent sync <agent-ref> --skill review-prs --skill agent-browser --mode add --company-id <company-id>
geetorusai skills agent clear <agent-ref> --yes --company-id <company-id>
```
...
`geetorusai skill` is a lower-level wrapper over the same company-skill API. It does not resolve `key`/`slug` references — every subcommand takes a literal `<skillId>` — and the create/import/scan/file-update operations are driven by a raw JSON payload. Reach for it only when you are scripting against a known skill ID and want direct control of the request body.

```sh
geetorusai skill list -C <company-id>
geetorusai skill get <skill-id> -C <company-id>
geetorusai skill file <skill-id> --path SKILL.md -C <company-id>
geetorusai skill file:update <skill-id> --payload-json '{"path":"SKILL.md","content":"..."}' -C <company-id>
geetorusai skill update-status <skill-id> -C <company-id>
geetorusai skill install-update <skill-id> -C <company-id>
geetorusai skill delete <skill-id> -C <company-id>
```
