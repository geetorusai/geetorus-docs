...
Geetorus tries to record which code the decision was made against, so a training example can be replayed meaningfully. The `code` object carries `repoUrl`, `ref`, `commitSha`, and a `resolution` field telling you how confident that commit is:
...
curl -sS -X POST \
  ...
  "https://geetorus.example.com/api/companies/{companyId}/decision-training" \
  -d '{
    "sourceKind": "approval",
...
curl -sS \
  -H "Authorization: Bearer {token}" \
  "https://geetorus.example.com/api/companies/{companyId}/decision-training/export.jsonl" \
