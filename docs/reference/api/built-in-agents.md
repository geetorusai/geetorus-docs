...
seo_description: First-party agents Geetorus ships ready to provision into any company by name, instead of configuring an equivalent role from scratch every time.
---

# Built-in Agents

First-party agents that Geetorus ships with, ready to provision into any company.

A built-in agent is a normal agent that Geetorus knows about by name. Instead of hiring it from scratch and wiring up its role, purpose, and instructions yourself, you provision it from a small registry that the server maintains. Each built-in has a stable `key` — like `briefs` or `learning` — and Geetorus resolves it through that key rather than a hardcoded database ID, so backend features can rely on "the Briefs agent for this company" without anyone pinning an ID.

Use this API when you want to turn on one of Geetorus's shipped agents for a company, check whether it's set up, point it at an adapter, or (for bundled built-ins like the Reflection Coach) control its schedule.
...
Under the hood a built-in agent is an ordinary row in the same `agents` table as any agent you hire. What sets it apart is a small, immutable marker stored on the agent's `metadata` as `metadata.geetorusBuiltInAgent`. The marker records the registry `key` and the feature keys the agent backs. That marker is how Geetorus finds the agent later, and it is also protected: you cannot forge, move, or strip it through the normal agent create and update routes. Only the built-in service is allowed to write it.
...
Three built-in agents ship with Geetorus today. Each row below comes straight from the registry.
...
What counts as "finished" depends on the adapter type. For the `process` and `command` adapters, Geetorus looks for a `command` or a `script`. For `http`, it looks for a `url`, `endpoint`, or `webhookUrl`. For the `openclaw_gateway` and `hermes_gateway` adapters, it looks for a `baseUrl` or `url`. For everything else — including the local adapters the shipped built-ins default to — it looks for a `model`.
...
The Reflection Coach is worth a closer look because it is both a built-in agent **and** a bundled skill. When you provision it, Geetorus materializes three things for the company: the agent's instructions, the `reflection-coach` skill, and a weekly `recent-agent-reflection` routine (disabled until you enable it). It also starts **paused** by default, so it never runs until you deliberately configure and enable it.
