---
geetorus_version: v2026.817.0
seo_title: Routines API
seo_description: Geetorus's recurring execution layer. Run an agent on a schedule, fire it from a webhook, or kick it off manually, and see what a routine does not do.
---

# Routines

Routines are Geetorus's recurring execution layer. Use them when you want an agent to run on a schedule, respond to a webhook, or be kicked off manually through the API.
...
- `hmac_sha256` uses `X-Geetorus-Signature` or `X-Hub-Signature-256`
- `github_hmac` uses `X-Hub-Signature-256` or `X-Geetorus-Signature` plus `X-Geetorus-Timestamp`
...
| `worktree_execution_cutoff` | `skipped_worktree_execution_cutoff` | Skipped — worktree execution cutoff | The server is running inside a development worktree (`GEETORUS_IN_WORKTREE`) where automatic run execution is not armed for this routine — either the worktree isn't armed at all, or the routine was created before the worktree's activation cutoff. This applies to scheduled ticks and webhook firings alike. |
