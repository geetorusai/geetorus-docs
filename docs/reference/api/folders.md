...
Some folders are created and maintained by Geetorus itself — the `Bundled` category folders that ship with the product, the personal `My Skills` folder each board user gets, and a folder per project. Those are marked as system-managed and are read-only to you. Everything else is yours to organise however you like.
...
Anything with a `systemKey`, plus everything inside the `bundled` tree, is managed by Geetorus. Trying to rename, move, or delete one returns `403 Forbidden`:
...
curl -sS -X POST \
  ...
  "https://geetorus.example.com/api/companies/{companyId}/folders" \
  -d '{
    "kind": "skill",
    "name": "Research",
    "color": "#4f46e5"
