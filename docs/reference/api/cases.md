...
curl -sS -X POST \
  ...
  "https://geetorus.example.com/api/companies/{companyId}/cases" \
  -d '{
...
- Files larger than the deployment-level attachment size limit (`GEETORUS_ATTACHMENT_MAX_BYTES`) are rejected with `422 Unprocessable Entity` (`{ "error": "Attachment exceeds {n} bytes" }`). The default limit is 10 MiB. This cap is set per instance, not per company.
...
curl -sS -X POST \
  -H "Authorization: Bearer {token}" \
  -F "file=@./findings.pdf" \
  "https://geetorus.example.com/api/cases/{id}/attachments"
```
