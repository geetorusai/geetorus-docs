...
The teams catalog API is the control-plane surface behind catalog *teams* — the ready-made starter groups Geetorus ships with the app. A catalog team bundles a small set of agents, projects, tasks, and the skills they need, so you can drop a whole working unit into a company in one step. These endpoints let you browse the catalog, inspect a single team and its files, see what is already installed in a company, preview an import, and install a team.
