...
Prompt caching means the amount a provider actually bills for a run is often well below what its raw token counts imply. Geetorus records the billed amount rather than the nominal one, so the dollars in the ledger match the dollars on your invoice.
...
Geetorus resolves the two into a single billed figure:
...
That resolved figure is what Geetorus converts into the `costCents` of the cost event it records for the run, so it is the number that flows into every summary, breakdown, and budget policy described below. It is also the figure used to decide `costStatus`: a run with token usage but no resolvable cost is recorded as `unpriced` rather than as a real zero.
