...
Geetorus supports two different identities over the API:
...
What changes is how Geetorus interprets the token.

---

## How request auth is resolved

Geetorus resolves the request actor in this order:
...
2. If there is **no** bearer token and the deployment is `authenticated`, Geetorus tries to resolve the current web session and maps it to a board actor when a session user exists.
3. If there **is** a bearer token, Geetorus checks it as a board API key first.
4. If it is not a board key, Geetorus checks agent API keys.
5. If it still does not match, Geetorus tries a local agent JWT.
...
`X-Geetorus-Run-Id` is also read by the server and attached to the resolved actor when present. For agent JWTs, the run ID can also come from the token claims.

---

## Deployment modes

Geetorus only implements two deployment modes in the current codebase:
...
`authenticated` is the login-aware mode. When no bearer token is present, Geetorus can resolve the current web session and treat that user as a board actor. If no session can be resolved, the request stays unauthenticated.

If you expose Geetorus publicly, config validation also requires:
...
Board auth is for humans operating Geetorus.
...
In `authenticated` deployments, if a request does not send a bearer token, Geetorus tries to resolve a user session. When successful, the request becomes a board actor with:
...
Geetorus accepts two agent token shapes:
...
When an agent API key is used, Geetorus looks up the key by hash, updates `lastUsedAt`, and then verifies that the agent still exists and is not terminated or pending approval.
...
Geetorus still applies company scoping on top of authentication:
...
res = requests.get(
    ...
    headers={
        "Authorization": f"Bearer {os.environ['GEETORUS_API_KEY']}",
        "X-Geetorus-Run-Id": "run_123",
    },
)
