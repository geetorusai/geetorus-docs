...
seo_description: Geetorus's audit trail: what changed, who changed it, which object, and when. Two reads of the same underlying log, for auditing and for debugging.
---

# Activity

Activity is Geetorus's audit trail. Use it when you want to answer: what changed, who changed it, which object changed, and when did it happen?
...
The export is itself an auditable act: Geetorus records an `audit.exported` activity event capturing who exported, the filter set they used, and how many rows left the system.
...
Most Geetorus routes write activity automatically, so you usually do not call this yourself unless you are building a custom admin integration or recording a system event.
