...
Companies are the top-level tenant boundary in Geetorus. Every agent, project, issue, approval, cost event, and asset belongs to exactly one company, and the API enforces that boundary on every company-scoped route.
...
Creates a new company and assigns it an automatic issue prefix. If `budgetMonthlyCents` is greater than `0`, Geetorus also creates the matching monthly budget policy in UTC calendar-month mode.
...
Geetorus exposes both broad and company-scoped portability routes:
...
A large export doesn't have to arrive in one upload. When a package is big, the client slices it into parts and streams them one at a time through a **transfer** — a durable run the server tracks so an interrupted upload resumes from the parts it already has instead of starting over. The UI and `geetorusai company import` both use this path automatically; see the [Export & Import guide](../../guides/power/export-import.md) for the user-facing flow.
