...
seo_description: The control-plane surface for the server-side adapter registry: which agent runtimes Geetorus can reach, how each is configured, and how packages install.
---

# Adapters API

The adapters API is the control-plane surface for the **server-side adapter registry**: which agent runtimes Geetorus can talk to, how each is configured, and how external adapter packages get installed at the instance level.
...
Returns the registry of server-side adapters Geetorus currently knows about: built-in adapter types plus any external adapters installed on the instance.
...
Installs an external adapter package on the instance. Used by the Adapter Manager UI and the `geetorusai adapter install` CLI workflow.
...
Serves the adapter-supplied UI parser bundle. The Geetorus UI fetches this script to render run output for the adapter. The wire contract is documented in [Adapter UI Parser](../adapters/adapter-ui-parser.md).

---

## Adapter device login

Some adapters authenticate with a **device login** — the kind where the tool prints a code and a URL, you approve it in a browser, and the CLI finishes signing in. These routes drive that flow from Geetorus for a company, scoped to one environment. They currently apply to the Codex adapter.
