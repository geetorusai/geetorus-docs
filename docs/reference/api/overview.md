...
seo_description: The shared reference for Geetorus's JSON control-plane API — companies, agents, issues, approvals, costs, routines, secrets, activity, and dashboard state.
---

# API Overview

Geetorus exposes a JSON API for company control-plane work: companies, agents, issues, approvals, costs, routines, secrets, activity, and dashboard state. This page is the shared reference for how the API is structured before you jump into a specific resource.
...
Geetorus supports different request identities depending on the deployment mode and caller type:
...
> **Note:** The server also reads `X-Geetorus-Run-Id` on mutating requests during agent runs. That is mostly relevant for issue comments, checkout, and other run-linked actions.
