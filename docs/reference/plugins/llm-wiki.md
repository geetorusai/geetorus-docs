...
If you want a cited, in-dashboard wiki that your agents build and maintain from raw source material — Geetorus issues, comments, documents, and files you drop into a folder — the `@geetorusai/plugin-llm-wiki` plugin is the surface that does it. It captures sources into a local folder, lets agents ingest them into markdown pages with backlinks and provenance, and ships a Wiki page inside Geetorus so you (and the agents) can browse the result.

This page is for **operators** who want to enable and configure the plugin.

> The LLM Wiki plugin is in alpha alongside the plugin runtime itself. Expect breaking changes between Geetorus releases and pin your version when you depend on it.

---

## Quick install

The plugin ships as a published npm package, so any Geetorus install (Docker, systemd, bare-metal — all the same) can pull it in with one command from a host that has your Geetorus CLI configured:

```sh
geetorusai plugin install @geetorusai/plugin-llm-wiki
```

Or from the dashboard: **Settings → Plugins → Install Plugin**, paste `@geetorusai/plugin-llm-wiki`, submit.

The plugin won't appear in `geetorusai plugin examples` — that command only lists the four built-in reference *example* plugins, not the full catalogue of first-party plugins. Install LLM Wiki by full package name.
...
Once the plugin is installed and enabled, three things show up in the Geetorus UI:
...
- Three managed routines, all paused by default: `cursor-window-processing` (process Geetorus issue-history windows), `nightly-wiki-lint` (audit pages for orphans, stale provenance, and contradictions), and `index-refresh` (rebuild `wiki/index.md`).
- A bundle of managed skills installed for the maintainer agent: `wiki-maintainer`, `wiki-ingest`, `wiki-query`, `wiki-lint`, `geetorus-distill`, and `index-refresh`.
...
One thing to know up front: **Geetorus-derived ingestion always writes into the `default` space.** The cursor-window, distill, and backfill flows are not yet space-aware, so non-default spaces stay on manual or raw-file ingest for now. Per-space Geetorus ingestion profiles are tracked as a later phase.
...
- [How-to → Develop a plugin locally](../../how-to/develop-a-plugin-locally.md) — point Geetorus at a local checkout of the plugin.
