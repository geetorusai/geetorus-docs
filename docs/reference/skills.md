...
If you want to browse Geetorus's shipped skill catalog, start with the [Bundled skills](./skills/bundled.md) and [Optional skills](./skills/optional.md) indexes. Those pages group every shipped catalog skill by category and link to the full per-skill reference pages.
...
The frontmatter is parsed by Geetorus's own minimal YAML reader (`parseFrontmatterMarkdown` in `server/src/services/company-skills.ts`). It supports flat scalars, nested objects, and list literals — not the full YAML grammar.
...
| Path | Used for |
|---|---|
| `metadata.skillKey` / `metadata.canonicalKey` / `metadata.geetorusSkillKey` | Carry a canonical key across imports. |
| `metadata.geetorus.skillKey` / `metadata.geetorus.key` | Same, nested under a `geetorus` block. |
| `metadata.sourceKind` | One of `geetorus_bundled`, `github`, `skills_sh`, `url`, `local_path`, `project_scan`, `managed_local`, `catalog`. |
...
When importing from a project workspace where `SKILL.md` lives at the repo root (rather than a dedicated skill folder), Geetorus switches to `project_root` inventory mode and only walks `references/`, `scripts/`, and `assets/` — so it does not slurp the entire repository.

### Example skills you can read

Bundled with the Geetorus server (`skills/` next to the server):

- `geetorus` — base heartbeat procedure.
- `geetorus-board` — manage a company as a board member via chat: onboarding (company creation, CEO setup, hiring plans), agent management, approvals, task monitoring, cost oversight, and work-product review. Set up by `geetorusai board setup`.
- `geetorus-create-agent` — governance-aware hire workflow.
- `geetorus-converting-plans-to-tasks` — the Geetorus way of translating a plan into assigned issues with the right specialty, dependencies, and parallelization.
...
Community examples in [`geetorusai/companies`](https://github.com/geetorusai/companies):
...
### Artifact upload in the bundled `geetorus` skill

The bundled `geetorus` skill ships an artifact-upload helper so that work products end up on the issue, not stranded in the agent's workspace. Its **Generated Artifacts and Work Products** section in `SKILL.md` tells the agent that whenever a run produces a user-inspectable file, it should upload that file to the current issue before final disposition — a local filesystem path is useless to board users, reviewers, and cloud operators who can't reach the agent workspace.

The skill bundles a helper script at `scripts/geetorus-upload-artifact.sh` (relative to the installed skill directory) and a reference doc at `references/artifacts.md`. The helper takes a file path plus `--title` and `--summary` flags:
...
It reads the run's environment — `GEETORUS_API_URL`, `GEETORUS_API_KEY`, `GEETORUS_COMPANY_ID`, `GEETORUS_TASK_ID`, and `GEETORUS_RUN_ID` — then uploads the file as an issue attachment, creates an attachment-backed artifact work product (the default), and prints issue-safe markdown links the agent can drop into its final comment. The underlying upload route is the attachment endpoint documented under [Issues API → Attachments](./api/issues.md#attachments).

### Monitors and watchers in the bundled `geetorus` skill

When an agent writes "I've set a watcher and I'll check back once CI finishes", you want that to be true — otherwise the issue quietly stalls while you wait for a wake-up that was never scheduled. The bundled `geetorus` skill has a **Monitors and Watchers** section whose whole job is to keep what an agent says in its comments matched to what is actually stored on the issue.

The reason it needs saying: a run (heartbeat) is an ephemeral execution window, and nothing keeps watching after it exits. The only thing that can auto-resume an issue on its own is a persisted **issue monitor** — durable state on the issue itself (`monitorNextCheckAt`, `monitorScheduledBy`, plus an execution-policy `monitor` block carrying `kind`, `serviceName`, `externalRef`, `timeoutAt`, and `maxAttempts`). A server-side scheduler (`tickDueIssueMonitors`) polls for issues whose `monitorNextCheckAt` has passed and wakes the assignee agent again with `GEETORUS_WAKE_REASON=issue_monitor_due`. That is timer-based polling, not an event subscription: Geetorus is not notified the moment CI or another external check finishes, it just wakes the agent on a schedule so it can look again.
...
| Create empty (Geetorus-managed) | `POST /api/companies/{companyId}/skills` |
...
```json
{ "source": "https://github.com/geetorusai/geetorus" }
{ "source": "https://github.com/geetorusai/geetorus/tree/main/skills/geetorus" }
{ "source": "https://github.com/geetorusai/geetorus/blob/main/skills/geetorus/SKILL.md" }
{ "source": "geetorusai/geetorus" }
{ "source": "geetorusai/geetorus/geetorus-create-agent" }
{ "source": "https://skills.sh/geetorusai/geetorus/geetorus-create-agent" }
{ "source": "npx skills add geetorusai/geetorus --skill geetorus-create-agent" }
...
4. Materialise files for catalog-style sources into `<geetorusInstanceRoot>/skills/{companyId}/__catalog__/<runtimeName>/` so adapters can read them.
...
Editable, Geetorus-managed skills are written to `<geetorusInstanceRoot>/skills/{companyId}/<slug>/`. Read-only sources (GitHub, skills.sh, URL) keep the `markdown` body in the database row and only materialise into a temporary location when an adapter needs the files on disk.

Bundled skills (those shipped in the server's `skills/` directory) are re-imported on every list call (`ensureBundledSkills`). They cannot be edited or deleted — installing the same Geetorus release will recreate them.

---

## 3. App-shipped catalog

Beyond importing skills from GitHub, skills.sh, URLs, or local folders, Geetorus ships its own **catalog** of ready-made skills inside the app. You browse the catalog, install the skill you want into a company, and from then on Geetorus can keep that copy in step with the version it shipped — checking for updates, auditing the bytes, and resetting back to the pinned origin if needed.
...
For the equivalent `geetorusai skills` CLI commands (browse, install, audit, update, reset), see the [Skills commands](./cli/skills.md) page in the CLI reference.
...
```json
{
  ...
  "desiredSkills": ["geetorus", "code-review", "deploy-runbook"]
}
```
...
- a canonical **key** (`geetorusai/geetorus/geetorus`),
- or a **slug** (`code-review`) — but only when the slug is unique inside the company.

The server resolves each reference to its canonical key and persists exactly that list under `adapterConfig.geetorusSkillSync.desiredSkills` — the desired set is precisely what you send, with no extra keys folded in. Unknown or ambiguous references fail the request with `Invalid company skill selection (...)`.

### After hire — `POST /api/agents/{agentId}/skills/sync`

```json
{
  "desiredSkills": ["geetorus", "code-review"]
}
```
...
```json
{
  ...
  "supported": true,
  "mode": "persistent",
  "desiredSkills": ["geetorusai/geetorus/geetorus", "company/<id>/code-review"],
  "entries": [
    {
...
| `unsupported` | Geetorus records the assignment but cannot push files into the runtime. Adapters such as `openclaw_gateway` fall here — manage skills inside the remote runtime instead. |
...
**Bundled skills live in the library.** `geetorus` and any other skill marked `geetorus_bundled` are imported into the company library as read-only entries. They are no longer force-added to an agent's resolved set — `resolveDesiredSkillAssignment` now persists exactly the desired set you sync, nothing more.
...
- **"({N}) User-installed skills, not managed by Geetorus"** — collapsed by default; read-only entries surfaced when an adapter reports skills it discovered itself (for example, a global skills bundle on the host machine). These carry a `user_installed` or `external_unknown` origin and cannot be edited from here.
...
|---|---|---|---|
| **What it is** | An instruction document the agent loads on demand | A code package that adds API routes, UI surfaces, or runtime services | A bridge between Geetorus and an agent runtime |
...
| **Lives at** | Company level, in the company skill library | Instance level, in `~/.geetorus/adapter-plugins/` | Built into the server or registered as an external adapter |
...
| **Authored by** | Anyone who can edit the company skill library | Plugin authors (use the [`geetorus-create-plugin` skill](https://github.com/geetorusai/geetorus/blob/master/.agents/skills/geetorus-create-plugin/SKILL.md)) | Adapter authors (see [Creating an Adapter](./adapters/creating-an-adapter.md)) |
| **Versioning** | Pinned to a git commit (GitHub/skills.sh) or live (local) | Pinned by the plugin's package version | Pinned by the Geetorus release |
...
|---|---|---|
| Bundled with Geetorus | `geetorusai/geetorus/{slug}` | `geetorusai/geetorus/geetorus` |
| GitHub / skills.sh | `{owner}/{repo}/{slug}` | `geetorusai/geetorus/geetorus-create-agent` |
...
| Geetorus-managed (created from the UI) | `company/{companyId}/{slug}` | `company/5cbe79ee.../delegation-checklist` |
...
| `local_path` (managed) | Live | None — files refresh on read | Stored under `<geetorusInstanceRoot>/skills/{companyId}/`. Edited via `PATCH /skills/{id}/files`. |
...
| `geetorus_bundled` | Pinned to the Geetorus release | Upgrade Geetorus itself | Re-imported on every list call; cannot be edited. The bundled `geetorus` skill additionally carries beta releases an agent can pin — see [below](#beta-releases-of-the-bundled-geetorus-skill). |
...
### Beta releases of the bundled `geetorus` skill

The bundled `geetorus` skill (canonical key `geetorusai/geetorus/geetorus`) also carries **releases**: frozen snapshots an agent can be pinned to instead of the live default. For why you would pin one and how the picker behaves, read [Pinning a beta release of the Geetorus skill](../guides/org/skills.md#pinning-a-beta-release-of-the-geetorus-skill) in the guide. This section is the data and wire shape.

**The flag.** `enableBetaSkills` is an instance experimental setting, catalogued as **Beta skills** — *"Allow agents to pin beta releases of the Geetorus core skill."* Its tier is `preference`, meaning a tenant-controlled setting the Geetorus Cloud harness never manages, in contrast to the `managed`-tier flags that can arrive locked. Both `cloudDefault` and `selfHostedDefault` are `false`, so it is off on every instance until an operator turns it on.

**Where the snapshots come from.** The server reads `skills-releases/geetorus/releases.json` and seeds one skill version per manifest entry into every company that has the bundled `geetorus` skill. Each entry carries `id`, `releaseName`, `releasedAt`, `notes`, and `dir` — the folder holding that snapshot's files, resolved relative to the registry root. Seeding never moves the skill's current version: the live default is left alone, and a release only takes effect on agents that pin it.
...
```json
{
  "desiredSkills": [
    "code-review",
    { "key": "geetorusai/geetorus/geetorus", "versionId": "<company skill version uuid>" }
  ]
}
```
...
- Geetorus's YAML reader is intentionally strict-but-narrow. If a value is silently empty, check that:
...
- Recover by validating the YAML in a normal parser; if it parses there but not in Geetorus, file a bug — the in-house parser is documented above.

### "An agent gets a skill it never asked for"

- It's bundled-required. `geetorusai/geetorus/*` skills with `metadata.sourceKind: geetorus_bundled` are unioned into every resolved `desiredSkills` set by the server. Removing them from the request has no effect.
...
- It's a bundled skill. The server re-imports `skills/` from disk on every company list call. To suppress a bundled skill, remove it from the Geetorus release or run a forked build.
