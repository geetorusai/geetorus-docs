...
> Take a new MCP or vendor integration from an input link or brief all the way to delivery — cited research first, an explicit human approval gate in the middle, then one governed Geetorus connector per approved connection.

A two-phase playbook for researching and shipping MCP/vendor integrations in a way people can review and sign off on. It turns a link or a short vendor brief into a cited research pull request, waits for a human to accept the exact revision, and only then writes the connector code — one Geetorus pull request per approved connection.
...
Starting from the input link or vendor brief, Phase A produces a reviewable, cited **research pull request** in the `geetorus-content` repository. It gathers what the integration would involve and writes it up with its sources, so a person can read the findings and decide. No connector code is written in this phase.
...
Only after that approval does the skill implement the connector in the Geetorus app. It opens **one Geetorus app pull request per approved connection**, so each connection is reviewed and shipped on its own.

## The one-PR-per-connection rule

Every approved connection gets its own Geetorus app pull request. Connections are not bundled together — this keeps each one reviewable in isolation and makes it clear exactly what was approved and shipped for each.
...
| Field | Value |
|---|---|
| Catalog id | `geetorusai:optional:software-development:prepare-mcp-integration` |
| Canonical key | `geetorusai/optional/software-development/prepare-mcp-integration` |
