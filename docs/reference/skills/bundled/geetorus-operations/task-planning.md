...
> Turn a Geetorus issue or request into a structured implementation plan with child task graph, blockers, owners, and acceptance criteria, then save it as the issue `plan` document.

Turn a Geetorus issue or request into an implementation plan the executor can actually run: a child task graph, real blockers, named owners, and a defined acceptance bar — saved as the issue `plan` document. Avoid plans that read well but can't be split into work.
...
| Field | Value |
|---|---|
| Catalog id | `geetorusai:bundled:geetorus-operations:task-planning` |
| Canonical key | `geetorusai/bundled/geetorus-operations/task-planning` |
| Catalog path | `catalog/bundled/geetorus-operations/task-planning` |
| Kind | `bundled` |
| Category | `geetorus-operations` |
| Slug | `task-planning` |
| Entrypoint | `SKILL.md` |
...
| Requires | — |
| Tags | `geetorus`, `planning`, `issues`, `delegation` |
| Files | 1 |
