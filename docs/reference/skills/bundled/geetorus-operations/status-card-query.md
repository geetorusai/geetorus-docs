...
> Create and maintain agent-authored Geetorus status cards, or compile a prose interest prompt into bounded CompanySearchQuery objects and write the first summary from the assigned Summarizer run.
...
| Field | Value |
|---|---|
| Catalog id | `geetorusai:bundled:geetorus-operations:status-card-query` |
| Canonical key | `geetorusai/bundled/geetorus-operations/status-card-query` |
| Catalog path | `catalog/bundled/geetorus-operations/status-card-query` |
| Kind | `bundled` |
...
| Requires | — |
| Tags | `geetorus`, `status`, `search`, `reporting`, `operations` |
| Files | 1 |
...
The skill normalises the run-provided API base first, so the same snippet works whether `GEETORUS_API_URL` ends in `/api` or not, then creates the card:
