...
seo_description: What changed in these docs — pages added, rewritten, or expanded — with every documentation update. For product releases, see the Geetorus changelog.
---

# Documentation Changelog

What changed in **these docs** — pages added, rewritten, or expanded — with each documentation update. This is a changelog for the documentation itself, not for Geetorus the product.

The docs track Geetorus's [calendar-versioned](https://github.com/geetorusai/geetorus/releases) releases (`YYYY.MDD.P`), so each entry is tagged with the Geetorus release the docs were brought in line with. For the product's own release notes — the actual feature and fix history — see the [Geetorus releases page](https://github.com/geetorusai/geetorus/releases). To update your install, see [Update Geetorus](../how-to/update-geetorus.md).
...
- [The managed-agent Command](cli/managed-agent.md) — provision and qualify a locked-down Anthropic managed agent and environment, then save the company profile Geetorus reads when it dispatches work to it.
- [The test-drive Command](cli/test-drive.md) — one command spins up an isolated local instance with a ready-made company and CEO agent, seeds the provider credential, and opens the dashboard, so you can try Geetorus without wiring anything up.
...
- [Connect an Agent to GitHub](../how-to/connect-agent-to-github.md) — a new **Option C: managed GitHub connection** through Connectors, where Geetorus resolves a per-run credential you never mint or rotate, authors commits as the connected GitHub identity automatically, fails closed rather than falling back to a PAT, and keeps the linked PR's status in sync over the connection's webhook.
...
- [Environment Variables](deploy/environment-variables.md) — a new **Geetorus ID Connector** block for the Gmail/Workspace OAuth broker (`GEETORUS_ID_CONNECTOR_BASE_URL`, `_ENVIRONMENT`, `_INSTANCE_ID`, and the sign/seal private keys), and the HTTP adapter's `GEETORUS_HTTP_ADAPTER_PRIVATE_ENDPOINT_ALLOWLIST`.
- [Codex Adapter](adapters/codex.md) — the default model is now the concrete `gpt-5.6-sol` (the bare `gpt-5.6` alias is rewritten automatically), Fast mode via `fastMode`, `modelReasoningEffort` tiers, and interactive sandbox device login backed by a company-scoped credential cache you can disable with `GEETORUS_CODEX_AUTH_CACHE`.
...
- [Environment Variables](deploy/environment-variables.md) — new deployment settings: `GEETORUS_WORKSPACE_REAPER_COOLDOWN_DAYS` (how long a terminal workspace waits before it's archived), opt-in Sentry error monitoring via `SENTRY_DSN`, and the operator controls `GEETORUS_HIDDEN_SETTINGS` and `GEETORUS_SETTING_DEFAULTS`.
- [Instance Settings](../administration/settings.md) — a new section for operators hosting Geetorus for others: hiding settings surfaces by key and overriding setting defaults, neither of which is ever persisted.
...
- [Companies API](api/companies.md) and [Cases API](api/cases.md) — `brandColor` removed from the company shape and branding routes; the attachment cap is the deployment-level `GEETORUS_ATTACHMENT_MAX_BYTES`, not a per-company field.
...
- [CLI Setup Commands](cli/setup-commands.md) — after `onboard` installs the background service, it now hands you off to the running instance: it waits for the port the service actually bound, prints the dashboard URL, and opens it in your browser. Headless runs print the URL, and `GEETORUS_NO_BROWSER=1` opts out of the browser launch.
...
- [Update Geetorus](../how-to/update-geetorus.md) and [CLI installation](cli/installation.md) — the four release channels (`stable`, `beta`, `nightly`, `canary`) and the new `geetorusai channels` command that shows which one your install follows.
...
- [`service` CLI](cli/service.md) — installing, starting, and inspecting Geetorus as a background service.
- [Status Card Query skill](skills/bundled/geetorus-operations/status-card-query.md) — the bundled skill that teaches an agent to manage status cards.
...
- [Update Geetorus](../how-to/update-geetorus.md) — rewritten around checking before you commit, switching channels, rolling back, and the pre-update backup.
...
- [Environment Variables](deploy/environment-variables.md) — `GEETORUS_*` binding pass-through and opt-outs.
...
- The `geetorus-board` bundled skill — added to [Skills](../guides/org/skills.md).
...
- [Develop a plugin locally](../how-to/develop-a-plugin-locally.md) — a walkthrough of `geetorusai plugin init`, local-path install, the dev watcher, and reload.
...
_This changelog begins at v2026.513.0, the first release tracked in this repo. For the product's full feature and fix history, see the [Geetorus releases page](https://github.com/geetorusai/geetorus/releases)._
