---
geetorus_version: v2026.618.0
seo_title: Pi Adapter
seo_description: Run the Pi coding agent CLI on the Geetorus host, with its built-in tool set — read, bash, edit, write, grep, find, ls — and provider configuration.
---

# Pi

`pi_local` runs the Pi coding agent CLI on the same machine as Geetorus. Use it when you want Pi's built-in tool set (read, bash, edit, write, grep, find, ls), provider/model routing, and session resume across heartbeats.
...
- You want Geetorus to resume Pi sessions across heartbeats via `--session`.
...
> **Note:** Geetorus requires an explicit `model` value for `pi_local` agents. Use `pi --list-models` to list valid options on your machine.

---

## Session Persistence

Pi sessions are stored under `~/.pi/geetoruss/` and resumed with `--session` on the next heartbeat. The session is scoped per agent so multiple Geetorus agents can share a host without stepping on each other.
...
Pi has no base-URL flag or env var, so the only way to point it at a custom or remote OpenAI- or Anthropic-compatible endpoint is a `models.json` file in its agent-config directory. Geetorus wires this up for you through the `GEETORUS_PI_PROVIDERS` environment variable.

Set `GEETORUS_PI_PROVIDERS` to a JSON object in Pi's `models.json` `providers` shape — keyed by provider name, with each entry describing how to reach the endpoint and which models it serves. Geetorus reads it server-side, writes `{"providers": ...}` to a managed `models.json` in a temporary agent-config dir, and points `PI_CODING_AGENT_DIR` at that dir for the run. (For remote execution targets, the dir is shipped to the sandbox and `PI_CODING_AGENT_DIR` is repointed at the in-sandbox copy.)
...
- **Secrets stay declarative.** An `apiKey` may be a literal value or an `{env:VAR}` placeholder. Placeholders are expanded server-side against the run `env` and the Geetorus process env, so gateway keys are baked into `models.json` where the value is reliably present. An unresolvable placeholder (unset, or set to an empty string) is left intact for Pi to try itself.
