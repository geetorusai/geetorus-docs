...
seo_description: Run Google's Gemini CLI on the Geetorus host, with session resume across heartbeats and configurable sandboxing for each agent you point at it.
---

# Gemini CLI

`gemini_local` runs Google's Gemini CLI on the same machine as Geetorus. Use it when you want a local agent with session resume, configurable sandboxing, and Geetorus skills injected into Gemini's skills directory.
...
- You want Geetorus to manage skills locally without polluting the project directory.
...
> **Running in Docker?** The official Geetorus image pre-bundles the Gemini CLI alongside the other local CLIs, so `gemini_local` works in-container without a manual install. The image also sets `GEMINI_SANDBOX=false` for safe in-container use. You still need to supply credentials — see [Docker](../deploy/docker.md).
...
- **`auto` (default) — ACP preferred.** Geetorus runs Gemini through the Agent Client Protocol (ACP) when the host meets the prerequisites, and falls back to the Gemini CLI — with diagnostics explaining why — when it can't.
...
If Gemini reports an unknown session error, Geetorus retries with a new session automatically.
...
This pre-selection only applies to runs where Geetorus provisions a managed home directory (remote execution targets). For ordinary local runs the adapter uses your real home directory, where your existing Gemini auth is already in place.

## Skills Injection

Gemini CLI symlinks Geetorus skills into `~/.gemini/skills`.

The adapter does not overwrite existing user skills. It only exposes the Geetorus-managed skills it needs for the run.
