...
seo_description: Run OpenAI's Codex CLI on the Geetorus host as a local coding agent, with persistent session state and a managed CODEX_HOME per agent.
---

# Codex

`codex_local` runs OpenAI's Codex CLI on the same machine as Geetorus. Use it when you want a local coding agent with persistent session state, managed `CODEX_HOME`, and Geetorus skills injected into the Codex skills directory.
...
- You want Geetorus to manage a per-company Codex home when possible.
- You want repo instructions, Geetorus instructions, and Codex's own runtime behavior to work together.
...
- Codex CLI is not installed or cannot be executed from the Geetorus host.
...
|---|---:|---|
| `cwd` | no | Absolute working directory for the agent. Recommended in practice. If omitted, the adapter falls back to the current process working directory. Geetorus creates the path when permissions allow. |
...
| `fastMode` | no | Enables Codex Fast mode by setting `service_tier="fast"` and `features.fast_mode=true`. Supported on `gpt-6-astra`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5.5`, and `gpt-5.4`, and passed through for manual model ids. When the configured model can't use it, Geetorus ignores the setting and notes why. |
...
| `outputInactivityTimeoutMs` | no | How long the adapter waits for Codex to produce output before treating the run as stuck. The timer resets every time Codex emits a parsed event, so a busy run never trips it. Defaults to 7 minutes (`420000`) when unset. Set it to `null` to switch the monitor off entirely — only do that for tasks you know go quiet for long stretches, since Geetorus's platform-level one-hour silent-run safety net still applies. When it fires, the adapter stops the Codex process and reports the run as failed with a message like `monitor: no codex output for 7m 0s`. |
...
- **`auto` (default) — ACP preferred.** Geetorus runs Codex through the Agent Client Protocol (ACP) when the host meets the prerequisites, and falls back to the Codex CLI — with diagnostics explaining why — when it can't.
...
| `stateDir` | Geetorus-managed | Optional ACP session-state directory. Defaults to Geetorus's company- and agent-scoped storage. |
...
You can keep `engine` on `auto` when this agent runs in a Geetorus sandbox environment. If that sandbox provides Geetorus's bidirectional process session, Geetorus keeps the ACP engine and its structured live transcript; you do not add a separate bridge setting to the adapter config.
...
You can also type a model id that is not in this list. Anything Geetorus does not recognize is treated as a manual model id and passed straight through to the Codex CLI.

> **Heads-up:** The default is now the concrete `gpt-5.6-sol` slug. The bare `gpt-5.6` alias has no OpenAI-published metadata, so Geetorus rewrites `gpt-5.6` to `gpt-5.6-sol` automatically — an agent still configured with the old alias keeps working and stops triggering the CLI's "model metadata not found" warning.
...
New Codex agents start with this profile switched off. When an agent is created and its runtime config does not already mention `cheap`, Geetorus records `modelProfiles.cheap = { enabled: false }` — the profile is present but dormant, waiting for you to configure it and turn it on.
...
Codex injects Geetorus skills into the effective `CODEX_HOME/skills` directory.

In the default managed-home mode, Geetorus uses a per-company Codex home under the active Geetorus instance and seeds it from the shared Codex home for auth continuity.

When Geetorus runs inside a managed worktree instance, that Codex home is worktree-isolated so sessions, logs, and skills do not leak across checkouts.

For manual local CLI use outside Geetorus, run:

```sh
pnpm geetorusai agent local-cli codexcoder --company-id <company-id>
```
...
If you set `instructionsFilePath`, Geetorus reads that file and prepends it to the stdin prompt sent to `codex exec` on every run.
...
> **Tip:** Use `instructionsFilePath` for Geetorus-managed instructions. Use repo-local instruction files when you want Codex to pick them up naturally from the workspace.

---

## Authentication And Per-Agent Homes

Each `codex_local` agent runs against its own managed Codex home, so one agent can never spend against another agent's login or share its Codex state. Because that home starts empty, Geetorus seeds credentials into it for you before launching Codex:

- **You inherit the host Codex login by default.** If you are already signed in to Codex on the Geetorus host (a ChatGPT-subscription login), Geetorus links that login into the agent's managed home automatically. You do not have to log in again per company or per agent.
- **A per-agent API key wins when you set one.** If the agent's `env` carries an `OPENAI_API_KEY`, Geetorus writes that key into the managed home as API-key auth instead of borrowing the host login.
- **Your own `CODEX_HOME` is left alone.** If you point the adapter at a `CODEX_HOME` outside Geetorus's managed company tree, Geetorus treats it as self-managed and never seeds or overwrites it.
...
When an agent runs in a Geetorus sandbox environment that has no ready Codex login, the environment test reports that authentication is missing and Geetorus can start an interactive device login for you. Complete the login in your browser and Geetorus promotes the resulting subscription credential into the company scope so later heartbeats reuse it.

Geetorus also keeps a company-scoped, identity-keyed cache of subscription credentials (one usable credential per account), so a fresher login recovered from a sandbox run can refresh an account the host already holds. The cache never seeds an empty host and never switches which account an agent uses. It is on by default; set `GEETORUS_CODEX_AUTH_CACHE` to a falsy value (`0`, `false`, `no`, or `off`) on the Geetorus host to turn it off.

### CODEX_HOME sync into a sandbox

When this agent runs in a Geetorus sandbox, the adapter does not copy your whole Codex home into the sandbox. Instead it stages an explicit allowlist, so only the material a run actually needs crosses the boundary and none of your host-local runtime state comes along for the ride. The synced entries are:
...
Codex has no CLI flag or env var for pointing at a custom OpenAI-compatible endpoint. Custom endpoints live as `[model_providers.<id>]` tables in `$CODEX_HOME/config.toml`. To route Codex at your own gateway or provider without hand-editing that file, set the `GEETORUS_CODEX_PROVIDERS` environment variable on the Geetorus host.

`GEETORUS_CODEX_PROVIDERS` is a JSON object that maps onto Codex's `config.toml` schema:
...
- String values may use `{env:VAR}` placeholders. Geetorus expands them server-side against the run env and `process.env`, which is handy for fields that must carry a literal value such as `http_headers`. Prefer Codex's own `env_key` indirection for the bearer key. Unresolvable placeholders are left intact.

Geetorus writes these definitions into a managed region of the per-company managed Codex home's `config.toml`, delimited by `# >>> geetorus codex providers ... managed, do not edit >>>` marker comments. Your existing config is preserved, conflicting definitions are excised so the managed ones win, and the original file is restored after the run.

> **Note:** This merge only happens for the managed Codex home. If your adapter config explicitly sets `env.CODEX_HOME`, Geetorus leaves that user-managed home untouched and surfaces a note instead of merging.
...
```json
{
  ...
  "adapterConfig": {
    "cwd": "/Users/me/projects/geetorus-workspace",
    "model": "gpt-5.6-sol",
    "instructionsFilePath": "/Users/me/projects/geetorus-workspace/INSTRUCTIONS.md",
