...
- **Environment variables travel through ACPX session options.** Instead of being baked into a shell script, your agent's environment (including the values Geetorus injects for a run) is handed to the ACPX runtime as part of the session options and applied when the process spawns.
...
- **Symlinks fall back to copies.** Geetorus stages runtime skills and config files into place with symlinks. On Windows, creating a symlink can fail with an `EPERM` permission error; when that happens, the engine copies the file instead so setup still succeeds.
...
Geetorus deliberately keeps the retired adapter registered so that a stale row fails with a clear message instead of silently falling back to the `process` adapter. If a run logs a retirement message naming `acpx_local`, that agent's row predates the migration — open the agent and switch it to `claude_local` or `codex_local` with `engine` set to `acp`.
