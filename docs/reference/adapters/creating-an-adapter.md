...
|---|---|---|
| Source | Lives in the Geetorus repo | Lives in its own package |
...
For most new runtime integrations, start as an external adapter package. Move to a built-in only if Geetorus itself needs to ship it.
...
The `cli/format-event.ts` file is optional. Add it only if you want a custom live-watch formatter for `geetorusai run --watch`.
...
That module is the contract Geetorus relies on for the adapter lifecycle.
...
1. Read config with the safe helpers from `@geetorusai/adapter-utils/server-utils`.
2. Build the runtime environment with `buildGeetorusEnv(agent)`.
...
| `buildGeetorusEnv()` | Inject the standard `GEETORUS_*` variables. |
...
Your adapter should make Geetorus skills visible to the runtime without polluting the user's workspace.
...
For built-in adapters, the UI parser can live inside Geetorus source. For external adapters, it must be standalone and browser-safe.
