...
`hermes_gateway` connects Geetorus to a **remote Hermes API server** over HTTP or HTTPS. Use it when Hermes Agent is already running somewhere — another host, a cloud VM, or a separate process — and exposes its API server. Geetorus calls the Hermes REST/SSE API directly to create runs, stream events, and stop runs on timeout.

This is a first-class built-in adapter as of v2026.626.0. You configure it through the standard Geetorus agent config form, the same way you would configure `claude_local` or `codex_local`.
...
- You want to connect Geetorus to a cloud-hosted or team-shared Hermes instance.
- You need to call multiple Hermes servers from one Geetorus deployment.
- You are building a multi-machine setup where Hermes and Geetorus run independently.

## When Not To Use

- You want Geetorus to launch Hermes as a local child process. Use [`hermes_local`](./hermes.md) instead.
...
| `apiKey` | yes | — | The Hermes `API_SERVER_KEY`, **not** your `GEETORUS_API_KEY`. Stored as a Geetorus secret reference so it never appears in plain text. |
...
| `geetorusApiUrl` | no | — | Optional Geetorus API URL reachable from the remote Hermes host. This is not a credential; it tells Hermes how to call back into Geetorus if needed. |
...
When Geetorus triggers a run for a `hermes_gateway` agent, the adapter does the following:

1. **Creates a run** via `POST /v1/runs`. The request includes an `Idempotency-Key` equal to the Geetorus run id, which lets you correlate Hermes runs back to Geetorus records.
...
**API key storage.** The `apiKey` field maps to Hermes' `API_SERVER_KEY`. Geetorus stores it as a secret reference and never writes it to plain-text config. If the field shows that a secret is stored, do not repaste the key unless you are intentionally rotating it.
...
|---|---|---|
| `issue` (default) | One session per Geetorus issue | Prevents cross-task memory bleed. Recommended for most teams. |
...
The default `issue` strategy is the safest choice. It keeps each Geetorus issue's work in its own Hermes session so that one task's context does not leak into another.
...
```json
{
  ...
  "adapterConfig": {
    ...
    "geetorusApiUrl": "https://geetorus.internal.example.com",
    "instructions": "You are an engineering assistant. Work carefully and post status updates as you go."
  }
}
```

For a local development setup where Hermes and Geetorus both run on the same machine:
...
Check that the Hermes API server is running and that `apiBaseUrl` is reachable from the Geetorus backend — not just from your browser. If Geetorus is inside Docker and Hermes is on the host, `127.0.0.1` will point to the container, not the host. Use `host.docker.internal` or a LAN address instead.

**The API key field says a secret is stored**

That is expected. Geetorus masks stored adapter secrets. Do not repaste the key unless you are rotating it.
...
A high reconnect rate usually means the SSE stream drops frequently — often a proxy timeout, a load balancer with short keepalive limits, or an unstable network. Try increasing `eventReconnectMs` to reduce churn, and check whether a proxy between Geetorus and Hermes is terminating long-lived connections early.

**Runs complete in Hermes but Geetorus shows no results**

The adapter polls `GET /v1/runs/{run_id}` as a fallback when the SSE stream is lost. If both the stream and the poll fail, verify that `apiBaseUrl` resolves correctly and that the Hermes API server is healthy. Check the Geetorus run log for the last error from the adapter.
...
- [Bring Your Own Agent](../../how-to/bring-your-own-agent.md) — for the external agent invitation flow and the broader model of connecting third-party agents to Geetorus.
- [Agent Adapters](../../guides/org/agent-adapters.md) — how Geetorus adapters work in general.
