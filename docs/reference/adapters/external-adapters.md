...
External adapters let you ship a Geetorus adapter as its own npm package or local directory. Use them when you want independent versioning, separate distribution, or a runtime that should not live inside the Geetorus repo.

---

## When To Use

- You want to distribute an adapter independently from Geetorus.
- You are building an internal adapter for your team or workspace.
- You need a local plugin that can be installed and reloaded without editing Geetorus source.

## When Not To Use

- The adapter should ship with Geetorus itself. Use a built-in adapter.
...
|---|---|---|
| Source | Geetorus repo | Separate npm package or local directory |
| Installation | Ships with Geetorus | Installed through the Adapter Manager or `POST /api/adapters/install` |
| Updates | Requires a Geetorus release | Independent package versioning |
| UI parser | Static import inside Geetorus | Optional `./ui-parser` export |
| Registration | Hardcoded in the host | Loaded at startup from the adapter plugin store |

> **Tip:** External adapters are the right choice whenever you want to own the release lifecycle of the adapter separately from the Geetorus app.
...
```json
{
  "packageName": "@henkey/hermes-geetorus-adapter",
  "version": "latest",
  "isLocalPath": false
}
```
...
1. Geetorus loads the adapter package at startup.
...
2. Add `geetorus.adapterUiParser` if you ship a custom parser.
...
- Keep the package self-contained. The host expects the adapter to load cleanly without modifying Geetorus source.
