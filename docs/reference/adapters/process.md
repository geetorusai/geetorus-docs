...
seo_description: Run an arbitrary command on the Geetorus host. The right choice when your agent is really a script, a shell command, or a custom runtime.
---

# Process

The `process` adapter runs an arbitrary command on the same machine as Geetorus. Use it when your agent is just a script, a shell command, or a custom runtime that already knows how to do its own work.

> **Info:** `process` is a built-in internal adapter used by Geetorus's runtime. It's currently shown as **"Coming soon"** in the agent-config adapter-type dropdown and can't be selected manually. To target it today, configure the agent via the API or an imported company export.
...
- You want Geetorus to inject company context into a process and collect its output.
...
1. Geetorus builds the runtime environment and injects the standard `GEETORUS_*` variables.
2. The adapter resolves the command for logs and execution.
3. Geetorus starts the child process with the configured arguments.
...
- If your script needs to call back into Geetorus, use the injected `GEETORUS_API_URL` and `GEETORUS_API_KEY`.
