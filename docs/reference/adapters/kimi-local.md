...
seo_description: Run Moonshot's Kimi Code CLI on the Geetorus host as a local coding agent, on the shared ACP engine by default with an automatic headless-CLI fallback.
---

# Kimi Code

`kimi_local` runs Moonshot's Kimi Code CLI (`kimi`) on the same machine as Geetorus. Use it when you want a local coding agent that streams a structured live transcript, resumes sessions across heartbeats, and gets your Geetorus skills injected without polluting your own Kimi skills home.
...
- You want Geetorus skills injected into the Kimi skills home without touching the agent workspace.
...
- Kimi Code CLI is not installed or cannot be executed from the Geetorus host.
...
|---|---:|---|
| `cwd` | no | Default absolute working directory for the agent process. Geetorus creates the path when permissions allow; if omitted, it falls back to the current process working directory. |
...
You can also type a model alias that is not in this list; Geetorus passes an unrecognized value straight through to the Kimi CLI.

---

## Thinking Effort

The `effort` field maps a Geetorus effort value onto Kimi's supported thinking-effort tiers. Kimi exposes `low`, `high`, and `max` — there is **no `medium` tier**, so `medium` maps onto `high`.
...
For local runs, Geetorus delivers the desired skills through `--skills-dir` pointing at a per-run managed directory, so skills load reliably without polluting your own `~/.kimi-code/skills` home. Remote runs sync skills into the remote skills home instead.

The Kimi skills home defaults to `~/.kimi-code/skills`, or `$KIMI_CODE_HOME/skills` when you set `KIMI_CODE_HOME`. When you install Geetorus skills through the CLI for manual use, that is the directory they land in.
...
- **`kimi login`** — the OAuth device flow, the same login you use for Kimi Code outside Geetorus.
