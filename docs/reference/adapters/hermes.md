...
seo_description: Run Hermes Agent by Nous Research on the Geetorus host, with persistent memory and a 30-plus tool suite available to the agent on every run.
---

# Hermes

`hermes_local` runs [Hermes Agent](https://github.com/NousResearch/hermes-agent) — a full-featured AI agent by Nous Research — on the same machine as Geetorus. Use it when you want persistent memory, a 30+ tool suite, 80+ loadable skills, multi-provider model routing, and MCP client support in a single adapter.

---

## When To Use

- Hermes is a built-in adapter — no plugin install required. Geetorus ships `hermes_local` out of the box.
...
| `geetorusApiUrl` | no | — | Optional API base override. Defaults to `GEETORUS_API_URL`. |

---

## Model Selection

Hermes manages its own model. You set the model in `~/.hermes/config.yaml` or through Hermes's own configuration — there is no `model` field in the Geetorus adapter config. This keeps the "add a Hermes agent" flow one-click: if Hermes is already configured on your machine, you're done.
...
- **Geetorus-managed skills** — bundled with the adapter, togglable from the board UI.
- **Hermes-native skills** — from `~/.hermes/skills/`, read-only, always loaded.

`listSkills` and `syncSkills` expose the unified snapshot so the Geetorus UI shows both categories in one view.
