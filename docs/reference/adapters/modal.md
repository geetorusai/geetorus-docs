...
`modal` provisions [Modal](https://modal.com/) sandboxes as the execution environment for Geetorus agent runs. Pick it when you want a managed cloud sandbox with first-class container images, configurable network policy, and a 24-hour ceiling on per-sandbox lifetime — without running your own infrastructure.
...
- You want Geetorus's agents to run inside Modal sandboxes that you provision on demand.
- You want a specific container image, working directory, and network policy on every run.
- You have Modal account credentials (`tokenId` / `tokenSecret`) you can paste into Geetorus as secrets.

## When Not To Use

- You want local execution on the Geetorus host. Use a local adapter instead.
...
- You can't get to Modal's API from where Geetorus runs.
...
Modal's official JS SDK supports **Node 22 or later**. Geetorus's baseline is now `node >= 24.11.0`, comfortably above that floor, so the plugin runs on a supported Node by default and the old best-effort caveat no longer applies.
...
A note on `tokenId` / `tokenSecret`: the plugin worker runs in a child process that doesn't inherit host env vars, so `MODAL_TOKEN_ID` / `MODAL_TOKEN_SECRET` set on the Geetorus server are **not** read by the plugin. Always provide the tokens through the environment config — Geetorus stores pasted values as company secrets.
...
- **`reuseLease: true`.** On release the plugin detaches the sandbox. It keeps running on Modal until `sandboxTimeoutMs` or `idleTimeoutMs` elapses, then the next acquire reconnects to it by id. If the sandbox has already expired, Geetorus notices and provisions a new one.
