...
`openclaw_gateway` connects Geetorus to an OpenClaw instance over the OpenClaw Gateway WebSocket protocol. Use it when OpenClaw is running on another machine, inside Docker, or behind a shared gateway and you want Geetorus agents to delegate work to it.
...
- You want a shared OpenClaw instance to serve multiple Geetorus agents.
- You need device-auth pairing between Geetorus and the gateway.
...
5. Stream `event agent` frames into Geetorus's logs and transcript parser.
...
| `geetorusApiUrl` | no | Optional override for the Geetorus API base URL the gateway/agent calls back into. No default. |
...
- `idempotencyKey` — Geetorus's `runId`.
...
Geetorus ships several smoke harnesses that spin up OpenClaw in Docker and wire it to Geetorus for you.
...
Clones or updates `openclaw/openclaw` in `/tmp/openclaw-docker`, builds `openclaw:local`, writes isolated smoke config under `~/.openclaw-geetorus-smoke/`, starts `openclaw-gateway` via Compose, and prints `http://127.0.0.1:18789/#token=...`.
...
| `GEETORUS_HOST_FROM_CONTAINER` | `host.docker.internal` | How the container reaches Geetorus |

### Network tips

Inside OpenClaw Docker, `127.0.0.1` points to the container — not the host. For URLs consumed inside Docker, use `host.docker.internal`. If Geetorus rejects that hostname, allow it:
...
When you add OpenClaw to a Geetorus company:

1. Start Geetorus in auth mode and a clean OpenClaw Docker.
2. In the company settings, use **Generate OpenClaw Invite Prompt** and paste it into OpenClaw's main chat.
3. Approve the resulting join request in Geetorus and confirm the agent appears in your company.
