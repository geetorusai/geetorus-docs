---
seo_title: Geetorus Connectors
seo_description: Every service Geetorus can connect, grouped by what it is for, with the access model, action permissions, and a setup guide behind each one.
---

# Connectors

A **connector** is a saved connection to an outside service: a Gmail mailbox, a GitHub organization, a Notion workspace, a PostHog project. You set one up once, decide who and what it is for, and Geetorus carries that decision into every run.

> **Note:** These guides cover the connector model in Geetorus **v2026.916.0**. Available methods depend on your installed version, instance configuration, and provider approval. A catalog listing does not by itself establish that provider sign-in is available.

> **Warning:** **Google verification pending.** Geetorus has not yet completed Google app verification. You may see an unverified-app warning during authorization. If Google offers an **Advanced** option to continue to Geetorus, you can choose to proceed after reviewing the requested access. This option is not available for every account; Workspace administrator restrictions and other Google access requirements still apply. Contact [support@geetorus.ing](mailto:support@geetorus.ing) if you cannot connect.
...
The **Connectors** list holds three kinds of connection. They look alike — each one is a credential Geetorus stores for you — but you set them up differently and different people use them.
...
**Model providers** supply the credential Geetorus uses to run models. Setup ends in a stored key or subscription, and there is no action list, because nothing here is a tool an agent can call. Geetorus uses it on the agent's behalf whenever the agent thinks. If you have ever wondered why your Anthropic connection has no permissions to set, this is why.

**Chat channels — Experimental** let people talk to your agents from a messaging app. An administrator must enable **Chat connectors**, which is off by default. Setup connects a provider app or bot to Geetorus; eligible messages can start or continue agent tasks.
...
| --- | --- |
| Connect with Geetorus | Geetorus holds the OAuth client and you approve access in the provider's own consent screen. Offered only on instances enrolled with Geetorus Cloud. |
| Sign in with the provider | Geetorus registers itself with the provider's authorization server at connect time, then you approve access there. Nothing to create first. |
| Your own OAuth app | You register an OAuth client with the provider and add Geetorus's callback URI before connecting. Use this when the provider does not support automatic registration, or when you want the client to be yours. |
| API key | You generate a key or token in the provider's console and paste it into Geetorus. |
...
| --- | --- | --- |
| [Gmail](connectors/gmail.md) | Google's email service. Agents search and read mail, and on a draft connection create drafts for you to review in Gmail. Sending, deleting, and relabelling mail are never enabled. | Connect with Geetorus · Your own OAuth app |
| [Google Calendar](connectors/google-calendar.md) | Google's calendar. Agents read calendars, and on a write connection create and change events. | Connect with Geetorus · Your own OAuth app |
| [Google Chat](connectors/google-chat.md) | Google Workspace's team messaging. Agents search and read conversations, and on a write connection post messages. This is an agent tool, not a way for people to talk to an agent. | Connect with Geetorus · Your own OAuth app |
| [Google Docs](connectors/google-docs.md) | Google's word processor. Agents read documents, and on a write connection edit them. | Connect with Geetorus · Your own OAuth app |
| [Google Drive](connectors/google-drive.md) | Google's file storage. Agents search and read files, and on a write connection create and copy them. | Connect with Geetorus · Your own OAuth app |
| [Google People](connectors/google-people.md) | The contacts and directory behind a Google account. Agents look up people and profiles. Read-only; there is no write connection. | Connect with Geetorus · Your own OAuth app |
| [Google Sheets](connectors/google-sheets.md) | Google's spreadsheets. Agents read sheets, and on a write connection update them. A third path shares individual spreadsheets with a Geetorus robot account instead of connecting a Google identity at all. | Connect with Geetorus · Your own OAuth app · No credential |
| [Google Slides](connectors/google-slides.md) | Google's presentations. Agents read decks, and on a write connection edit them. | Connect with Geetorus · Your own OAuth app |
| [Google Workspace Search](connectors/google-workspace-search.md) | One read-only search that spans Gmail, Drive, Calendar, and Chat in a single Google account. Use it when an agent needs to find something without knowing which app holds it. | Connect with Geetorus · Your own OAuth app |
...
| [Notion](connectors/notion.md) | Workspace for notes, documents, and databases. Agents read and update content accessible to the Notion account you authorize. Geetorus does not add a page-level filter. | Sign in with Notion |
...
| Connector | What you can do | Connection methods |
| --- | --- | --- |
| [AgentMail](connectors/agentmail.md) — **Experimental** | Email inboxes built for software agents. Gives an agent its own inbox and turns each email conversation into a Geetorus task. Requires the **Chat connectors** experimental setting. | API key |
...
These hold the credential Geetorus uses to run models. They publish no actions and appear in no permission list. See [Agent adapters](guides/org/agent-adapters.md) for which adapter uses which provider.
...
These give people a place to talk to an agent. They require an administrator to enable **Chat connectors** in the instance's experimental settings; the setting is off by default. Setup registers an app or bot with the provider and points it at a Geetorus webhook; one connection carries one agent.

| Connector | What you can do | Connection methods |
| --- | --- | --- |
| [Discord](connectors/discord.md) | Mention the agent in a server channel and Geetorus opens a thread, keeping it tied to one task. | Provider app registration |
...
**Check your own instance first.** Your installed version may offer a different catalog, and the methods it offers you depend on whether the instance is enrolled with Geetorus Cloud and whether chat channels are switched on. What **Connectors** shows you is authoritative; this page is not.
...
**If the provider gates access.** A few providers review and approve MCP clients before an independently registered one will work. Nothing you do in Geetorus changes that, and the approval has to come from the provider. [Providers Geetorus recognizes but does not list](connectors/recognized-providers.md) names the ones in this snapshot and what each is waiting on.
...
**Most action lists come from the provider, not from Geetorus.** For a connector that talks to a provider-hosted server, the provider publishes the tools. Geetorus reads that list, classifies each entry as read, write, or destructive, and lets you set each one to **Allowed**, **Ask first**, or **Off**. When the provider changes its server, your list changes with it — so the authoritative list of what a connector can do is the one in your own instance, not the one on its page here.
...
- [Providers Geetorus recognizes but does not list](connectors/recognized-providers.md) — the withheld and provider-gated inventory in this snapshot.
