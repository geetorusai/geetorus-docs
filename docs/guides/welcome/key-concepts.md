...
seo_description: The seven ideas behind Geetorus — company, agent, task, heartbeat, adapter, budget, approval — explained with analogies before the technical detail.
---

# Key Concepts

Before you start clicking around, it's worth spending five minutes here. Geetorus introduces a handful of ideas that don't have perfect equivalents in other software — and if you misunderstand them early, the product will seem confusing in ways that are hard to untangle later.

This guide explains seven concepts that underpin everything in Geetorus. Analogies come first; technical detail follows.
...
**The analogy:** Think of a Company the way you'd think of a Notion workspace — everything related to a project lives inside it. The difference is that a Geetorus Company isn't just a place to store information. It's an active organisation that plans and executes work autonomously.

**Why it exists:** Geetorus can run multiple, separate AI organisations in parallel. The Company boundary keeps their goals, agents, tasks, and budgets isolated from each other — so you could run a "Content Marketing" company and a "Software Development" company side-by-side without them interfering.
...
Every single thing in Geetorus — every agent, every task, every dollar spent — belongs to a Company.
...
**Why it exists:** Without agents, Geetorus has no one to do the work. The agent layer is what turns a goal into action. Each agent has a clearly scoped role so that multiple agents can collaborate without confusion about who's responsible for what.
...
**In one sentence:** An Adapter is the connection between Geetorus and the AI system that actually runs your agent.
...
**Why it exists:** Geetorus itself is not the AI — it's the orchestration layer above the AI. The adapter system lets Geetorus work with any AI runtime without being locked into one provider. You can run some agents on Claude and others on Codex, or connect entirely custom systems via the HTTP adapter.
...
**In one sentence:** A Budget is a monthly spending cap — a dollar limit that Geetorus enforces automatically so your agents never overspend.
...
Budget behaviour:
- At **80% utilisation**: Geetorus shows a warning so you can intervene before the hard stop
...
**In one sentence:** An Approval is a governance gate — a checkpoint where Geetorus pauses and asks you to sign off before an important action proceeds.

**The analogy:** Think of it like a purchase order process at a company. Below a certain threshold, employees can spend freely. Above it, they need a manager's sign-off before the purchase goes through. Approvals are Geetorus's equivalent of that sign-off process — but for AI agents.
...
Geetorus can also surface budget override approvals when a hard stop is hit, and other board-level approval requests in more advanced flows.
...
You now have the mental model for everything Geetorus does. When something in the UI seems unfamiliar, come back here.
