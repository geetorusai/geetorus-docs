...
When you hire an agent, you're telling Geetorus: which AI system should run this agent, what role does it play in the company, and what constraints does it operate within. The AI itself (Claude, Codex, etc.) lives outside Geetorus. Geetorus is the management layer above it.
...
2. **Set the agent's name**
   ...
   Give the agent a name (e.g. "CEO"). If this is your first agent, Geetorus automatically makes it the **CEO**.
...
3. **Choose an adapter**
   ...
   An adapter tells Geetorus how to run your agent. Click the **Adapter Type** dropdown to see your options.
   ...
   **Claude Code** runs a Claude Code agent directly on your Mac. The agent has full access to your filesystem in its working directory, can run terminal commands, write and edit files, and call the Claude API on your behalf.
   ...
   This is the most capable and most commonly used adapter for Geetorus agents.
   ...
   **Configuration fields:**
   ...
   - **Working directory** — The folder on your Mac where the agent will do its work. This is where files get created, edited, and read. If you're not sure what to use, create a folder called `geetorus-workspace` on your Desktop and paste that path here (e.g. `/Users/yourname/Desktop/geetorus-workspace`).
   ...
   - **Environment variables** — Add `ANTHROPIC_API_KEY` and either paste the key as a plain value or store it as a Geetorus secret. This is how the adapter gets access to Claude.

   - **Test environment** — Use this button to confirm Geetorus can see Claude Code and that your `ANTHROPIC_API_KEY` binding works before you create the agent.

   > **Tip:** If you're unsure about the working directory, create a new folder called `geetorus-workspace` on your Desktop. Use that path until you decide on a better home for your agents' work.
   ...
   **Configuration fields:**
   ...
   - **Working directory** — The folder on your Mac where the agent will do its work. Create a folder called `geetorus-workspace` on your Desktop if you don't have a preferred location.

   - **Model** — Which OpenAI model to use. `gpt-5.3-codex` is the default and a sensible starting point for a CEO agent.

   - **Environment variables** — Add `OPENAI_API_KEY` and either paste the key as a plain value or store it as a Geetorus secret.
...
5. **Create the agent**
   ...
   Click **Create agent**. Geetorus creates the agent and takes you to the agent detail page. You should see the agent with a status of **idle** — meaning it's configured and ready, but hasn't fired a heartbeat yet.
...
7. **Test the environment again if needed**
   ...
   Geetorus will attempt to connect to the adapter — in the case of `claude_local`, it will check that Claude Code is installed and accessible, and that the environment variable binding is valid.
...
- Claude Code isn't installed, or isn't at the path Geetorus expects — confirm you can open Claude Code from your Mac independently of Geetorus.
