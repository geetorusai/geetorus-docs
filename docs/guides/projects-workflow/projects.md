...
A project is the container Geetorus uses to group related work. If goals answer "why are we doing this?" and issues answer "what exactly needs doing?", projects sit in between: they bind a body of work to a concrete place — a repository, a working directory, a budget envelope, a set of execution workspaces — where agents can actually pick up tasks and make progress.

Every issue in Geetorus lives under a project. Every execution workspace is scoped to a project. Budgets can be pinned to a project. When you tell an agent "work on the marketing site", you are really telling it "work on issues in the Marketing Site project, using the code and configuration the project points at". Projects are how the abstract strategy the CEO writes becomes something concrete an agent can check out and run.
...
- **Project name** — the main title. Clicking anywhere on the row opens the project detail page and defaults to the Issues tab (unless you previously visited the project, in which case Geetorus restores the tab you last used).
...
Progress on a project is implicit: Geetorus does not maintain a single "percent complete" field. Instead, progress is the shape of the Issues tab — how many issues are `todo`, `in_progress`, `in_review`, `done`, or `cancelled`. If you want a rolled-up view, read the Issues tab with the status grouping turned on.
...
The Workspaces tab shows the execution workspaces currently attached to the project. It only appears when the company has the **isolated workspaces** experimental setting enabled and the project has at least one non-default workspace with activity. On projects without isolated workspaces, Geetorus redirects you to the Issues tab instead.
...
- **Local folder only** — for projects where the code lives on the machine Geetorus is running on. Agents work directly in this folder (or in isolated worktrees derived from it).
- **Remote repository only** — for projects where Geetorus clones or references a remote repository. Useful when no local checkout exists yet.
...
- **Branch template** — the naming convention Geetorus uses when it creates new branches for isolated workspaces (e.g. a template that includes the issue identifier).
- **Worktree parent directory** — where Geetorus places new git worktrees on disk.
...
- **Metric** — `billed_cents`. Geetorus tracks billable cost in cents and converts to a currency figure for display.
