...
When a task runs, Geetorus resolves which execution workspace to use for that run:

1. **The heartbeat fires** and the agent picks up a task
2. **Geetorus resolves the workspace** — creating a new one, reusing an existing one, or sticking with the project default, depending on your settings
...
Geetorus creates a fresh working copy for this task — a new git worktree at a new path, branched from the base. The agent works here without any risk of interfering with other workspaces. When the task is done, you can review and merge the branch, then archive the workspace.
...
> **Note:** Execution workspaces are local working copies. Geetorus does not push them to a remote git host on your behalf — finished work is reviewed and merged through Geetorus, not through a `git push` from inside the workspace.
...
Runtime services are manually controlled from the workspace UI. Geetorus does not start or stop them automatically when a heartbeat fires.
...
- **Provisioning HTTPS…** — the service is up and Geetorus is bringing the HTTPS mapping online.
...
Because HTTPS is fail-closed, Geetorus never falls back to plain HTTP and calls it ready — if the probe doesn't pass, the preview stays unavailable rather than silently downgrading.
...
The card shows a badge, a title, and a single primary action. The primary action is either a button you can click now or a disabled wait state while Geetorus finishes something:
...
3. Geetorus cleans up the workspace according to its mode and project settings
...
Either way, you'll see the project's primary workspace and any isolated execution workspaces Geetorus has provisioned. If nothing has been provisioned yet, the company-wide view shows **"No workspace activity yet."**
...
Installed plugins can add their own tabs alongside these (for example, a workspace-diff plugin contributes a **Changes** tab). The tabs remember your last selection per workspace, so if you always land on Runtime logs for a specific workspace, Geetorus keeps taking you there.
...
1. **You need a working directory.** Every command Geetorus runs against the workspace needs an absolute path to run in. If the workspace's `cwd` is empty, the action buttons are disabled and a hint reminds you to fix that first. Fill in the **Working directory** field on the Configuration tab, save, and the controls light up again.
...
The **Workspace settings** card holds the editable fields for the workspace. Most of these are populated when Geetorus provisions the workspace; you usually only need to touch them when an agent or project has moved, a branch was renamed, or the provisioning scripts changed.
...
- **Working directory** — the absolute path on disk. This is where the agent runs its commands. If this path is empty, Geetorus can't run any local commands for this workspace — it's the single most important field to keep correct.
...
- **Provision command** — runs when Geetorus prepares this execution workspace (for example, a script that clones a worktree and installs dependencies).
...
Changes in the Workspace settings card stay local until you click **Save changes**. The **Reset** button reverts the form to whatever Geetorus has currently persisted, so you can back out of a half-edited change without reloading the page.

### Workspace context

The **Workspace context** card (labelled "Linked objects" in the UI) shows the other Geetorus entities this execution workspace is connected to:
...
- **Derived from** — if this workspace was created by splitting or branching off another execution workspace, you'll see a link back to the original. This is how Geetorus represents follow-up worktrees that share a code lineage with an earlier one.
...
- **Opened** — timestamp when Geetorus first opened the workspace.
- **Last used** — the most recent time an agent interacted with it.
- **Cleanup** — if Geetorus has scheduled this workspace for cleanup, the eligible-at time and the reason are shown here. Otherwise this reads `Not scheduled`.
...
Runtime configuration is one of the places where project defaults and workspace-specific needs sometimes pull in different directions. Geetorus resolves this with a two-level inheritance model:
...
- **None** — neither level has runtime config defined, so Geetorus can't start services. Jobs may still run if they're provided, but services won't appear in the Services tab.
...
Geetorus validates the JSON when you hit **Save changes**; if it can't be parsed or isn't a JSON object, you'll get an inline error and the save is aborted.

---

## Runtime logs tab

Switch to the Runtime logs tab to see what Geetorus has actually done with this workspace — which commands ran, when they ran, how they exited, and what they printed.
...
- **Cleanup operations** — provision, teardown, and cleanup command runs that happen when Geetorus prepares or closes the workspace.
...
- **Provision** operations are emitted automatically when Geetorus first prepares the workspace; you won't trigger these from the UI, but you'll see them here when debugging first-boot problems.
...
- The view's local state — which columns are visible, how you've sorted — is remembered under the `geetorus:execution-workspace-issues-view` key, so different execution workspaces can have different preferred layouts.
...
- The **source issue** is the single issue that originally caused Geetorus to provision this workspace. It's stored on the workspace itself and appears in the Workspace context card. For isolated worktrees created from a specific task, this will typically be the task that kicked the whole flow off.
...
Closing a workspace is a two-step operation in Geetorus:
...
The **Cleanup** row on the Concrete location card tells you whether Geetorus has already scheduled the workspace for automatic cleanup — for example, because it's been idle past a retention window — and what the reason is. If you see a scheduled cleanup you didn't expect, look at the project's workspace lifecycle settings to understand why.
...
- **Is the branch still there?** If someone deleted the branch outside Geetorus, the agent will fail when it tries to check out. Re-create or re-point the branch.
...
- **Is the agent looking at the right workspace?** When follow-up tasks are created, make sure either `parentId` or `inheritExecutionWorkspaceFromIssueId` is set on the child issue so Geetorus can resolve it back to this workspace.
...
Geetorus's default for "isolated" execution workspaces is a **git worktree** branched off the project workspace's base ref. If you haven't used git worktrees before, here's what that means in practice:
...
When Geetorus provisions an isolated workspace:
...
2. Let Geetorus provision an isolated workspace when the agent checks out the task.
...
2. Geetorus will wire the new issue up to the existing workspace on checkout.
