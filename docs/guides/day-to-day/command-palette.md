...
When you want to get somewhere fast — jump to a project, open a task, or start a new agent — you don't have to hunt through the sidebar. Press **⌘K** (or **Ctrl+K** on Windows and Linux) from anywhere in Geetorus and the command palette opens: a single search box that lets you navigate, search, and run common actions from the keyboard.
...
The palette isn't the only quick way back to a task. Whenever you open a task's detail page, Geetorus remembers it under a **Recent Tasks** heading near the bottom of the sidebar — so the handful of tasks you've been looking at stay one click away, no searching or scrolling the full Tasks list required.
