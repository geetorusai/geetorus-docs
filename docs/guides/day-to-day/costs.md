...
seo_description: Set limits Geetorus enforces automatically. Five tabs cover overview, budgets, providers, billers, and finance, with headline spend metrics on top.
---

# Costs

> **Warning:** AI agents make real API calls that cost real money. Every time an agent works — every heartbeat, every task, every comment it reads and writes — it sends and receives text through a provider like Anthropic or OpenAI, which charges you per token (roughly per word). This isn't a Geetorus fee; it's a cost you pay directly to the AI provider. Read this guide before your agents start running in earnest.

Geetorus's budget system exists to make sure you're never surprised. You set limits, and the platform enforces them automatically — an agent won't spend a cent beyond what you allow.
...
Geetorus records every one of these calls: which agent made it, which model it used, how many tokens were used, and the exact dollar cost. These records are aggregated per agent per calendar month (resetting on the 1st of each UTC month).
...
If you multiply an agent's token count by the provider's list price, you'll usually land somewhere above what Geetorus reports. That gap is normally prompt caching doing its job, not an error in the arithmetic.
...
- **Estimated** — debits that are not yet invoice-authoritative (for example, a Geetorus estimate of a subscription day that hasn't been reconciled with the provider's invoice yet).
...
To the right of the agent breakdown is the **By project** card. Geetorus attributes run costs to a project when the run was triggered by an issue that belongs to that project. Each row shows the project name and its total attributed cost. Runs that didn't happen inside a project-linked issue appear as `Unattributed`.
...
The **Budgets** tab is where you configure the spend ceilings that protect your company. Think of it as the control plane for hard stops — the place where you decide, in dollars, how much any given scope is allowed to spend before Geetorus pulls the emergency brake.

### Budget Levels

Geetorus operates two levels of budget protection that work together:
...
All budgets are displayed in dollars in the Geetorus UI.
...
Geetorus enforces budgets automatically, in two stages:
...
**At 80%:** Geetorus records a warning so you can intervene before the hard stop. The agent continues running.
...
For providers that have a subscription plan with a rolling usage window (e.g. Anthropic Pro/Max with a 5-hour window, daily, or weekly caps), the card also renders a **Quota windows** section. For each window Geetorus queries the provider and shows:
...
Underneath the quota bars, Geetorus also shows **window spend** — what the provider cost you inside each active quota window. This is how you tell the difference between "I'm at 80% of my 5-hour cap because of one burst" and "I'm at 80% and it's evenly distributed".
...
- **Estimated** — debits that Geetorus has estimated but are not yet invoice-authoritative. These are the events most likely to change as invoices arrive.
...
Geetorus separates estimated debits from authoritative ones because your provider's invoice is the source of truth. Until an invoice is reconciled, Geetorus's number for a subscription day is its best guess — typically derived from the plan price divided by the billing cycle length. Once the real invoice arrives and is imported, the estimated flag flips off and the number becomes authoritative.
...
Check this panel whenever you open Geetorus. A bar that has jumped unexpectedly since your last check is worth investigating.
