...
Every time an agent posts a comment or revises a document, you can rate the result with **Helpful** (thumbs up) or **Needs work** (thumbs down). These votes are the fastest way to tell an agent — and the humans maintaining Geetorus — when something worked well and when it didn't.
...
pnpm geetorusai feedback report --api-base http://127.0.0.1:3000 --company-id <company-id>
...
feedback-export-20260331T120000Z/
  ...
  traces/
    PAP-123-e5f6g7h8.json       # Geetorus feedback envelope
  full-traces/
    PAP-123-e5f6g7h8/
...
Exports are full by default. `traces/` contains the Geetorus envelope; `full-traces/` holds the richer per-trace bundle plus any recoverable adapter-native files.

```sh
# Custom server and output directory
pnpm geetorusai feedback export --api-base http://127.0.0.1:3000 --company-id <company-id> --out ./my-export
```
...
- **Default endpoint:** when no backend URL is configured, Geetorus falls back to `https://telemetry.geetorus.ing`.
