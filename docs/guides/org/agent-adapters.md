...
When you create an agent in Geetorus, one of the first things you configure is its adapter. The adapter is the bridge between Geetorus and the AI system that actually runs your agent — it tells Geetorus how to launch the agent, how to pass it work, and how to receive results back.

Think of it like a power adapter for different countries. The electricity (Geetorus's task system and org chart) is the same everywhere, but the plug you need depends on the socket (the AI runtime you're connecting to). Claude Code needs one configuration, OpenAI Codex needs another, and a custom cloud-based agent needs a third.
...
The `claude_local` adapter runs your agent using Claude Code — Anthropic's command-line AI tool — directly on your Mac. When Geetorus triggers a heartbeat, it launches Claude Code with the agent's context and task, waits for the run to complete, and reads back what Claude did.
...
**Working directory**
The folder on your Mac where the agent does its work — reads files, writes outputs, runs commands. If you're using this agent for a software project, point this to your project folder. If you're not sure, create a new folder called `geetorus-workspace` on your Desktop and use that.
...
**Timeout (seconds)**
How long a single heartbeat run is allowed to take before Geetorus cuts it off. 300 seconds (5 minutes) is a safe default for most tasks. Complex coding tasks may need longer — you can increase it to 600 or more. Setting it too low will cause agents to time out mid-task.
...
**"Claude Code not found"** — Claude Code isn't installed, or isn't on the system PATH. Install it from [claude.ai/code](https://claude.ai/code) and then use the Test Environment button to verify Geetorus can find it.
...
The `http` adapter connects to a web server or cloud function that you control. When a heartbeat fires, Geetorus sends an HTTP request to your endpoint with the agent's context and tasks. Your endpoint processes the work and sends results back.
...
**Endpoint URL**
The full URL Geetorus will POST to when a heartbeat fires. This must be publicly accessible or reachable from your Geetorus instance.

**Authentication**
A secret token that Geetorus includes in the request header, so your server can verify the call came from Geetorus and not someone else.

**Timeout**
How long Geetorus waits for a response before treating the heartbeat as failed.
...
The `process` adapter runs a command on the same machine as Geetorus. When a heartbeat fires, Geetorus executes the command you specify, passing the agent's context as stdin or environment variables.
...
**Command**
The executable command and any arguments. The command must be accessible from the machine running Geetorus.
