...
- **Bundled** teams ship inside the app and are the everyday starting points. They lean on skills Geetorus already knows about, so installing one is self-contained. Examples shipping today: a **core executive team** (a CEO, CTO, and QA with a starter project and a recurring CEO heartbeat task), a **product-engineering** pod (CTO, Senior Coder, QA, and a weekly engineering sync), and a **product-design** team (a UX designer with the `wireframe` and `design-critique` skills and a weekly design review).
...
- **Tasks** — including any recurring routines the team ships, such as a CEO heartbeat or a weekly sync, imported the same careful way Geetorus imports any routine.
...
Because every catalog team carries a version, Geetorus knows which version you installed. The catalog flags a team as out of date when a newer version has shipped, so you can reinstall the updated team when you're ready.
...
```sh
# Browse and inspect
geetorusai teams browse --kind bundled
