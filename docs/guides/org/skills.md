...
Gists work through the same source field. Paste the gist URL and Geetorus treats it like a remote file source — useful when you want to share a one-off procedure without setting up a full repository.

### 3. Import from a local file or folder

You can also paste a local path (Linux, WSL, or Windows) into the source field. Geetorus walks the folder, finds any `SKILL.md` files, and imports them. This is the right choice when you already keep skills in a project workspace on disk — the scan button (circular arrow next to the plus icon) re-runs the local import across every project workspace in the company so newly added `SKILL.md` folders show up automatically.

### 4. Import from the skills.sh marketplace

[skills.sh](https://skills.sh) is a community catalog of ready-made skills. Each listing provides a copy-ready install command; paste it into the source field and Geetorus imports the skill along with its source metadata (you will see a small skills.sh badge on the skill row).
...
- **Optional shortname / slug** — the `kebab-case` key the agent uses to reference this skill. If you leave this blank, Geetorus derives it from the name.
- **Short description** — the routing logic ("Use when… Don't use when…") the agent reads before loading the skill body.

Click **Create skill** and the skill is added to the library as a Geetorus-managed, editable entry.
...
You don't have to write every skill from scratch. Geetorus ships with a built-in **catalog** of ready-made skills you can browse and install with a click — a head start for common procedures.
...
Because these skills come from Geetorus, the app knows exactly which version you installed. That means a catalog skill can be **kept up to date**: Geetorus can check whether a newer version shipped and install the update, audit the skill's contents for safety before you trust it, and reset the skill back to its original shipped version if it was edited locally. For the mechanics of installing, updating, auditing, and resetting catalog skills, see the [Skills reference](../../reference/skills.md#3-app-shipped-catalog).
...
Under **Advanced** you can choose the **run template**, which is the wrapper Geetorus puts around your input. **Default test template** is the sensible starting point; **No template** *("Run only the input text.")* hands the agent your raw input and nothing else. You can also save your own templates alongside the built-in ones, and edit, duplicate, or delete them. Your choice is remembered per company.
...
Renaming changes three things at once, because in Geetorus they travel together:
...
- The **slug** — the `kebab-case` shortname used in URLs. Give it explicitly if you want to, or leave it out and Geetorus derives it from the new name.
- The **key** — the stable identifier agents reference. It follows the slug, so a new slug means a new key.

That last one is where renaming would normally hurt, and it's the part Geetorus handles for you. **Every agent that had the old skill attached is moved to the new key automatically**, keeping any pinned skill version as it was. Nothing silently detaches, and nobody has to go around the Agents list re-ticking checkboxes.

A few things worth knowing before you rename:

- **Only Geetorus-managed skills can be renamed.** Skills from the built-in catalog, GitHub, skills.sh, or a project scan are read-only for the same reason their contents are — see [Editing a skill that isn't yours to edit](#editing-a-skill-that-isnt-yours-to-edit) if you want an editable copy you *can* rename.
...
**Edit a copy** forks the skill into an editable, Geetorus-managed copy in your library. Before it does anything it runs a precheck and tells you plainly how many agents currently use the original — and offers a switch, on by default, to move those agents over to your copy. That's the decision that actually matters: fork without moving the agents and you've made a copy nobody runs; fork with the switch on and your edits take effect on the next run. The original is left untouched either way.
...
- **Editable** — a skill is editable only when Geetorus controls its source. Skills imported from GitHub, gists, or the skills.sh catalog are **read-only**: you can view any file but cannot save changes; the detail pane shows an explanation badge instead of an Edit button. Geetorus-managed and local-folder skills open in the Markdown editor when you click **Edit**.
...
- The **Source** badge (skills.sh, GitHub, local folder, or Geetorus) with the source label — clicking the label copies the path to the workspace when applicable.
- The **Pin** for GitHub-sourced skills — the short commit SHA Geetorus is locked to, plus a **Check for updates** button and, when an update exists, an **Install update** action.
...
- **Tracked only** (`unsupported`) — Geetorus cannot push skills into the adapter, so it only records which skills you have assigned. Adapters like `openclaw_gateway` fall into this category; you will see a banner directing you to manage skills inside the remote runtime (for example, OpenClaw) rather than from Geetorus.

Assignments still save regardless of mode — the mode only controls whether Geetorus physically syncs the skill files. Warnings surface in an amber banner when, for example, a desired skill is missing from the company library or the adapter rejected a sync.
...
3. You will see three groups:
   ...
   - **Unmanaged skills** — read-only rows for skills the adapter picked up from somewhere Geetorus does not control (for example, a global skill bundle on the host). These are shown for visibility only.
4. Changes autosave about 250 ms after you stop clicking — the small "Saving soon..." / "Saving changes..." indicator in the top-right confirms when Geetorus has persisted the update. The agent will pick up the new skill list on its next run.
...
## Pinning a beta release of the Geetorus skill

The bundled `geetorus` skill — the base procedure every agent follows on each run — keeps moving. Most of the time that is exactly what you want: better guidance ships and every agent picks it up. But sometimes you want one agent held still, either because you are comparing how two versions of the procedure behave or because a particular snapshot suits a run you would rather not disturb while you evaluate a newer one.

**Beta releases** let you do that. A release is a frozen snapshot of the Geetorus core skill, seeded into your company library alongside the live version. Pin an agent to one and that agent runs the snapshot; every other agent keeps running the live default.

### Turn it on first

The picker stays hidden until someone enables it. Open **Settings → Instance settings → Experimental** and flip the **Beta skills** card: *"Allow agents to pin beta releases of the Geetorus core skill. Disabling this returns every agent to the default live skill without removing saved pins."*
...
Open the agent, go to the **Skills** tab, and find the Geetorus core skill row — it needs to be enabled for that agent. A **Skill release** dropdown appears alongside it:
...
The same rule holds over the API: saving a version pin while the flag is off is rejected with *"Beta skill version pins require the Beta skills experimental setting to be enabled."* For the request shape, the release fields, and where the snapshots come from, see [Skills reference → Beta releases of the bundled `geetorus` skill](../../reference/skills.md#beta-releases-of-the-bundled-geetorus-skill).

---

## Trust level

Skills are loaded into agent runs as additional instructions, so where they come from matters. Geetorus uses the **Source** badge on every skill row to make that provenance obvious:

- **Geetorus-managed** — authored inside Geetorus. You have full control of the contents; these are the safest skills to give an agent.
...
You can also scan and import skills from project workspaces when you already have skill files on disk. That's useful for teams migrating existing `SKILL.md` folders into the Geetorus library — see [Import skills straight from a project](#5-import-skills-straight-from-a-project).
