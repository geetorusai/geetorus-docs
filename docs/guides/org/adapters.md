...
Every agent in Geetorus needs an **adapter** — the shim that lets the control plane talk to a specific AI runtime. Built-in adapters ship with the product (Claude Code, Codex, Gemini, OpenCode, Cursor, Pi, Hermes, HTTP, Process, OpenClaw Gateway), and you can extend the set by installing **external adapters** from npm or a local path.
...
- **Built-in Adapters** — adapters that ship with Geetorus. These cannot be removed, but they can be hidden from the agent dropdown.
...
| Badge | Meaning |
|---|---|
| `Built-in` | Ships with Geetorus. Cannot be removed. |
...
2. Enter the package name — for example, `@my-org/geetorus-adapter-openrouter`.
3. Optionally enter a version; leave it blank for `latest`.
4. Click **Install**.

Geetorus fetches the package, validates that it exports a `createServerAdapter()` factory, registers the adapter type, and adds a row to **External Adapters**. A toast confirms the install (`Adapter installed — Type "..." registered successfully. (v...)`). On failure the dialog stays open and shows the error.
...
> **Tip:** The adapter package must export a `createServerAdapter()` function that conforms to Geetorus's adapter contract. See [Creating an Adapter](../../reference/adapters/creating-an-adapter.md).
...
- **Package** — `@my-org/geetorus-adapter-x`
- **Current** — `v0.3.2`
