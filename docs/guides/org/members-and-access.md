...
- **Human roles** gate access to the Geetorus *product* — who can open the app, invite people, approve spend, change settings.
...
It's a useful lens when you want to see what a teammate has actually been doing, or attribute cost to a person. You can pull the same data from the CLI with `geetorusai profile company-user <user-slug> --company-id <company-id>` (see the [CLI reference](../../reference/cli/access.md#profile)).
...
One last distinction, because the word *membership* is overloaded. Alongside company membership, Geetorus has **resource memberships** — a lightweight "joined / left" flag on individual projects and agents.
