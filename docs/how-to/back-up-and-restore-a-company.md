...
The CLI shortcut (`geetorusai company export | import`) is fine for one-off work — see [Export & Import](../guides/power/export-import.md). The HTTP routes below are what you'll wire into a routine, a CI job, or any agent that needs durable backups.

---

## What a package contains

A bundle is a tree of markdown files plus a `.geetorus.yaml` sidecar and a content-addressed blob store:
...
```bash
curl "$GEETORUS_API_URL/api/companies/$COMPANY_ID/export/fidelity" \
...
```bash
curl -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/exports/preview" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
...
```bash
curl -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/exports" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
curl -X POST "$GEETORUS_API_URL/api/companies/import/preview" \
...
```bash
curl -X POST "$GEETORUS_API_URL/api/companies/import" \
  -H "Authorization: Bearer $BOARD_TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "$GEETORUS_API_URL/api/companies/import?async=1" \
  -H "Authorization: Bearer $BOARD_TOKEN" \
...
```bash
curl -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/imports/preview" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/imports/apply" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  --data-binary @import-request.json
```
...
Moving a company to a different host — laptop to server, self-hosted to Geetorus Cloud, or back again — is the same two steps with the second one pointed somewhere else:
...
This replaces the retired host-to-host Cloud Sync. If you have scripts pointed at `cloud connect` or `cloud push`, port them to `POST /api/companies/{id}/export` plus `POST /api/companies/import`, or to `geetorusai company export` and `geetorusai company import`.
