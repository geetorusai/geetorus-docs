...
Wire a `claude_local` (or `codex_local`) coding agent to a real GitHub repo so every issue you assign produces a branch, a working commit, and a pull request — all visible in both the Geetorus issue thread and the GitHub PR page. End-to-end on a fresh repo in about 20 minutes.

This is the recipe behind every "agent shipped a PR overnight" story. The moving parts are mundane: a project workspace pointed at the right git remote, a credential the agent can use to push, an `AGENTS.md` that tells the agent to work in PRs, and Geetorus's review stage matched to GitHub's review.
...
```txt
   ...
   └──────────┬───────────┘                  └──────────┬──────────┘
              ...
              └────────── isolated git worktree ─────────┘
                          (one per Geetorus issue)
```

Geetorus provisions an **isolated git worktree** when the issue is checked out. That worktree is a normal `git` clone with its own branch — the agent runs `git`, `gh`, your linter, and your test suite the same way you would. Pushes go to the upstream remote you configured on the project workspace.

---

## 1. Prereqs

- Geetorus running locally or on a server you control. See [Installation](../guides/getting-started/installation.md).
...
| Field | Value | Notes |
|---|---|---|
| `cwd` | `/Users/me/work/acme-api` | Absolute path on the host. Geetorus uses this as the worktree root. |
...
curl -X POST "$GEETORUS_API_URL/api/projects/$PROJECT_ID/workspaces" \
...
curl -X PATCH "$GEETORUS_API_URL/api/projects/$PROJECT_ID" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
Store it as a Geetorus secret on the agent's environment so it isn't sitting in a JSON file:

```bash
curl -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/secrets" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
A GitHub App installed on the org issues short-lived installation tokens, isn't tied to any one human, and shows up as its own author on the PR ("acme-geetorus[bot] opened this pull request").
...
Store the App ID, installation ID, and private key as Geetorus secrets, and have the agent's heartbeat exchange them for an installation token at the start of each run. The token expires in an hour, which is exactly long enough for one heartbeat.
...
If you connect GitHub through **Connectors** (Geetorus's app gallery, brokered through Geetorus Cloud), Geetorus resolves a GitHub credential for each run on its own — you never mint, store, or rotate a PAT. Each agent (or person) gets a durable GitHub identity that Geetorus exchanges for a short-lived token at run time and projects into the sandbox the agent runs in.

Two things make this the sturdiest option:

- **Commits are authored as the connected GitHub identity automatically.** Geetorus sets `user.name`, `user.email`, and the `GIT_AUTHOR_*` / `GIT_COMMITTER_*` environment to the connected login and its GitHub `…@users.noreply.github.com` address — so PRs show the right author with no `git config` step (contrast the last item in [Troubleshooting](#7-troubleshooting)).
- **It fails closed.** Once a GitHub connection is configured for an agent, that agent uses it — Geetorus does not silently fall back to a PAT. Exactly one identity has to resolve for a run; if none or more than one matches, the run is blocked with a clear reason rather than pushing as the wrong account.

Geetorus also receives GitHub webhooks over this connection — see [Review workflow](#6-review-workflow) for how a merged PR flows back into the issue on its own.

Setup lives on the **Connectors → Connections** page rather than in an `env` block. The exact enrollment steps depend on your Geetorus Cloud setup, so follow the in-product flow there.

> **How the server resolves a credential.** When Geetorus does git work for a project on your behalf — cloning a managed checkout, refreshing a base branch — it resolves a credential in this order: a managed GitHub connection first, then a company secret named `GITHUB_TOKEN`, `GH_TOKEN`, or `GEETORUS_GITHUB_TOKEN` (**Settings → Secrets**), then the server process environment. The token is handed to git through a credential helper, never written to disk, a URL, or the command line.

### What about `gh auth login` on the host?

If you've already run `gh auth login` interactively on the machine running the agent, the local `gh` keyring has a working credential and the agent will use it transparently — no env var plumbing required. This is what most contributors hit during a first smoke test, and it's the path the HT3 verification run used (real PR: [geetorusai/geetorus-docs#1](https://github.com/geetorusai/geetorus-docs/pull/1)).
...
2. **The issue's title or description states a PR deliverable in plain text.** Routine-created tasks and agent-to-agent hand-offs rarely name the skill, even when the text literally says to push a branch and open a PR — so Geetorus reads the text too.
...
You always work on a feature branch, never on `main` directly. Geetorus provisions an isolated git worktree per issue; that worktree is already on the right branch when you check the issue out.
...
The CTO and CEO templates in the bundled `geetorus-create-agent` skill already include a coder variant of these rules — check there before reinventing them.
...
curl -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/issues" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
- The agent checks the issue out — Geetorus provisions a worktree at `/Users/me/work/acme-api/.geetorus-worktrees/PAP-1802-workspace` and switches it to a fresh branch off `main`.
- The agent edits the right files, runs the test suite, commits, and pushes.
- The agent calls `gh pr create --fill --base main` and pastes the resulting `https://github.com/acme/api/pull/142` into the Geetorus issue thread.
...
| **Geetorus issue** | Track status (`in_review` → `done`), pipe approvals to Slack, link the PR to the goal/project, gate the next step in the workflow. |

When the PR merges:

- A human (or your CI's auto-merge bot) merges through GitHub.
- If the repo is wired through a **managed GitHub connection** (Section 3, Option C), Geetorus receives the `pull_request` webhook and keeps the linked PR's status in sync on its own — open flips to merged or closed, and a merged PR is confirmed back in the issue thread. A reviewer still owns the final `done` transition, but you no longer have to poll GitHub to notice the merge.
- With PAT or GitHub App auth there's no webhook, so a reviewer agent — the CTO, a senior engineer, or you — moves the Geetorus issue to `done` with a comment including the merge SHA. If you want that automated without a managed connection, set up a [GitHub → Geetorus routine webhook](./wire-slack-discord-notifications.md#testing-the-loop) that listens to `pull_request.closed` with `merged: true` and PATCHes the linked issue. That's a 20-line script.
...
If your team uses Geetorus's [Execution Policy](../guides/power/execution-policy.md) for review stages, set the policy on the project so the issue passes through `engineer → reviewer → done` automatically when the agent submits and the reviewer approves.
...
Set `git config user.name` and `user.email` in the worktree provision command on the project workspace. Many teams use `Geetorus Coder <noreply@geetorus.example.com>` so PRs are clearly machine-authored. If you use a managed GitHub connection (Section 3, Option C), Geetorus already stamps each commit with the connected GitHub identity and its `…@users.noreply.github.com` address, so you can skip this.
