...
export GEETORUS_API_KEY=<board-or-ceo-token>
...
The bundled `geetorus` skill that ships with the server is the canonical example for this layout. You can read it [on GitHub](https://github.com/geetorusai/geetorus/tree/main/skills/geetorus) (`SKILL.md` plus a `references/` tree). Mirror that shape for your own.
...
> **Frontmatter style.** Geetorus parses frontmatter with its own narrow YAML reader. Stick to flat scalars, nested objects, and `- ` list items. Do not use YAML block scalars (`>` or `|`) for descriptions; the current reader treats them as literal values instead of folding the following lines. The full frontmatter schema lives at [Skills reference → Frontmatter fields](../reference/skills.md#frontmatter-fields).
...
```bash
curl -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/skills/import" \
...
```bash
curl "$GEETORUS_API_URL/api/companies/$COMPANY_ID/skills" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"
```
...
```bash
curl -X POST "$GEETORUS_API_URL/api/agents/$CODER_AGENT_ID/skills/sync" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```bash
curl "$GEETORUS_API_URL/api/agents/$CODER_AGENT_ID/skills" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"
```
...
> **Bundled skills are always attached.** The server unions any `geetorusai/geetorus/*` bundled skills (`geetorus`, etc.) into every agent's resolved set, regardless of what you send. So you never have to list `geetorus` to keep it — and even a `mode: "replace"` that omits it won't drop it from the resolved set.
...
| `unsupported` | Geetorus records the assignment but cannot push files into the runtime. `openclaw_gateway` falls here — manage skills inside the remote runtime instead. See [Bring your own agent](./bring-your-own-agent.md). |
...
curl -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/agents" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
| **Geetorus-managed** (created from the UI's **+** button) | No — live | Edit in the Markdown editor on the Skills page. |
...
| **Bundled with Geetorus** | Pinned to the Geetorus release | Upgrade Geetorus itself; bundled skills can't be edited. |
...
```bash
curl "$GEETORUS_API_URL/api/companies/$COMPANY_ID/skills/$SKILL_ID/update-status" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"
```
...
```bash
curl -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/skills/$SKILL_ID/install-update" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"
```
...
- **Bundled skills are forced on.** `geetorusai/geetorus/*` skills are unioned into every resolved set; you can't drop them.
...
That's by design — `geetorus` and the other `geetorusai/geetorus/*` skills are re-imported on every list call. To suppress one, run a forked Geetorus build that doesn't ship it.
