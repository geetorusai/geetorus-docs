...
seo_title: Update Geetorus to the Latest Version
...
# Update Geetorus to the latest version

A new Geetorus release has dropped — maybe you saw it on the [releases page](https://github.com/geetorusai/geetorus/releases), maybe an agent flagged a bug that's already fixed upstream, maybe you just want the newest UI. This guide walks you through updating an existing install.

The short version, if you installed Geetorus with `geetorusai install`:

```bash
geetorusai update
```

That checks npm, takes a database backup, swaps in the new version, restarts your background service if you have one, and leaves the previous version sitting there ready for an instant rollback. The rest of this page unpacks what that means, and covers the other ways you might have installed Geetorus.

If you're installing for the first time instead, follow [Installation](../guides/getting-started/installation.md).

---

## How Geetorus versions work

Geetorus uses **calendar versioning**: `YYYY.MDD.P`. The pieces are the year, the month-and-day, and a same-day patch slot. So `v2026.525.0` is the first stable release cut on May 25, 2026. Canary builds carry a `-canary.N` suffix and ship on the `canary` npm dist-tag; stable builds ship on `latest`.

Every stable release has notes at `releases/vYYYY.MDD.P.md` in the parent repo and on the [GitHub releases page](https://github.com/geetorusai/geetorus/releases). Skim those before updating — they call out breaking changes, migrations, and new env vars.

Geetorus also nudges you. `geetorusai run` and `geetorusai doctor` do a cached, once-a-day check against npm and print a one-line notice when a newer version exists on your channel. Set `GEETORUS_UPDATE_CHECK=0`, or `updates.checkEnabled` to `false` in your config, to turn that off.
...
1. **It works out how you installed Geetorus.** Managed install, global npm, `npx`, or a source checkout — each gets different treatment, covered below.
2. **It takes a database backup.** This happens before anything is swapped, using the same machinery as [`geetorusai db:backup`](../reference/cli/setup-commands.md#geetorusai-dbbackup). If the instance was never onboarded and has no data, the backup is skipped with a note.
...
Before you switch, it helps to know where you are. `geetorusai channels` shows the four release channels and marks which one this install follows.

```bash
geetorusai channels
geetorusai channels --json    # machine-readable output
```
...
At the bottom it tells you which channel this install is on. A source checkout reports a placeholder version that does not map to a published channel, so you'll see that noted instead. The same names apply to the Docker images: `ghcr.io/geetorusai/geetorus:{latest,beta,nightly,canary}`.
...
```bash
geetorusai update --canary                # try the canary channel
geetorusai update --latest                # come back to stable
geetorusai update --version 2026.609.0    # pin to one exact version
```
...
geetorusai update --rollback --dry-run    # see which version you'd land on
```
...
- **If the database isn't reachable, the update stops.** Geetorus won't quietly update without the safety net. The error tells you to start the service with `geetorusai service start` and retry, or to skip the backup deliberately with `--no-backup`.
...
To take a snapshot outside of an update, use [`geetorusai db:backup`](../reference/cli/setup-commands.md#geetorusai-dbbackup).

---

## If you didn't use the managed install

`geetorusai update` recognizes how you installed Geetorus and does the right thing — which sometimes means telling you it isn't going to touch your setup.
...
```bash
geetorusai update
geetorusai update --version 2026.609.0
```
...
```bash
npx geetorusai install     # move to a managed install
geetorusai update          # from then on, this works
```

If you'd rather keep using `npx`, just add `@latest` to force a fresh resolve instead of the cached copy:

```bash
npx geetorusai@latest run
```

### Source checkout

Geetorus will not mutate a git repository you're working in. `update` detects the checkout and hands it back to you:
...
If you installed with `geetorusai install --ref <branch-or-tag>`, `update` re-resolves that ref on GitHub and rebuilds if the commit moved. It warns you first, because building from source executes that repository's build scripts, and requires confirmation (or `--yes`) before it does. If you pinned to an exact commit SHA, there is nothing to move to and `update` tells you the install is pinned.

---

## Verify the update worked

1. **Check the running version.** In the UI, hover the small **`v`** badge at the bottom of the left sidebar (next to the Documentation link and the settings/theme icons) — the tooltip shows the full server version. From the terminal, `geetorusai --version` reports the CLI and `geetorusai service status` reports what the running server is actually serving.
2. **Run `geetorusai doctor`.** It checks the managed install and the background service, including whether the running server's version matches the version you just installed.
...
```bash
geetorusai service restart --expected-version <version>
```

**The update stopped with "the pre-update backup cannot be taken"** — the database isn't running or isn't reachable. Start it (`geetorusai service start`, or `geetorusai run` in a terminal), then retry. Use `--no-backup` only if you're deliberately skipping the snapshot.
...
**`npx geetorusai` still reports the old version after `@latest`** — npx caches by name and falls back to the cache if the registry lookup is rate-limited or offline. Clear it and retry:

```bash
npx clear-npx-cache         # or: rm -rf ~/.npm/_npx
npx geetorusai@latest --version
```
...
- [Installing the CLI](../reference/cli/installation.md) — the managed install store, channels, and `geetorusai uninstall`.
- [Service](../reference/cli/service.md) — running Geetorus in the background, and hot restarts.
