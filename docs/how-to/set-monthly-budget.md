...
seo_description: A misconfigured loop can run a four-figure tab in a weekend. Set a cap Geetorus enforces automatically, with warnings before the hard stop.
---

# Set a monthly budget and enforce it

Autonomous agents make real API calls. A misconfigured loop, an over-eager prompt, or a pricing change can run a four-figure tab in a weekend if nothing pulls the brake. Geetorus's budget system is the brake: you set a cap, Geetorus warns at 80%, hard-stops at 100%, and pauses the offending scope until you raise the cap or the calendar month resets.

Time to working budget: about 5 minutes. Pinned versions: Geetorus API v1, calendar-month-UTC windows.
...
```bash
curl -X PATCH "$GEETORUS_API_URL/api/companies/$COMPANY_ID/budgets" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{ "budgetMonthlyCents": 10000 }'
```

`budgetMonthlyCents` is in cents — `10000` is $100/mo. Geetorus stores the value, syncs a matching `company` budget policy with default thresholds (`warnPercent: 80`, `hardStopEnabled: true`, `notifyEnabled: true`), and returns the updated record.

The same shape works per-agent:

```bash
curl -X PATCH "$GEETORUS_API_URL/api/agents/$AGENT_ID/budgets" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/budgets/policies" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "scope": "agent",
...
```bash
curl -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/cost-events" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "agentId": "'$AGENT_ID'",
...
```bash
curl -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/budget-incidents/$INCIDENT_ID/resolve" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
1. Set a tiny cap on the agent — say `$0.50`:
   ...
   ```bash
   curl -X PATCH "$GEETORUS_API_URL/api/agents/$TEST_AGENT_ID/budgets" \
     -H "Authorization: Bearer $GEETORUS_API_KEY" \
     -H "Content-Type: application/json" \
