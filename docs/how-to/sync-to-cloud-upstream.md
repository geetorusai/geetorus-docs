...
# Sync a local company to a Geetorus Cloud upstream
...
- A running local Geetorus instance you can reach as the board user.
- A Geetorus Cloud stack URL that advertises the `cloud_sync` transfer feature flag at `/.well-known/geetorus-upstream`.
...
```bash
curl -X PATCH "$GEETORUS_API_URL/api/instance/settings/experimental" \
...
The CLI generates a fresh source identity (an ed25519 keypair plus a fingerprint), opens a browser to the cloud stack's PKCE authorize URL, and stores the resulting token under `~/.geetorus/<instance>/secrets/cloud-upstream-connections.json`. The file is written with mode `0600` — keep it that way.

```bash
geetorusai cloud connect https://cloud.example.com
```

If the host has no browser, fall back to the device-code flow:

```bash
geetorusai cloud connect https://cloud.example.com --no-browser
```
...
```bash
geetorusai cloud push --company "$LOCAL_COMPANY_ID"
```
...
```bash
curl -X POST \
  "$GEETORUS_API_URL/api/cloud-upstreams/$CONNECTION_ID/push-runs/$RUN_ID/activation" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -sS "$GEETORUS_API_URL/api/cloud-upstreams?companyId=$LOCAL_COMPANY_ID" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"
```
...
| `Remote URL is not a Geetorus Cloud upstream target.` | The remote did not serve a valid `/.well-known/geetorus-upstream` document with `schema: "geetorus-upstream-discovery-v1"`. |
...
| `Remote Geetorus Cloud stack does not advertise the cloud_sync transfer flag.` | The cloud stack is reachable but doesn't enable cloud sync. Ask the stack admin to advertise the `cloud_sync` feature flag. |
...
| `No cloud connection found.` | The CLI couldn't match `--remote-url` (or the default current connection) in the local store. Re-run `geetorusai cloud connect`. |

---

## See also

- [Cloud commands in the CLI reference](../reference/cli/cloud.md) — every flag for `geetorusai cloud connect` and `geetorusai cloud push`.
