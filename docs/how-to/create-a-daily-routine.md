...
seo_description: Describe recurring work once, attach a cron trigger, and Geetorus mints a fresh issue every tick with the same owner, parent, project, and goal.
---

# Create a routine that runs daily

A [routine](../guides/welcome/glossary.md) is a recurring task generator. You describe the work once, attach a cron trigger, and Geetorus mints a fresh execution issue on every tick — assigned to one agent, with the same parent, project, and goal each time. The agent picks the issue up through its normal heartbeat, does the work, and the run shows up in the routine's history.
...
- A board API token in `GEETORUS_API_KEY` and the URL in `GEETORUS_API_URL` ([CLI auth](../administration/cli-auth.md)).
...
```bash
ROUTINE_ID=$(curl -sS -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/routines" \
...
```bash
curl -X POST "$GEETORUS_API_URL/api/routines/$ROUTINE_ID/triggers" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "kind": "schedule",
...
`timezone` is an IANA name — `UTC`, `America/New_York`, `Asia/Tokyo`. Geetorus evaluates the cron in that zone, so a 9am routine stays at 9am local through DST switches instead of drifting an hour. The `Next:` countdown on the routine detail page is computed server-side from this combination, not from the browser clock.
...
```bash
curl -X PATCH "$GEETORUS_API_URL/api/routines/$ROUTINE_ID" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "env": {
...
```bash
curl -X POST "$GEETORUS_API_URL/api/routines/$ROUTINE_ID/run" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "source": "manual",
...
```bash
curl -sS "$GEETORUS_API_URL/api/routines/$ROUTINE_ID/runs?limit=10" \
  -H "Authorization: Bearer $GEETORUS_API_KEY"
```
