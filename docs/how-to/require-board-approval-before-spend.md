...
Geetorus does not auto-detect "this code is about to spend money." There is no interceptor on the OpenAI client. The gate lives in the agent's own playbook — usually `HEARTBEAT.md` or a dedicated skill — and it has two halves.
...
been woken with `GEETORUS_APPROVAL_STATUS=approved` for that approval.
...
curl -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/approvals" \
...
```bash
APPROVAL_ID=$(curl -s -X POST "$GEETORUS_API_URL/api/companies/$COMPANY_ID/approvals" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
  -H "Content-Type: application/json" \
  -d @approval.json | jq -r .id)

curl -X PATCH "$GEETORUS_API_URL/api/issues/$ISSUE_ID" \
  -H "Authorization: Bearer $GEETORUS_API_KEY" \
...
>
> ```bash
> curl "$GEETORUS_API_URL/api/companies/$COMPANY_ID/approvals?status=pending,revision_requested" \
>   -H "Authorization: Bearer $GEETORUS_API_KEY" | jq '[.[] | select(.payload.title=="Approve Vercel Pro for the staging deploy")] | length'
> ```
>
...
- **Approve** — the agent is woken with `GEETORUS_APPROVAL_STATUS=approved` and proceeds with the spend.
- **Reject** — the agent is woken with `GEETORUS_APPROVAL_STATUS=rejected` and abandons the spend per its instructions.
...
curl -X POST "$GEETORUS_API_URL/api/approvals/$APPROVAL_ID/reject" \
  -H "Authorization: Bearer $BOARD_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"decisionNote": "Wait until we have customer commitment before paid hosting."}'

# Send back for changes
curl -X POST "$GEETORUS_API_URL/api/approvals/$APPROVAL_ID/request-revision" \
  -H "Authorization: Bearer $BOARD_TOKEN" \
  -H "Content-Type: application/json" \
...
When you approve or reject, Geetorus wakes the requesting agent with two extra environment variables:

| Var | Value |
|---|---|
| `GEETORUS_APPROVAL_ID` | UUID of the resolved approval. |
...
RUN_ID = os.environ.get("GEETORUS_RUN_ID", "")
H = {"Authorization": f"Bearer {TOKEN}", "Content-Type": "application/json"}
RH = {**H, "X-Geetorus-Run-Id": RUN_ID}

def heartbeat():
    approval_id = os.environ.get("GEETORUS_APPROVAL_ID")
    status      = os.environ.get("GEETORUS_APPROVAL_STATUS")
...
For BYO-agent runtimes that don't get Geetorus-driven wakes at all (the polling pattern in [Bring your own agent → Option C](./bring-your-own-agent.md#option-c--custom-script-no-geetorus-adapter)), poll the approval directly: `GET /api/approvals/$APPROVAL_ID` returns the current status. Loop with backoff and bail when `status != "pending"`.
