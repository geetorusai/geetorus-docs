...
**Cause.** Geetorus protects you from an expensive no-op loop. After two successful runs for the same issue make no issue-visible progress, another recovery-style wake waits for an escalating cooldown. The delay starts at two minutes and never exceeds 30 minutes.
...
**Symptom.** A run finishes as `succeeded`, but the task never leaves `in_progress`. Geetorus posts a **Missing issue disposition** notice — *"Geetorus needs a disposition before this issue can continue."* — and the same agent wakes up one more time on that task, sometimes doing real work before it finally sets a status.

**Cause.** The run ended without telling Geetorus what happened to the task. Every successful run has to leave one of four outcomes behind:
...
When none of those is present and nothing else already owns the next action — no queued wake, no pending approval or interaction, no monitor, no open recovery issue, no pause hold — Geetorus queues one corrective wake back to the same agent, with the wake reason `finish_successful_run_handoff`.
...
- **It only tries once.** The corrective wake is bounded to a single attempt per run, so you will never see this turn into a loop. If that one attempt still lands no disposition, Geetorus posts a **Missing disposition recovery blocked** notice — *"Geetorus could not resolve this issue's missing disposition automatically. The issue is blocked on a recovery owner."* — and hands the task to a recovery owner instead of trying again.
...
If you would rather not pay for these wakes at all, the cure is upstream: have your agents record a status before they finish. A comment describing progress is not a disposition — Geetorus is looking at the task's status and path, not its prose.
