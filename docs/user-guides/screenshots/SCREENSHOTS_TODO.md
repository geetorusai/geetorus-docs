...
> **WARNING — Run on a disposable demo Geetorus instance ONLY. Never against live data.**
...
4. Install all three bundled example plugins on the demo instance to populate the plugin list:
   ...
   - `@geetorusai/plugin-file-browser-example`
...
10. Generate a CLI auth code via `geetorusai login` so `/cli-auth/{id}` renders a real pending request.
11. Generate a board claim token (`geetorusai board claim ...` or the admin flow) so `/board-claim/{token}` renders.
...
const SLUG = "ACME";
const OUT  = "/path/to/geetorus-docs/docs/user-guides/screenshots/light";
...
const page = await client.page("geetorus", { viewport: { width: 1440, height: 900 } });
