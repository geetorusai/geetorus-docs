...
`<GeetorusLoading />` first, which is exactly the blank-page failure mode
...
`reference/skills/bundled/geetorus-operations/status-card-query.md`,
...
2. Call the agent-scoped endpoints with `Authorization: Bearer <token>` and
   `X-Geetorus-Run-Id: <runId>` — `middleware/auth.ts:372` sets
