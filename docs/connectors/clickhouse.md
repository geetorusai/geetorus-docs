...
The real control is on the ClickHouse side. Reach is whatever the signed-in account's database users and grants permit — Geetorus does not add a table or database filter on top.
...
Limitations: one ClickStack service per connection. ClickHouse Cloud only. Access control is ClickHouse's grants, not a Geetorus resource filter.
