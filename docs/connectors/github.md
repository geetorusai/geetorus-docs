...
GitHub does two unrelated jobs in Geetorus, and they are separate connections with separate credentials.
...
| People to start and continue work by commenting in GitHub | **Chat with an agent** | A GitHub App that turns issue and pull-request comments into Geetorus tasks |
...
Geetorus's per-action **Allowed** / **Ask first** / **Off** switches govern tool calls that go through Geetorus's tool gateway. When a GitHub connection is bound to an agent as a dedicated identity, that same identity is also handed to the agent's workspace shell, and shell commands do not go through the gateway.

Geetorus states this when you change a GitHub permission:
...
The practical consequence: for an agent that will push code, the limits that matter are GitHub-side — selected repositories, branch protection, required reviews — not Geetorus's per-tool switches.
...
| --- | --- | --- |
| **Use this connection as an agent tool** | The default. Geetorus manages the GitHub App authorization, and the identity is also available to shell Git and `gh` | Only when the instance is enrolled with Geetorus Cloud and Cloud advertises the GitHub connector profile |
...
GitHub's hosted server supplies the action list, so Geetorus does not ship a frozen copy. Read the live list on the connection's **Permissions** tab and use **Refresh actions** after GitHub changes it.
...
People comment in an issue or pull request, mentioning the agent, and Geetorus creates a task.
...
You create one private GitHub App for this — with **Issues** and **Pull requests** read and write permission, the **issue_comment** and **pull_request_review_comment** events, and a webhook secret Geetorus generates — then give Geetorus its **App ID** and **private key (PEM)**. [Set up the GitHub connector](github-setup.md) has the numbered procedure.
...
[Connect an agent to a GitHub repo](../how-to/connect-agent-to-github.md) is a different job: pointing a project workspace at a remote, and matching Geetorus's review stage to GitHub's. That page remains the right one for the git and `gh` workflow itself. This connector is how the credential gets there. Use both.

## Troubleshooting

| Problem | Likely cause | Fix |
| --- | --- | --- |
| **Use this connection as an agent tool** is not offered | The instance is not enrolled with Geetorus Cloud, or Cloud is not advertising the GitHub profile | Use **Personal access token (advanced)** |
...
| Webhook deliveries fail in GitHub | The webhook secret or URL does not match | Regenerate the secret in Geetorus and update the App |
| **Needs attention** | The token expired or the installation was removed | Select **Reconnect** |

## Limitations

The two routes are independent. Repository scope lives in GitHub on both. The managed tool path depends on Cloud enrollment; the channel path depends on the experimental chat setting. Geetorus cannot constrain shell Git or `gh` with per-action settings.
