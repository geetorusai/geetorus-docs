...
- A Box administrator who will create the OAuth integration and enable AI access for the enterprise. If that is not you, start by asking them — no amount of work in Geetorus substitutes for it.
- The client ID and secret from that Box integration, and Geetorus's callback URI registered on it. Geetorus shows the URI during setup.
...
Most of this happens in Box's Admin Console and needs an administrator. Start in Geetorus only to read the callback URI.

### 1. Read Geetorus's callback URI

Open **Connectors** → **Box** → the **Access** step → **Use your own OAuth app**. Geetorus displays the callback URI; copy it and give it to whoever does step 3.
...
> **Warning:** Without these, authorization can succeed and still leave agents unable to do anything useful. If the connection looks healthy but every call fails or returns nothing, come back to this step before debugging anything in Geetorus.
...
Reach is the authorizing Box account's: the files, folders, and shared content that account can already open. Geetorus has no folder picker, so narrow access by authorizing with an account that has access to less.
...
File writes and deletions are consequential and not always recoverable from Geetorus. Leave them on **Ask first** or **Off**, and rely on Box's trash and version history for recovery. See [Set action permissions](action-permissions.md).
...
Limitations: one Box account per connection. No folder restriction inside Geetorus. Enterprise policy can override what the account appears to have.
