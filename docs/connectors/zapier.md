...
Zapier lets agents reach the apps you have already connected in your Zapier account. You build an MCP server in Zapier, choose which actions it exposes, and give Geetorus its generated URL.
...
The actions an agent can call are decided in Zapier, not Geetorus. This is the main thing to get right before connecting: a server with broad actions enabled gives the agent broad reach, and Geetorus cannot narrow it beyond switching off what Zapier advertises.
...
5. Paste the full URL on the **Add MCP URL** step and select **Check link**. Geetorus verifies it can reach the server, then stores the URL as a secret.
...
| Whether an agent may call an exposed action | Geetorus, on the **Permissions** tab | Require approval, or switch one off |

Changing the exposed action set in Zapier does not require reconnecting — use **Refresh actions** in Geetorus afterwards to re-read the list.

> **Warning:** On this connection, an action you expose in Zapier becomes **active** in Geetorus once you refresh, governed by the policies already in force rather than held back for approval. Widening the Zapier side is the access decision; review the Geetorus list after every refresh.

Anyone in Zapier who can edit that MCP server can widen what the agent can reach, without touching Geetorus. Keep the list of people who can edit it small, and review it when you review the connection.
...
| An action you enabled in Zapier is missing | Geetorus has not re-read the list | Use **Refresh actions** |
| An action you exposed in Zapier is missing | Geetorus's copy of the list is stale | Use **Refresh actions**, then confirm the new action's setting — it arrives active under existing policy rather than switched off |
| An action fails inside Zapier | The underlying app connection in Zapier has expired | Reconnect that app in Zapier |
| You suspect the URL leaked | The URL is a bearer credential | Regenerate the server URL in Zapier and reconnect in Geetorus |

Limitations: the action surface is whatever the Zapier server exposes, and Geetorus cannot see or restrict what happens inside a Zap beyond the action it calls. Zapier's own plan limits and task quotas apply. Rotating the URL means reconnecting.
