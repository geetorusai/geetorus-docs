...
- For subscription sign-in only: a sign-in environment. Geetorus offers two kinds — the Geetorus server host itself, or a sandbox environment that supports interactive sign-in. Which you get depends on how your instance is configured; see [With a Claude subscription](#with-a-claude-subscription).
...
Subscription plans carry their own usage limits, and those limits are Anthropic's, not Geetorus's. Check current plan terms with Anthropic rather than assuming a rate.
...
Paste the key. Geetorus stores it as a secret and it is not readable afterwards.

### With a Claude subscription

Geetorus never asks for your Claude password. It signs in through the `claude` CLI against a credential directory belonging to this connection alone.
...
| --- | --- | --- |
| **The Geetorus server host** | The `claude` CLI installed on that host, shell access to it, and an active local environment. You must be operating Geetorus locally — a remote board session cannot start this attempt | Geetorus shows a command to paste into a terminal on that host |
| **A sandbox environment** | A sandbox whose provider supports interactive sign-in, configured by an administrator | Sign-in happens in the environment Geetorus provides; no terminal on the server host is required |

If more than one is available, Geetorus shows a **Sign-in environment** selector. Note that the environment used to sign in may differ from the environment the agent later runs in; choosing it here does not change agent routing.

**On the Geetorus server host:**

1. Select **Sign in**. Geetorus shows a command that sets `CLAUDE_CONFIG_DIR` to a directory for this attempt and then runs `claude auth login`.
2. Run it in a terminal on that host and complete Anthropic's sign-in.
3. Geetorus detects the credential and finishes the connection. The attempt stays open for 30 minutes before it expires.
...
2. Wait for Geetorus to prepare the sign-in link, then use **Sign in to Claude** to open it.
3. Complete the provider sign-in, then return the authorization code to the code field shown in Geetorus. Do not paste it into a task or chat.
4. Wait for Geetorus to finish the connection. If the attempt expires or fails, start a new attempt; an open provider page alone does not establish that the credential was saved.
...
| The sign-in command does nothing | The `claude` CLI is not installed on the host you ran it on, or you ran it on the wrong machine | Install the CLI and run the command on the Geetorus server host. This branch also requires that you are operating Geetorus locally rather than over a remote board session |
...
| Runs fail with a provider quota error | Anthropic's plan or key limits, not a Geetorus limit | Check usage with Anthropic |
