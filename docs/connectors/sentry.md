...
Geetorus registers its client with Sentry automatically, so there is nothing to configure in a developer console. This is the supported path and the one this page documents.
...
> **Note:** The connector's definition mentions organization, project, and environment scope, but those are descriptive — Geetorus does not enforce a project or environment filter on top of what Sentry grants. If an agent should only see one project, authorize with an account whose team membership is limited to it.
...
Limitations: one organization per connection. No project or environment filter inside Geetorus. Sentry's data retention bounds how far back an agent can look.
