...
Agents can read Linear issues, create new ones, and update existing ones — useful for keeping engineering work in Linear while agents do the work in Geetorus.

## Before you connect

- A Linear account with access to the workspace and teams you want agents to use.
- You must register your own Linear OAuth app. This Geetorus connection method asks for registered client credentials. This is a constraint of the documented Geetorus route, not a claim that Linear lacks dynamic client registration. Add Geetorus's redirect URI to the app; Geetorus shows the exact URI during setup.

## Connect Linear

Geetorus supplies the redirect URI and Linear supplies the credentials, so start in Geetorus.

### 1. Read Geetorus's redirect URI

Open **Connectors** → **Linear**, choose the identity and agents on the **Access** step, and copy the redirect URI Geetorus displays. Leave the screen open.
...
> **Note:** There is no team or project picker in Geetorus. Fields describing workspace, team, and project scope exist in the connector's definition, but they do not narrow what an agent can reach — the limit is the authorizing account's own access in Linear. If an agent should only touch one team's issues, authorize with an account restricted to that team, or rely on action settings and clear instructions rather than assuming a resource filter.
...
| --- | --- | --- |
| Setup asks for a client ID and secret | Expected for this Geetorus connection method | Register the app in Linear's settings |
| Authorization fails with a redirect error | The redirect URI on the Linear app does not match the one Geetorus shows | Copy the URI exactly and retry |
...
| The agent reaches more teams than expected | Reach follows the authorizing account, not a Geetorus filter | Authorize with a more limited account |
...
Limitations: one workspace per connection. No team or project restriction inside Geetorus. Linear's own rate limits apply.
