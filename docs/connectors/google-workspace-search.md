...
- Without Geetorus Cloud enrollment, your own Google OAuth client with the **Gmail API**, **Drive API**, **Calendar API**, **Chat API**, and **Workspace MCP API** enabled — five APIs, more than any single-app connector needs. [Set up your own Google OAuth app](google-setup.md) is the complete procedure.
...
What the search covers is what the authorizing account can already see across those four apps — its own mail, files shared with it, calendars on its list, and spaces it belongs to. Geetorus does not narrow it, and there is no per-app or per-folder filter.
...
| **Connect with Geetorus** is not offered | The instance is not enrolled with Geetorus Cloud, or Cloud is not advertising the search profile | Use your own Google OAuth app |
