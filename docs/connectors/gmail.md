...
> **Note:** This is Geetorus's reviewed list, not a promise about Google's server. Google supplies the catalog and can change it; the reviewed list is what Geetorus will enable from it.
...
Google-side controls still apply on top: a Workspace administrator can restrict which third-party apps may hold Gmail scopes, and revoking the grant in your Google account stops the connection regardless of Geetorus's settings.
...
| **Connect with Geetorus** is not offered | The instance is not enrolled with Geetorus Cloud, or Cloud is not advertising the Gmail profile | Use **Use your own Google OAuth app**, or ask an administrator about enrollment |
