...
- For subscription sign-in only: a sign-in environment — either the Geetorus server host, or a sandbox environment that supports interactive sign-in. See [With a subscription](#with-a-subscription).
...
Confirm current plan entitlements and limits with OpenAI rather than assuming; they change independently of Geetorus.
...
| --- | --- | --- |
| **The Geetorus server host** | The `codex` CLI installed on that host, shell access to it, and an active local environment. You must be operating Geetorus locally — a remote board session cannot start this attempt | Geetorus shows a command to paste into a terminal on that host |
...
When both are available Geetorus shows a **Sign-in environment** selector. The environment used to sign in may differ from where the agent later runs; picking one here does not change agent routing.

**On the Geetorus server host:**

1. Select **Sign in**. Geetorus shows a command that sets `CODEX_HOME` to a directory for this attempt and runs `codex login --device-auth` with file-based credential storage.
...
2. Wait for Geetorus to prepare the sign-in link, then use **Sign in to OpenAI** to open it.
3. If Geetorus displays a device code, enter it on the provider's sign-in page. Complete authorization and return to Geetorus.
...
**A successful reply proves** that some OpenAI credential was accepted, the runtime is compatible, and the checks above passed. Geetorus refuses an ineligible binding outright rather than falling back to another account, so a run that completes under an explicit binding used an authorized credential.

**It does not prove which account was billed.** Geetorus resolves provider, method, connection and responsible user internally but does not surface them per run. To confirm a specific account, check usage on OpenAI's dashboard for that account afterwards.
...
| The sign-in command does nothing | The `codex` CLI is missing on the host you ran it on, or you ran it on the wrong machine | Install the CLI and run the command on the Geetorus server host. This branch also requires operating Geetorus locally rather than over a remote board session |
| Status **expired** or **needs attention** | The credential rotated or the key was revoked | Reconnect the account |
| Runs fail with a quota error | OpenAI's plan or key limits, not a Geetorus limit | Check usage with OpenAI |
