...
Restricted keys are the main control on this connector. Stripe lets you grant read-only access to specific resource types, which is far more precise than anything Geetorus can apply afterwards.

> **Warning:** Geetorus's key field shows the placeholder `sk_...`, which is a standard secret key carrying your account's full API access. A **restricted key** — its id begins `rk_` — works in the same field and is what you should use. Do not read the placeholder as a recommendation.

> **Note:** Whether a key is a test-mode or live-mode key is decided in Stripe when you create it, and it determines whether an agent is touching real money. Nothing in Geetorus displays or changes that, so label your connections clearly and check the key's mode in Stripe before connecting anything to production.
...
| Whether an agent may call an exposed action | Geetorus, on the **Permissions** tab | Per action, with approval |

Use the restricted key to remove capability, and Geetorus's settings to require review on what remains.

Test mode versus live mode is decided entirely by which credential you connect. A test-mode key reaches test data only. There is no toggle in Geetorus that switches a live connection into test mode, so if you want both, make two connections and name them clearly.
...
| Behaviour changes without a Geetorus change | The hosted server is in public preview and evolving | Re-read the action list after provider changes |
| **Needs attention** | The key was rolled or the sign-in expired | Select **Reconnect** |

Limitations: one Stripe account and one mode per connection. Geetorus cannot reverse a financial operation. Public preview means the surface can change.
