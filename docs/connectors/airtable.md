...
Geetorus registers its client with Airtable automatically, so there is nothing to set up in a developer console.

## Choose access

Base and table reach comes from Airtable: what the authorizing account can open, plus whatever you selected at Airtable's authorization screen. Geetorus has no base picker of its own, so narrow it there or by authorizing with an account that belongs to fewer workspaces.
...
Limitations: one Airtable account per connection. No base or table filter inside Geetorus. Airtable's API rate limits apply, so large scans are slow.
