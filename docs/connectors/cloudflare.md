...
Reach is Cloudflare's decision: the account, zones, and resources the credential permits. There is no zone picker in Geetorus.
...
Limitations: one Cloudflare credential per connection. No zone filter inside Geetorus. Geetorus cannot undo a Cloudflare configuration change.
