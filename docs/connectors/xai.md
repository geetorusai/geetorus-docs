...
- An agent running on the **Grok** adapter. This is the requirement that catches people out: the credential is only usable by an agent on the Grok adapter, and the general-purpose Geetorus runner harness does not resolve to it. Set the agent's adapter to Grok rather than expecting a runner-based agent to pick this up.
...
| --- | --- | --- |
| **The Geetorus server host** | The `grok` CLI installed on that host, shell access to it, and an active local environment. You must be operating Geetorus locally — a remote board session cannot start this attempt | Geetorus shows a command to paste into a terminal on that host |
| **A sandbox environment** | A sandbox whose provider supports interactive sign-in, configured by an administrator | Sign-in happens in the environment Geetorus provides |

When both exist Geetorus shows a **Sign-in environment** selector. The sign-in environment may differ from where the agent later runs.

**On the Geetorus server host:**

1. Select **Sign in**. Geetorus shows a command that sets `GROK_HOME` to a directory for this attempt and runs `grok login --device-auth`.
...
2. Wait for Geetorus to prepare the sign-in link, then use **Sign in to Grok** to open it.
...
**A reply proves** a Grok credential was accepted, the adapter matches, and the checks above passed — Geetorus refuses an ineligible binding rather than falling back. **It does not prove which account was billed**; that attribution is not surfaced per run, so check usage in the xAI console for the account you expect.
...
| The sign-in command does nothing | The `grok` CLI is missing on the host you ran it on, or you ran it on the wrong machine | Install the CLI and run the command on the Geetorus server host, operating Geetorus locally |
| Status **expired** or **needs attention** | The credential rotated or the key was revoked | Reconnect the account |
| Runs fail with a quota error | xAI's plan or key limits, not a Geetorus limit | Check usage with xAI |
