...
Connecting a service and letting an agent use it are not the same act. Geetorus separates them — but **how** it separates them depends on what kind of connector you are looking at. Start here.
...
Channels have no action switches, but they are not ungoverned. Each one has an **Access** tab with **External identity access**, and one toggle on it decides what happens when someone Geetorus does not recognize sends a message:

| **Allow unlinked people** | Effect |
| --- | --- |
| **On** | The sender is a restricted guest. Their tasks run only with an isolated workspace and sandbox environment, and Geetorus refuses the request when that is unavailable. Guests cannot approve, hire, spend, manage access, or reassign agents |
| **Off** | Only senders linked to a Geetorus person can start work |

Linked identities act as their current Geetorus user, with that person's permissions.
...
Each connection carries a company-scoped identity — a `uid` like `google-sheets/finance-sheet-1a2b3c4d` — that survives renaming. That identity is what the rest of Geetorus references, which is why renaming a connection in the UI never breaks a policy pointed at it.
...
A grant answers "whose account is this?". Geetorus supports three kinds. Available choices and UI labels depend on the connector and method; this table describes the data model, not three options offered by every setup screen:
...
Once connected, Geetorus reads the service's action list and gives each entry one of three states:
...
- **Always allowed** — the call runs, and Geetorus creates a **trust rule** so matching calls stop asking.
