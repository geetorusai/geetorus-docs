...
Site reach is Webflow's decision: the sites the authorizing account can access under its workspace and site roles, plus whatever you authorized. Geetorus has no site picker.
...
> **Note:** We have not run Webflow Designer tools through Geetorus. What is established here is the provider's own requirement, not a tested Geetorus result — so if you do try it, treat it as unverified and start on a site that does not matter. Webflow's [MCP getting started](https://developers.webflow.com/mcp/reference/getting-started) is the authority on the current tool surface.
...
Limitations: one Webflow account per connection. No site filter inside Geetorus. Designer tools need a companion app kept open in the Designer, so they do not suit unattended work. Webflow's plan limits apply to CMS item counts and API rates.
