...
The question to settle before connecting is *whose* memory an agent is reading and writing. Mem0 organizes memories by identifiers you supply at call time, so the boundary is a convention you enforce, not something Geetorus checks.
...
3. Paste the **Mem0 API key**. Geetorus stores it as a secret.
...
Limitations: one Mem0 project per connection, and the key reaches all of it. Namespacing is not enforced by Geetorus. Mem0's own plan limits apply.
