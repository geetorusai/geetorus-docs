...
> **Note:** You need this page only if you are bringing your own OAuth client. If **Connect with Geetorus** is offered on the connector and you are happy to use it, that path needs no Cloud console work — skip to [Check you can use the managed path first](#check-you-can-use-the-managed-path-first).
...
| --- | --- | --- |
| **Connect with Geetorus** | Nothing in the Cloud console. Geetorus owns the OAuth client | Only when the instance is enrolled with Geetorus Cloud *and* Cloud advertises that connector's profile. If it is missing, ask an administrator to check enrollment and profile availability |
...
1. Apply at [Google Workspace Developer Preview](https://developers.google.com/workspace/preview), naming the Workspace email that will authorize Geetorus and the Cloud project that will own the client.
...
> **Warning:** An unregistered account or project fails at Google's authorization screen, not when you start setup in Geetorus. If consent is refused, check registration, OAuth app verification or testing status, and your Workspace administrator's access policy.
...
## 3. Create the OAuth client and register Geetorus's callback
...
2. Add Geetorus's callback as an **Authorized redirect URI**. Geetorus displays the exact URI during setup — copy it from there rather than typing it. It is the `/api/tools/oauth/callback` route on your instance's own origin.
...
| `redirect_uri_mismatch` | The redirect URI on the OAuth client does not match the one Geetorus shows | Copy the URI from Geetorus's setup screen exactly |
...
| **Connect with Geetorus** is not offered | The instance is not enrolled with Geetorus Cloud, or Cloud is not advertising that profile | Use your own OAuth client, or ask an administrator about enrollment |
