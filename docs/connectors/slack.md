...
Slack does two unrelated jobs in Geetorus, and they are separate connections with separate credentials.
...
| People to start and continue work by talking to an agent in Slack | **Chat with an agent** | A Slack app that turns mentions and DMs into Geetorus tasks |
...
Wait for a release with verified Slack MCP compatibility, or contact [support@geetorus.ing](mailto:support@geetorus.ing) before attempting this route. The access notes below explain the intended tool behavior; they are not a record of successful setup on this release.

### Access and actions

Channel reach is Slack's decision: the authorized token sees what its scopes and the workspace allow, and private channels require the authorizing user or bot to be a member. Geetorus does not have a channel picker, so narrow access on the Slack side.
...
People mention the agent in a channel or send it a direct message, and Geetorus creates a task. Replies come back in the thread.
...
- A publicly reachable HTTPS address for the instance. Slack delivers events by calling Geetorus; this route does not use Slack's socket mode.
...
3. Geetorus generates a **Slack app manifest** containing exactly the scopes, events, and request URL this route needs. Copy it.
...
5. Back in Geetorus, paste the **Bot User OAuth Token** (it starts with `xoxb-`) and the **Signing Secret**.
...
> **Danger:** The bot token and signing secret are full credentials for the app. Paste them only into Geetorus. If either leaks, rotate it in Slack and reconnect.

### How a conversation becomes work

| In Slack | In Geetorus |
| --- | --- |
...
Geetorus acknowledges with a reaction so people can see a message was picked up before the agent has finished thinking.
...
3. Expect a reaction, then a threaded reply, and a matching task in Geetorus.
...
| The channel app is installed but silent | Slack cannot reach the instance, or the app was not created from Geetorus's manifest | Confirm the public HTTPS address, then recreate the app from the manifest |
...
One workspace per connection on either route. No channel picker in Geetorus on either route. The channel route needs public ingress and does not support socket mode. Private channels require explicit membership.
