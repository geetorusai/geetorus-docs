...
- If your instance is not enrolled with Geetorus Cloud, **Connect with Geetorus** is not offered and you will need your own Google OAuth client with the **Calendar API** and **Calendar MCP API** enabled. [Set up your own Google OAuth app](google-setup.md) is the complete procedure — do it before you start here.
...
3. Choose the capability group, then the path: **Connect with Geetorus** for the managed Google client, or **Use your own Google OAuth app** — which needs the client ID and secret from [Set up your own Google OAuth app](google-setup.md).
4. Complete Google's consent screen with the registered Workspace account.

## Choose access

Which calendars an agent can reach is decided by Google, not Geetorus: it is whatever appears on the authorizing account's calendar list, including calendars shared with that account. There is no calendar picker in Geetorus. If an agent should not see a calendar, remove the sharing in Google Calendar or authorize with an account that does not have it.
...
> **Warning:** Event writes have side effects outside Geetorus. Creating or updating an event with attendees can send invitations and notifications from your account, and `respond-to-event` answers an invitation as you. The connector's own guidance is that all event mutations should be approved — leave them on **Ask first**.
...
| **Connect with Geetorus** is not offered | The instance is not enrolled with Geetorus Cloud, or Cloud is not advertising the Calendar profile | Use your own Google OAuth app, or ask an administrator |
