...
PostHog has a large and fast-moving tool catalog, which shapes how Geetorus handles it: a tool PostHog does not annotate as read-only is classified as a **write**, and `exec` is always **destructive**. That conservative classification is deliberate, and it is explained below.
...
Project reach is the PostHog account's, narrowed by the pin you set and honoured by PostHog. On top of that, Geetorus's action settings apply as usual — and those are enforced on Geetorus's side.
...
| Queries time out or are throttled | PostHog's own query limits, not Geetorus's | Narrow the time range or the query |
