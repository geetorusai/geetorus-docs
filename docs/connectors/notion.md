...
- Permission to add a connection to the workspace. Some Notion plans let a workspace owner restrict which connections members may approve; if yours does, ask an owner to approve the Geetorus connection first.
...
4. Approve the authorization. Notion returns you to Geetorus and the connection becomes **Ready**.

Geetorus registers its client with Notion automatically, so there is nothing to set up in a developer console.
...
| Reachable content | Everything the connected Notion account can open. To narrow it, change that account's permissions in Notion, or reconnect under an account with less access. Geetorus has no page-level filter for Notion. |
...
> **Note:** Moving, duplicating, and converting pages are writes even though their names do not start with a create or update verb. Geetorus classifies them explicitly so they are not mistaken for reads.
...
| Calls stop working after a while | The Notion grant was revoked, in Notion or in Geetorus | The connection shows **Needs attention**; select **Reconnect** |
...
Workspace owners can review and revoke connected clients in Notion under **Settings** → **Connections**, independently of anything set in Geetorus.
