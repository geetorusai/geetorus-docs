...
seo_description: Turn dead references to pull requests, tickets, and deployments into live status you can read from inside Geetorus instead of opening another tab.
---

# External Objects

Issues constantly reference work that lives somewhere else — a GitHub pull request, a ticket in another tracker, a deployment. Normally those are dead links: to know whether that PR merged, you leave Geetorus and go look.

With **External Objects** on, Geetorus detects external URLs anywhere in an issue — title, description, comments, documents — and renders them as live status pills instead of plain links. A referenced GitHub pull request shows up as *Open*, *Draft*, *Merged*, or *Closed* right where it's mentioned, plus in a rollup panel on the issue.
...
Public repositories resolve without setup. For private repos (and friendlier rate limits), add a company **secret** named `GITHUB_TOKEN`, `GH_TOKEN`, or `GEETORUS_GITHUB_TOKEN`. Without access, private objects show *"Requires auth"*.
