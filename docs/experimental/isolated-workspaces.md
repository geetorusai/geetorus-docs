...
3. Optionally turn on **New tasks default to isolated checkout**, and open **Show advanced checkout settings** for the git details: base ref (`origin/main`), branch template (`{{issue.identifier}}-{{slug}}`), worktree parent dir (`.geetorus/worktrees`), and provision/teardown commands.
...
- **Auto** *(the default)* — Concurrent runs on local/SSH runners; runs take turns in cloud sandboxes. Geetorus picks the safe behaviour for where the run actually executes.
...
If a task also runs on an [environment](environments.md) — an SSH machine or a provider sandbox — there's one more question to settle: which filesystem the agent actually edits. Geetorus records that as the run's *workspace realization*, and it has two modes.
...
In `in_place` mode nothing is copied. The environment already holds the authoritative files and the right toolchain, so the run works directly in the environment's own tree, at the path Geetorus calls the `authoritativeRoot`, and there's no sync-back step afterwards. Your local worktree and branch still exist; the run just isn't the thing editing them.

You don't pick this. The environment's driver or sandbox provider declares it, and Geetorus follows — there is no setting for it in project or task configuration. On a Codex run you can tell which mode you got from two places: the run log says `[geetorus] Syncing CODEX_HOME to …` in place mode instead of `[geetorus] Syncing workspace and CODEX_HOME to …`, and the agent's process gets `GEETORUS_WORKSPACE_REALIZATION_MODE` and `GEETORUS_WORKSPACE_AUTHORITATIVE_ROOT` so your own scripts can check too.
...
- A Codex agent pinned to the ACP engine can't run in an in-place environment: *"In-place workspace realization requires the Codex CLI engine; ACP archive staging is not supported."* Leave the engine unset and Geetorus picks the CLI for you.
