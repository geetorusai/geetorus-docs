...
seo_title: Cloud Sync to a Geetorus Upstream
seo_description: Push a local company — agents, projects, issues, skills, portability files — into a Geetorus Cloud stack, previewing exactly what would change first.
---
...
**Cloud Sync** connects a locally-run Geetorus instance to a Geetorus Cloud upstream so you can push a local company — agents, projects, issues, skills, portability files — into a cloud stack. You preview exactly what would change, resolve conflicts, push, and then review activation before any automation resumes on the cloud side.
...
2. Turn on **Cloud Sync** — *"Show local Geetorus Cloud upstream connection, preview, push, retry, and activation review surfaces."*

Unlike most flags on the page, this one gates the API too: with it off, the cloud-upstream endpoints refuse requests and `geetorusai cloud push` errors out — not just hidden UI.

## Using it

1. Open **Company Settings → Cloud upstream** (the nav item appears once the flag is on).
2. **Connect** — paste your Geetorus Cloud stack URL and approve the authorization grant on the cloud side.
...
The step-by-step recipe — including the CLI flow and troubleshooting — is in [Sync a local company to a Geetorus Cloud upstream](../how-to/sync-to-cloud-upstream.md).

## When it's off

The Cloud upstream page shows *"Cloud sync is disabled. Enable it in Instance Settings to show upstream connection and push tools."*, the *Send to Geetorus Cloud* button disappears, and the API and CLI refuse cloud-sync operations. Saved connections and run history are preserved and return when you re-enable.
...
- [Sync a local company to a Geetorus Cloud upstream](../how-to/sync-to-cloud-upstream.md) — the full how-to.
