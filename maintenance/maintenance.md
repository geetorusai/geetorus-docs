...
Operational notes for maintainers of the Geetorus docs site. Reader-facing instructions live in [README.md](../README.md).
...
- **Production custom domain** — use `/` (the default in `docs:build` and `docs:publish`) for `https://docs.geetorus.ing/`.
- **GitHub Pages repo subpath preview** — use `/geetorus-docs/` via `npm run docs:build:github-subpath`.
...
Mismatched base paths are the most common cause of 404s after a build — if asset URLs point to `/geetorus-docs/...` but you serve at `/`, everything breaks.
...
- `github.com/geetorusai/geetorus-docs/edit/main/<path>` — GitHub web editor.
- `github.com/geetorusai/geetorus-docs/issues/new?template=03-docs-feedback.yml&...` — prefilled docs-feedback issue.
...
Primary publishing uses Cloudflare Pages, matching the rest of the public Geetorus sites:
...
```sh
wrangler pages deploy .site --project-name geetorus-docs --branch main
```

The Cloudflare Pages project name is `geetorus-docs`; production deploys use branch `main`.
...
By default, the publish script builds for `https://docs.geetorus.ing/` and writes a `CNAME` file containing `docs.geetorus.ing`. Override with `PAGES_BASE_PATH=/geetorus-docs/ PAGES_CUSTOM_DOMAIN= npm run docs:publish:github` only when intentionally publishing a GitHub Pages subpath preview without the custom domain.

## Sync workflow

These docs follow the parent **geetorusai/geetorus** repo automatically via the [`/sync-docs`](skills/sync-docs/SKILL.md) skill. The skill detects code-surface changes (CLI commands, env vars, REST routes, adapters, plugin SDK, schemas) and produces edits in our friendly-tutorial voice.
...
```mermaid
flowchart LR
    subgraph parent["geetorusai/geetorus (parent)"]
        direction TB
    ...
    subgraph docs["geetorus-docs"]
        direction TB
        docsmain["main<br/>(docs.geetorus.ing — released)"]
...
|---|---|---|---|
| `main` | Latest parent **release tag** (e.g. `v2026.512.0`) | `docs.geetorus.ing` (production) | Everyone — end users to devs |
| `nightly` | Parent's `main` HEAD | Cloudflare Pages branch preview | Early adopters, contributors |

End users on the latest *released* geetorus must never see docs for unreleased features. That's why nightly drafts live on a separate branch and never touch `main` until a parent release tag drops.
...
- Most of the work is already done by nightly drafts; this mode mainly verifies completeness, stamps `geetorus_version` frontmatter, and opens a single PR: `Release docs for geetorus vX.Y.Z` (merges `nightly` → `main`).
...
gh api "repos/geetorusai/geetorus/commits?path=<removed-path>/src/index.ts&sha=master&per_page=3" \
  -q '.[] | "\(.sha[0:9]) \(.commit.message | split("\n")[0])"'

# Did the basename reappear as a nested path elsewhere in the window?
node -e "require('/tmp/geetorus-sync/window.json').files
...
gh api "search/code?q=<identifier>+repo:geetorusai/geetorus" -q '.items[].path'
```
...
If you spot a typo or broken example on `main` (released docs), fix it on `main` directly — it deploys to `docs.geetorus.ing` on merge. The sync skill merges `main` back into `nightly` at the start of every nightly run, so the fix isn't lost.
...
Both commands spin up an isolated `local_trusted` Geetorus instance on loopback, seed it with demo data, capture light + dark variants at 1440×900 @2x via Playwright, and stamp `captured_sha` / `captured_against` in `registry.json`. The output PNGs go into a PR for human review — never auto-pushed.
