# Proposal: keep `/sync-docs` working under Geetorus's release-channel model
...
that were **not in the release** — Kimi adapter, `GEETORUS_WORKSPACE_REAPER_COOLDOWN_DAYS`,
...
Geetorus has formalized a four-lane release model where `master` runs far ahead
of `stable`.
...
- **release mode** → fires when `gh api …/releases/latest` returns a newer tag than
  `base_release_tag`; targets `main` (→ docs.geetorus.ing) via a PR cut from `nightly`.
...
| `GEETORUS_WORKSPACE_REAPER_COOLDOWN_DAYS` | absent from `.env.example`/config at the tag | manual check |
...
- **GitHub Releases and git tags exist for `stable` only.** All releases are
  ...
  the repo are legacy pre-CalVer package tags (`geetorusai@0.3.x-canary.N`).
- **No git ref for beta/nightly/canary.** `git/ref/tags/{beta,nightly,canary}` →
  404. There is no branch or moving tag to diff against.
- **npm exposes no `gitHead`.** `npm view geetorusai@beta` returns the version
...
- Add an anchor-map watcher for `doc/CHANNELS.md` + `cli/src/commands/channels.ts`
  → `docs/how-to/update-geetorus.md`, `docs/reference/cli/installation.md`, so
