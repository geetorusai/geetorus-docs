...
function runCompare(args, fixtureDir) {
  const r = spawnSync(process.execPath, [COMPARE_SCRIPT, ...args], {
    encoding: "utf8",
    env: { ...process.env, GEETORUS_SYNC_FIXTURE_DIR: fixtureDir },
  });
  return { code: r.status, stdout: r.stdout, stderr: r.stderr };
}
...
// ----------------------------------------------------------------------------
// compare-window:integration — live network test against geetorusai/geetorus
// ----------------------------------------------------------------------------
...
test("compare-window:integration v2026.318.0...v2026.512.0 (live)", () => {
  ...
  const r = spawnSync(process.execPath, [
    ...
    "--repo",
    "geetorusai/geetorus",
  ], { encoding: "utf8", env: { ...process.env, GEETORUS_SYNC_FIXTURE_DIR: "" } });
  if (r.status !== 0) {
...
test("check-drift: parent path drift caught (high confidence)", () => {
  ...
  const r = spawnSync(process.execPath, [scriptInRoot, "--json", "--against", "master"], {
    encoding: "utf8",
    env: { ...process.env, GEETORUS_SYNC_FIXTURE_DIR: fixtures },
  });
...
test("check-drift: cache is keyed by resolved SHA, never a moving ref name", () => {
  ...
  const r = spawnSync(process.execPath, [scriptInRoot, "--json", "--against", "master"], {
    ...
    env: {
      ...process.env,
      GEETORUS_SYNC_FIXTURE_DIR: fixtures,
      GEETORUS_SYNC_CACHE_DIR: cacheDir,
    },
  });
  assert(r.status === 0, `expected exit 0, got ${r.status}; stderr=${r.stderr}`);
  const out = JSON.parse(r.stdout);
...
test("frontmatter: parses a well-formed YAML head and strips it from the body", () => {
  ...
  assert(frontmatter.geetorus_version === "v2026.512.0", `geetorus_version=${frontmatter.geetorus_version}`);
...
test("screenshots: instance env does not forward host credentials", () => {
  ...
  try {
    ...
    const home = resolve(tmpdir(), "geetorus-docs-test-home");
    const env = instanceEnv(home);

    assert(env.HOME === home, `HOME should point at scratch home, got ${env.HOME}`);
    assert(env.GEETORUS_HOME === home, `GEETORUS_HOME should point at scratch home, got ${env.GEETORUS_HOME}`);
...
// parent via GEETORUS_SYNC_FIXTURE_DIR — no network)
// ----------------------------------------------------------------------------
...
test("check-tag-accuracy: flags a post-tag leak, keeps in-tag page clean, scopes to added lines", () => {
  ...
  const r = spawnSync(
    ...
    { cwd: dir, encoding: "utf8", env: { ...process.env, GEETORUS_SYNC_FIXTURE_DIR: fixDir } },
  );
