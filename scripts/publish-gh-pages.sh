...
tmp_root="$(mktemp -d "${TMPDIR:-/tmp}/geetorus-docs-publish.XXXXXX")"
publish_clone="$tmp_root/publish"
site_dir="$tmp_root/site"
origin_url="$(git -C "$repo_root" remote get-url origin)"
pages_base_path="${PAGES_BASE_PATH:-/}"
pages_custom_domain="${PAGES_CUSTOM_DOMAIN:-docs.geetorus.ing}"
...
git config user.name "Geetorus"
git config user.email "noreply@geetorus.ing"
git commit -m "Publish docs site" -m "Co-Authored-By: Geetorus <noreply@geetorus.ing>"
