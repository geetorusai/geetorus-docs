...
const outDir = mkdtempSync(join(tmpdir(), "geetorus-docs-static-routes-"));
const subpathOutDir = mkdtempSync(join(tmpdir(), "geetorus-docs-static-routes-subpath-"));
const SUBPATH_BASE = "/geetorus-docs/";
...
const REPRESENTATIVE_ROUTES = [
  {
    route: "/",
    file: "index.html",
    h1: '<h1 id="landing-title">Everything you need to <em>run Geetorus.</em></h1>',
    contentMarker: '<div class="card-grid" id="landing-cards" data-server-rendered="true">',
    isDocsRoot: true,
  },
  {
    route: "/guides/welcome/what-is-geetorus/",
    file: "guides/welcome/what-is-geetorus/index.html",
    h1: '<h1 id="what-is-geetorus">What is Geetorus?</h1>',
    contentMarker: "Geetorus is the operating system for your AI company",
  },
  {
    route: "/reference/api/overview/",
    file: "reference/api/overview/index.html",
    h1: '<h1 id="api-overview">API Overview</h1>',
    contentMarker: "Geetorus exposes a JSON API for company control-plane work",
  },
  {
    ...
    contentMarker: "Attach a Model Context Protocol (MCP) server to a specific Geetorus agent",
  },
  {
...
try {
  ...
  assert(
    skillsHtml.includes("<title>Skills Reference | Geetorus Docs</title>"),
    "skills route did not get a route-specific title",
  );
  assert(
    skillsHtml.includes('<link rel="canonical" data-seo-managed href="https://docs.geetorus.ing/reference/skills/" />'),
  ...
  assert(
    rootHtml.includes("geetorus/blob/main/CONTRIBUTING.md"),
  ...
  assert(sitemap.includes("https://docs.geetorus.ing/reference/skills"), "sitemap is missing reference/skills");
  ...
  assert(
    sitemap.includes("https://docs.geetorus.ing/reference/skills/bundled"),
    "sitemap is missing reference/skills/bundled",
  );
  assert(
    sitemap.includes("https://docs.geetorus.ing/reference/skills/optional"),
    "sitemap is missing reference/skills/optional",
  );
  assert(
    skillsHtml.includes('class="page-nav-btn prev" href="https://docs.geetorus.ing/'),
    "static docs pages should expose a crawlable previous-page link",
  );
  assert(
    skillsHtml.includes('class="page-nav-btn next" href="https://docs.geetorus.ing/'),
    "static docs pages should expose a crawlable next-page link",
  );

  const robots = read("robots.txt");
  assert(robots.includes("Sitemap: https://docs.geetorus.ing/sitemap.xml"), "robots.txt is missing sitemap reference");
  ...
  for (const { route, file, h1, contentMarker, isDocsRoot } of REPRESENTATIVE_ROUTES) {
    ...
    if (isDocsRoot) {
      ...
      assert(
        !html.includes("Everything you need to <em>run Geetorus.</em>"),
