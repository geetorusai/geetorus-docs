 ...
 *      connects into — a real local Geetorus already on the default 54329).
 *   2. Spawn `pnpm geetorusai onboard --yes --run` inside PARENT_REPO with a
 *      fully isolated env (scratchHome as GEETORUS_HOME, loopback binding,
 ...
 * config.json. A developer's real local Geetorus uses the same default, so an
 ...
 * and fail with `role "geetorus" does not exist`. Keeping the data dir pristine
...
async function generateIsolatedConfig({ home, env }) {
  ...
  const genEnv = { ...env, DATABASE_URL: "postgres://geetorus:geetorus@127.0.0.1:1/none" };
  const gen = spawn("pnpm", ["geetorusai", "onboard", "--yes"], {
    cwd: PARENT_REPO,
    env: genEnv,
    stdio: "pipe",
    detached: true,
...
async function main() {
  ...
  try {
    ...
    console.error(
      `run: PARENT_REPO does not exist: ${PARENT_REPO}\n` +
        "      Set the GEETORUS_REPO env var to the correct path.",
    );
    process.exit(1);
  }
  ...
  async function cleanup() {
    if (server) {
      // `pnpm geetorusai onboard --run` spawns child processes (the actual
  ...
  console.log("run: starting Geetorus onboard server…");
  console.log("run:   cwd =", PARENT_REPO);
  console.log("run:   home =", home);

  server = spawn("pnpm", ["geetorusai", "onboard", "--yes", "--run"], {
    cwd: PARENT_REPO,
    env,
    stdio: "pipe",
  ...
  // Geetorus's stalled-work sweep keeps moving those issues to `blocked` with
