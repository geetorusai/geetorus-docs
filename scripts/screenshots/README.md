...
1. **Parent repo present.** The pipeline spins up a real Geetorus instance from source. Set `GEETORUS_REPO` to its absolute path, or ensure it lives at the default location `~/Documents/GeetorusAI/geetorus` (the default resolved by `config.mjs`). See [geetorus-localdev-runbook.md](../../../GeetorusAI/geetorus-localdev-runbook.md) conceptually — the runner follows the same `local_trusted` / `private` / `loopback` isolation pattern.
2. **Playwright Chromium installed.** Run once after `npm install`:
   ```sh
   npx playwright install chromium
   ```
3. **`pnpm` available.** The runner starts the parent's onboarding entrypoint via `pnpm geetorusai onboard --yes --run`.
...
1. **Isolated instance.** `run.mjs` spawns `pnpm geetorusai onboard --yes --run` inside the parent repo with a throw-away `HOME` / `GEETORUS_HOME` under `os.tmpdir()`, `PORT=3197`, `GEETORUS_BIND=loopback`, `GEETORUS_DEPLOYMENT_MODE=local_trusted`, and `GEETORUS_DEPLOYMENT_EXPOSURE=private`. The child process receives only a small allowlist of runtime environment variables, so real database URLs, provider keys, and GitHub tokens are not forwarded.
...
- **Never run against live data.** The pipeline always talks to the seeded demo instance on loopback port 3197. `GEETORUS_BIND=loopback` ensures no external traffic.
- **Safe to run alongside your real local Geetorus.** The screenshot instance pins its embedded Postgres to a free port (see "Isolated instance" above), so you do not need to stop a real instance first. It will not touch the real database on 54329.
