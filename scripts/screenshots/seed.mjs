/**
 * seed.mjs — create a rich, realistic demo dataset on a running Geetorus
  ...
  if (!companyExisted && agentList.length) {
    await step("seed cost + finance events", async () => {
      ...
      const finance = [
        ...
        { eventKind: "platform_fee", biller: "Geetorus", amountCents: 4_900, direction: "debit" },
        { eventKind: "credit_purchase", biller: "Anthropic", amountCents: 50_000, direction: "credit" },
        { eventKind: "log_storage_charge", biller: "Geetorus", amountCents: 1_200, direction: "debit" },
      ];
      let financeEvents = 0;
    ...
    if (!issue) {
      issue = await post(
        C("/issues"),
        {
          ...
          description:
            "Upstream shipped the fix in https://github.com/geetorusai/geetorus/pull/8512 — verify it covers our case, tracked upstream in https://github.com/geetorusai/geetorus/issues/4419.",
          status: "in_progress",
          priority: "medium",
 ...
 * issue they were woken for, and Geetorus treats a successful run with no
