...
/**
 * GEETORUS_INSTANCE_ID for the throw-away screenshot instance. Shared by
 * instanceEnv() (passed to the spawned server) and the DB-seeding helper, which
 * needs it to locate the embedded-postgres data dir under GEETORUS_HOME.
 */
 ...
 * assumes 54329 is free, because a developer's real local Geetorus uses it too.
 */
...
/** Absolute path to the geetorus-docs repo root (two levels up from this file). */
...
/**
 * Absolute path to the parent Geetorus repo.
 * Override via GEETORUS_REPO env var if your checkout lives elsewhere.
 */
export const PARENT_REPO = resolve(
  process.env.GEETORUS_REPO ||
    resolve(os.homedir(), "Documents/GeetorusAI/geetorus"),
);
 ...
 * Used as GEETORUS_HOME so the real ~/.geetorus is never touched.
 */
export function scratchHome() {
  return resolve(os.tmpdir(), "geetorus-docs-shots-home");
}
...
export function instanceEnv(home) {
  ...
  return {
    ...
    PORT: String(PORT),
    GEETORUS_HOME: home,
    GEETORUS_INSTANCE_ID: INSTANCE_ID,
    ...
    // `.geetorus/config.json` (see server/src/paths.ts) BEFORE honoring
    // GEETORUS_HOME. Since onboard runs with cwd = PARENT_REPO, a developer's
    // real `.geetorus/config.json` in the parent repo would be picked up,
    // binding the screenshot run to the real instance's DB. Pin GEETORUS_CONFIG
    // to the scratch instance's config path so onboard reads/writes the
    // isolated config instead.
    GEETORUS_CONFIG: instanceConfigPath(home),
  };
}
 ...
 * `${GEETORUS_HOME}/instances/${instanceId}/config.json` layout. We rewrite
 ...
 * Geetorus (or anything else) is already using, so a screenshot run started
 ...
 * developer's real local Geetorus uses, so guessing it could connect a direct
 ...
 * not a dependency of geetorus-docs). Caller must `await sql.end()`.
 */
export function openInstanceDb(home = scratchHome()) {
  ...
  return postgres(`postgres://geetorus:geetorus@127.0.0.1:${port}/geetorus`, {
    max: 1,
    idle_timeout: 5,
    onnotice: () => {},
  });
...
export function mintAgentJwt(
  ...
  try {
    ...
    secret = /^GEETORUS_AGENT_JWT_SECRET=(.*)$/m.exec(env)?.[1]?.trim();
  } catch {
    return null;
  }
