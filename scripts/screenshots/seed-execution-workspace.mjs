 ...
 * GEETORUS_HOME. The `postgres` client is resolved from the parent repo's
 * node_modules (it isn't a dependency of geetorus-docs).
 */
...
function createDemoWorktree(prefix) {
  const dir = resolve(os.tmpdir(), "geetorus-docs-shots-worktree", `${prefix}-204`);
  const files = {
 ...
 * developer's real local Geetorus uses, so guessing it could connect this
