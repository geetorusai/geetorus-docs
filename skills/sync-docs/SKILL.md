---
name: sync-docs
description: Keep geetorus-docs in lockstep with the parent geetorusai/geetorus codebase. Detects code-surface changes (CLI, env vars, API routes, adapters, plugin SDK, schemas) and produces friendly-tutorial-voice doc updates. Two modes — `nightly` (tracks parent's default branch (currently `master`), targets the `nightly` branch, preview deploy) and `release` (on parent tag, merges nightly → main, tags, ships live). Trigger phrases:  "sync docs", "update docs from geetorus", "/sync-docs", "check for geetorus changes", "is docs current".
---

# /sync-docs — Geetorus docs sync skill

This skill keeps **geetorus-docs** in lockstep with the parent code repo **geetorusai/geetorus**, rewriting changes in our friendly-tutorial voice for an audience that spans end users, operators, and developers.
...
| Deploys to | Cloudflare Pages branch preview | docs.geetorus.ing |
...
The branch model is non-negotiable: end users on the latest *released* geetorus must never see docs for features that aren't out yet.
...
- `docs/reference/changelog.md` — **release mode only**: append one accordion entry per release (Phase 6.5). User-facing and linked from the nav; nothing else writes it. Writing the entry is only half the job — it is not done until `docs.geetorus.ing` is serving it (Phase 9).
...
2. Fetch parent's latest **stable** release: `gh api repos/geetorusai/geetorus/releases/latest -q '.tag_name'`. `releases/latest` already excludes prereleases, but **guard defensively**: the release target MUST match `^v?\d{4}\.\d{1,4}\.\d+$` (clean CalVer). Any `-beta`/`-nightly`/`-canary` tag is never a release target — if the API ever returns one, treat the run as nightly mode and note it in the summary.
...
```
gh api repos/geetorusai/geetorus/compare/$PREV...$NEXT \
  -q '.files[] | {filename, status, additions, deletions, patch}'
```

Cache result under `/tmp/geetorus-sync/<sha>/` so we don't refetch within a run.
...
>
> Leaf responses are cached under `/tmp/geetorus-sync/` so re-runs within a day are cheap.
...
|---|---|---|
| parent-path-missing | `cli/src/...`, `server/src/...`, `packages/<name>/...`, `skills/geetorus/...` references with `.ts`/`.mjs`/`.js` extensions | high |
| cli-command-missing | `geetorusai <subcommand>` invocations under `docs/reference/cli/**` | high |
...
Output is structured JSON with a `drift` array of records: `kind`, `doc` (file:line), `documented`, `parent_searched`, `confidence`, `suggest`. The script always exits 0 — drift is a warning, never a hard failure. Results are cached under `/tmp/geetorus-sync/drift-<ref>/` so re-runs within a day are cheap.
...
- watcher: cli-commands
  ...
  surface: "geetorusai worktree prune"
...
Write the manifest to `/tmp/geetorus-sync/manifest.yaml` and, in nightly mode, **regenerate** `PENDING.md` at repo root from this manifest (overwrite — do not append). Compute `manifest_hash = sha256(canonical manifest yaml)` for use in the reconciliation step below.
...
> **Builder note.** `site/build-release.mjs` strips YAML frontmatter from the rendered markdown body and surfaces parsed fields into `content.json`. `geetorus_version` is **internal metadata only** — the SPA does NOT render it on-page. Authored pages SHOULD still include `geetorus_version` in release mode so the field is queryable from `content.json` (e.g., to drive future per-page version badges or release-PR diff tooling) and so the source of truth for "which release does this page document" lives next to the prose.
...
> - Add `geetorus_version: <tag>` to the frontmatter of touched pages in release mode; leave alone in nightly mode (nightly pages are versionless until they merge to main).
...
Phase 5.5 verifies the edits *this run* authored. This phase verifies **everything the release would ship** — including drafts nightly authored in earlier runs — against the stable tag. It exists because our `nightly` branch tracks parent `master` (≈ canary), which runs far ahead of the stable tag, so a release branch cut from `nightly` inherits post-tag `master` drafts as leaks (this is exactly how Kimi, `GEETORUS_WORKSPACE_REAPER_COOLDOWN_DAYS`, and a mission-less onboarding rewrite reached a release branch). Skip entirely in nightly mode.
...
5. Bump the page's own `geetorus_version` frontmatter to the release tag.
...
3. Stage edits.
4. Commit strategy:
   - Nightly auto-merge edits → single commit titled `nightly: <surface name> (geetorus <short-sha>)`.
   - Nightly PR-tier edits → branch `nightly-draft/<short-sha>-<surface>` off `nightly`, open PR against `nightly`.
   - Release mode → branch `release/v2026.X.Y` off `nightly`, open PR against `main` titled `Release docs for geetorus v2026.X.Y`. PR body = manifest + screenshot staleness + structured sections (below) + checklist.
...
**A release is done when `docs.geetorus.ing` serves it, not when the PR merges.**
...
2. **Verify production is serving the release.** Wait a few minutes for the build,
   ...
   ```sh
   curl -s -o /dev/null -w '%{http_code}\n' https://docs.geetorus.ing/<new-page-route>/
   curl -s https://docs.geetorus.ing/<changed-page-route>/ | grep -c '<new claim>'
   curl -s https://docs.geetorus.ing/reference/changelog/ | grep -o 'Docs for v[0-9.]*' | head -1
   ```
...
| Field | Rule |
|---|---|
| `seo_title` | Required. Unique across all pages. ≤ 43 chars, because the build appends `" \| Geetorus Docs"` and the total must stay ≤ 60. Must not contain `\|`. |
...
| Build fails after edits | Roll back edits in working tree, save manifest to `/tmp/geetorus-sync/failed-manifest.yaml`, tell user which entry caused it. |
...
| Release merged but `docs.geetorus.ing` still serves the previous build | Phase 9. Confirm `npm run docs:build` succeeds on `main` (rules out a build break), then republish with `npx wrangler pages deploy .site --project-name geetorus-docs --branch main`. If that fails too, escalate — the Pages project or its GitHub connection is broken. Never report the release as shipped on the strength of the merge alone. |
