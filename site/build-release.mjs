...
const defaultSiteUrl = "https://docs.geetorus.ing";
const defaultSeoDescription = "Guides, references, and walkthroughs for running Geetorus, an AI company operating system for agent teams, governance, budgets, and workflows.";
...
Options:
  --base-path <path>  Public URL base path for the uploaded docs bundle.
                      Examples: /, /docs/, /random/geetorus-docs/, auto
...
async function detectAppBasePath() {
  ...
  output = output.replace(
    ...
    if (!isNavPayload(navData)) {
      throw new Error("content.json did not match the expected Geetorus docs schema.");
    }
  } catch (e) {`,
  );
...
function getDeploymentBasePath(basePath) {
  return basePath === "auto" ? "/geetorus-docs/" : basePath;
}
...
function buildJsonLd(metadata) {
  const graph = metadata.page
    ? {
        ...
        "@type": "TechArticle",
        headline: metadata.title.replace(/ \| Geetorus Docs$/, ""),
...
function injectSeo(html, metadata, { baseHref = null } = {}) {
  ...
  const tags = [
    ...
    `<meta property="og:site_name" data-seo-managed content="Geetorus Docs" />`,
...
async function pageMetadataForNav(nav, outDir, siteUrl, basePath) {
  ...
  for (const { page, section } of navPages) {
    ...
    const pageTitle = authoredTitle || page.title || h1 || "Geetorus Docs";
    pages.push({
      page,
      sectionTitle: section.title,
      title: `${pageTitle} | Geetorus Docs`,
...
function buildNotFoundPage(siteUrl, basePath) {
  const metadata = {
    title: "Not found | Geetorus Docs",
    description: "This Geetorus Docs URL does not exist.",
...
<body>
  <main>
    <h1>Not found</h1>
    <p>This Geetorus Docs URL does not exist.</p>
...
function buildNginxConfig(basePath) {
  ...
  const placeholderComment = basePath === "auto"
    ? "# Replace /geetorus-docs/ with the public mount path for this bundle before using this snippet.\n"
    : "";
  return `${placeholderComment}# Geetorus docs static SPA
...
\`\`\``
  ...
  return `# Geetorus Docs Release Deployment
...
- The \`geetorusai/geetorus-docs\` repository is connected to Cloudflare Pages; do not deploy this bundle with Wrangler for normal docs releases.
- Pushing \`main\` triggers the production deployment. Cloudflare serves it on \`docs.geetorus.ing\`, the project domain, and a deployment-specific \`https://<hash>.geetorus-docs-74t.pages.dev\` URL.
- Pushing any other branch triggers a preview/canary deployment. In the Cloudflare Pages dashboard, open **Workers & Pages -> geetorus-docs -> Deployments** and use the row whose source branch and commit match your push.
- Canary URLs are Cloudflare-generated deployment URLs, for example \`https://92b9a99c.geetorus-docs-74t.pages.dev\`; do not derive them from the branch name by hand.
...
async function main() {
  ...
  // surface fields like `geetorus_version` per page.
  ...
  const betaMetadata = { ...rootMetadata, title: "Welcome to the Geetorus hosted beta", description: markdownDescription(betaMarkdown), url: siteUrlForPath(options.siteUrl, options.basePath, "hosted-beta/"), page: betaPage };
  ...
  if (options.basePath === "auto") {
    console.warn("Warning: --base-path auto is less robust for deployed subdirectory hosting. Prefer an explicit path such as /random/geetorus-docs/.");
  }
